...
```text
WMS 入库药检单入口
  -> outbox / 异步投影（正常目标 <= 30 秒）
  -> 独立客户平台服务 + 独立 PostgreSQL
     -> 本地账号/地址/订单范围授权
     -> H-FILE 只读药检单对象
     -> 自有导出前缀（异步 ZIP）
```

- 客户查询、预览和下载只访问独立平台，不读取 WMS API 或数据库。
- 客户管理员可管理本客户多账号；普通账号绑定一个或多个客户地址，未绑定地址时无订单权限。
- 平台仅展示已发货/已签收订单及其实际商品、批号和当前已确认药检单；历史版本权限按账号配置，默认关闭。
- WMS 原件不可变；客户副本统一为 PDF，按“人工选择图像处理 → 审核 → 盖货主图章”顺序生成。
- 单文件下载 URL 有效 15 分钟；批量任务最多 200 个去重后报告且不超过 2 GB，ZIP 保留 7 天并附缺失清单。
- 图章仅为视觉标记，不代表电子签名；签名失效的源 PDF 可生成副本，但只在平台页面警告。
- 派生失败不得回退为未盖章原件，也不得阻塞 WMS 药检单确认或出库；平台显示“处理中/资料暂缺”。
- 详细业务决策见 [ADR-0042](../adr/0042-drug-inspection-customer-portal.md)。
