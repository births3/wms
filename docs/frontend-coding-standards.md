...
- 只读详情统一使用 `detailAction`，放在“功能能力”区；行内操作列不再重复放通用详情入口。
...
- DataGrid 顶部工具栏分为“功能能力”和“私有能力”两区；刷新、详情、视图、字段显示、打印、导出等公共能力放“功能能力”，收货、验收、上架等模块动作放“私有能力”。
- 打印和导出统一使用 `printAction`、`exportAction`；默认显示为“打印”和“导出 Excel”，不得在 DataGrid 页面恢复“导出 CSV”文案。
...
- 修改 `DataGrid` 标准动作能力后必须运行 `node packages/ui/self-checks/data-grid-export-self-check.mjs` 和 `pnpm --filter @wms/web-admin build`。
