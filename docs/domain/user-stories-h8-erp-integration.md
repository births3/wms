...
**我要** 在 M1 系统配置中心为每个货主管理一个或多个 ERP 连接，并限定仓库、方向、消息类型、通道和凭证引用

**以便** ERP 报文按唯一、受控、可审计的规则进入或离开 WMS

### 验收标准

1. **配置入口与存储边界**：复用 M1 系统配置中心，新增“ERP 连接”配置域，不新增 H8 独立菜单；M1 只提供入口和配置域承载，连接数据存入 H8 专用表，不塞入通用键值表。
...
14. **权限与审计**：`m1.config.read` 授予系统管理员和仓库主管查看配置及测试结果；`m1.config.write` 只授予系统管理员执行新增、编辑、测试、启用、停用和删除。所有新增、编辑、测试、启用、停用、删除和自动主备切换均写入 H2 append-only 审计，记录货主、连接、配置版本、旧值/新值摘要、操作者或系统主体、时间和结果，不记录明文凭据。
15. **幂等与并发**：所有写动作必须支持 Idempotency-Key 或等价业务幂等键；编辑和状态动作按配置版本做乐观并发控制，重复启用、停用或测试不得产生重复配置转换和重复审计业务事件。
16. **页面证据**：因为复用既有 M1 菜单，本故事不触发“新增菜单页”截图门禁；但 ERP 连接配置属于新的前端交互，完成实现时仍必须用真实浏览器 E2E 覆盖新建、测试、启用、重叠拒绝、停用和只读权限，并至少归档一张测试通过后启用成功的真实数据截图。
...
| L8 权限 | 系统管理员具备 `m1.config.read/write`；仓库主管只有 `m1.config.read`；跨货主和越权写入拒绝 |
...
2. **H-INT 契约**：弹性复用 ADR-0018，字段规整复用 M-PM，凭证复用 ADR-0013，审计复用 H2，禁止在 H8 另造通用集成平台。
...
6. **完成声明**：本故事达到 S4，只有在配置中心前后端、H8 专用表、权限审计、真实数据 E2E 和真实 ERP dev/staging 证据齐全后才能标记完成；现有本地 Worker/Mock 不可替代。
