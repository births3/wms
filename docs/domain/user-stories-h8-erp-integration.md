...
1. **配置入口与存储边界**：在基础能力下提供 H8 集成中心菜单树，叶子页「H8 ERP 连接」（`view_id=h8-erp-connectors`）为正式入口；连接数据存入 H8 专用表，不塞入通用键值表。权限仍复用 `m1.config.read` / `m1.config.write`（系统管理员写、仓库主管只读），不另造业务域权限键。不把 ERP 连接嵌在 M1 Feature Flag 页内。
