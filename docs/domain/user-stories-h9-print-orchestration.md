...
> 性质：H9 第二阶段横向能力
> 状态：US-H9-006 已完成 V0-V3 软件验收；US-H9-007～015 的业务规则、安全残余风险、
> 机器身份协议和多货主物理打印站点已确认，软件与适用真实硬件证据待实现
...
- US-H9-006 已完成 V0-V3；US-H9-007～015 保持延期，继续按 outside-in TDD 分批实现。
- `print_document_category` 已由 US-H9-006 增加 `delivery_note` 受控分类；US-H9-008 再扩展
  药检单、发票及其 `source_mode`，不倒推扩大 US-M1-011 已验收范围。
