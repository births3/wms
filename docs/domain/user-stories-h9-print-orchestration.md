...
> 性质：H9 第二阶段横向能力
> 状态：业务规则、安全残余风险、机器身份协议和多货主物理打印站点已确认；软件与真实
> 硬件证据未实现
...
- `print_document_category` 是 US-H9-008 的待实现 M1 系统字典扩展，不计入已完成的
  US-M1-011 验收范围；实现 H9 时再同步扩展 M1 契约、种子、测试和质量矩阵。
