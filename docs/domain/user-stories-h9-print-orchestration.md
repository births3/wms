...
> 性质：H9 第二阶段横向能力
> 状态：US-H9-006、US-H9-007、US-H9-008、US-H9-011 已完成 V0-V3 软件验收（US-H9-011
> 的真实物理打印机/USB/Print Agent 证据待 S4 硬件验收；US-H9-008 的 H-FILE 摄取以受控
> 占位登记表承载，见验收记录缺口清单）；US-H9-009～010、012～015 的业务规则、
...
- US-H9-006/007/008 已完成 V0-V3；US-H9-009～015 保持延期，继续按 outside-in TDD 分批实现。
...
- US-H9-008 的组套版本、四层解析、就绪/失败策略与实例快照已闭环；`external_file` 权威文件
  经 `h9_ingested_document_files` 占位登记表承载，H-FILE 摄取与短期 URL 随 US-H9-009 收口。
...
- ADR-0040 已关闭机器身份、普通 HTTP 白名单、一次性秘密重放和 pilot 授权边界；
  ...
  US-H9-009～010、012～015 的软件与适用外部证据仍未实现。
