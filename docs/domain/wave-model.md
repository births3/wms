...
- **类型**（模板必选属性，**每类型独立模板**，一个模板只对应一种类型）：
  ...
  - `disposal`（处置类：不合格品出库/隔离处置/报损销毁——算单只允许带锁库存，锁-区域约束内作业，与普通波次不混编）
    - **处置类单据实体**：`outbound_orders.document_type` 扩展 `disposal_outbound`（不合格品出库）/ `quarantine_outbound`（隔离处置）；报损/销毁走既有 M3 报损报溢单流程（不建处置出库单）；处置类出库单创建需质量角色权限（`m1.quality-lock.manage` 或质量岗）并关联 M-QL 单号，作为带锁库存出库的审批源凭证；处置类单据行携带锁标记（关联锁事件），波次按锁类别匹配处置类型。
