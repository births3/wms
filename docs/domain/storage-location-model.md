...
- **散货位（`allows_container = false`）无容器可锁**：散货商品异常不走容器质量锁，走既有 M3 质检流程（批次 `status` 置 `quarantined`/`unqualified` + `qty_frozen` 部分冻结，审批源为 M3 质检/养护流程）；6 维校验③ 散货位读批次 `status` 即为此路兜底。
...
**⑥ 一致性校验**：批次 `status` ↔ 所在 `zone.quality_color` 强约束——`qualified` 批次不得位于质量区；`quarantined` 不得位于合格/不合格区；`unqualified` 不得位于合格/隔离区；加锁、移库、解锁各环节校验，治理对账脚本兜底（发现错位告警）。
