...
**⑦ 质量区滞留时限（GSP 85 条"及时处理"落点）**：**仍带锁**的容器在质量区滞留超期触发告警质量管理员（H4 企微 + 大盘高亮）——不合格品区默认 **7 天**、隔离区默认 **3 天**（均参数可配置），超期未处置强制进入人工复核流程（重新加锁/转报损/解锁，走正常质量锁流程与审计）；**已解锁待移回合格区的容器不适用滞留时限**（其出口由 ⑥ 错位告警覆盖：status 已 qualified 但位于质量区 = 错位，告警后人工移回）。
...
- **保留策略**：质量锁事件表为审计证据，**只增不删**，保留期 ≥ 5 年且不少于相关药品有效期后 1 年（GSP 记录保存要求）；`iot_event_logs` 同策略（设备事件证据）；归档压缩可选，不物理删除。
...
- **属性批量填充**：生成时直接批量赋予作业形态（`location_type`）、温区及容器开关、路径顺序字段（`pick_sequence_no` / `putaway_sequence_no`，按巷道-排-层-格自然序，支持巷道方向翻转参数）。
- **导入导出**：支持 Excel 标准格式批量导入与全仓导出（顺序字段可覆盖）。
- **权限**：向导生成操作需 H1 权限点 `m1.location.batch-generate`（实施时并入 Phase 1），无权限拒绝操作。
