...
| P1 已关闭 | 容器质量锁、6 维上架骨架、库位向导 | 只消费接口，不改语义。例外条款（先决确认拍板）：阶段 A 收口必须清零十条（8c611ef2/45904508/203f9f1e/afe41bfe 等容器锁 M-QL 联动、向导 AGV/CSV 扩展、上架推荐位 6 维正交流水线）作为合法前置基线生效，本切片承认并只读消费；Wave 4 出库 16 路由作为跨模块消费接口，不纳入 Phase 2 工单范围 |
