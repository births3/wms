...
- 关联 ADR：ADR-0015 多端规则；ADR-0001（当前 RN 正式方向）；ADR-0027 PDA 离线模型与技术栈定版框架（Proposed）
...
| ID | 假设 | 验证方式 |
|----|------|---------|
| H1 | RN bare（CLI）在中端 Android PDA 上接入实体扫码键 / 厂商扫码通道后，扫码反馈延迟 < 500ms | 在真 PDA 上用同一批 50 个脱敏条码样本扫描各 5 次取 P95；手机摄像头不能作为 SPIKE-005 evidence |
...
| 状态 | 条件 |
|------|------|
| accept | H1 / H3 / H4 / H5 全部确认；H2 至少 1 款 PDA 成功；H6 工具链选定；更新 ADR-0027 + spike 代码 |
| reject | H1 不成立（扫码 > 1s）→ 候选改用 native Android Java + RN bridge，或对照 SPIKE-005B WebView/Capacitor 结论复核 |
...
├── package.json              # spike-local package，不加入 pnpm production workspace
├── ios/                      # 暂不验证，但保留
├── android/
├── src/
│   ├── App.tsx
...
- 接入 PDA 实体扫码键 / Intent / DataWedge / KeyEvent 通道
- ScanScreen 上扫 50 个测试条码（GS1 + Code128 + 二维码各类）
- 测延迟：实体扫码键触发 ↔ JS 拿到字符串并完成 UI 反馈的 ms 数
...
### 步骤 7：更新 ADR-0027（0.5 小时）

`docs/adr/0027-pda-offline-model.md` 当前为 Proposed。SPIKE-005 启动并完成真 PDA evidence 后，追加：
- RN bare app 的实测结论
- 队列结构 + 持久化技术
- Token 状态机（图）
- 实体扫码键集成模式
- 与 H1 / H2 的服务端衔接（idempotency_key 头部）
- 与 SPIKE-005B WebView/Capacitor 的同口径对比
...
| 风险 | 概率 | 影响 | 后备方案 |
|------|------|------|---------|
| 实体扫码键 / 厂商扫码通道反馈 > 500ms | 中 | 高 | 换纯 native 扫码控件 + bridge，或对照 SPIKE-005B 结论复核 |
...
| 项 | 在当前环境跑的问题 |
|----|----------|
| H1 实体扫码键反馈延迟 < 500ms | 需要真 PDA + 实物条码 / 二维码；emulator 模拟摄像头和手机摄像头结果无意义 |
...
| H4 JWT 离线缓存（与 SPIKE-001 衔接） | SPIKE-001 已定型双 token + 5 状态机文档（[PDA 离线 token 状态机](spike-001-pda-offline-state.md)），代码实施在 PDA 端，需要 H1/H2 验证后才能写 |
...
本 spike 文档（5 假设 + 实施路径 + 风险表）**完整保留**，启动时按既有结构填 §7 v0.2 修订记录。[PDA 离线 token 状态机](spike-001-pda-offline-state.md)（已在 spike-001 落盘）继续作为 PDA 端 token 状态机参考，本 spike 启动时直接复用。
...
用户确认：先落 readiness/runbook，不引入 RN 生产依赖，不创建 `apps/pda-mobile` 生产 app。2026-06-06 起，§7.8 将该口径扩展为不引入 RN / Expo / EAS / Capacitor 生产 workspace 依赖，并把 SPIKE-005B 纳入同口径对比。
...
### 7.8 WebView/Capacitor 候选补充（2026-06-06）

用户确认：保留本 RN Spike，不直接替换 ADR-0001；新增 [SPIKE-005B](spike-005b-webview-capacitor-pda.md) 评估 Web UI + Android native shell + native plugins。

执行口径：

- SPIKE-005 与 SPIKE-005B 使用同一台真 PDA、同一批条码样本、同一套 M2/M3 dev/staging 测试数据和同一套 evidence 引用标准。
- 纯浏览器 / 纯 PWA 不进入生产候选；WebView/Capacitor 方案必须通过 native plugin 接实体扫码键、离线持久化和硬件打印能力。
- 最终 PDA 技术栈不在本文件直接改定，统一由 ADR-0027 基于两组 Spike 证据定版。
...
- 待填：证据引用 `docs/retros/wave-3-pda-runtime-evidence.json`，并记录设备资产、扫码输入方式和条码样本。
