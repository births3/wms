...
| `MSG_*` / `WMSFE_MSG_*` | 系统消息日志 | 28M + 21M + 14M | H4 通知 + H8 ERP 防腐层 | 🟡 缺独立日志表故事 |
| `P_ANALYZE_PIX_*` | 接口处理日志 | 24M | H8 ERP 防腐层日志 | 🟡 同上 |
...
| # | 现有表族 | 缺失原因 | 建议补充 | 优先级 |
|---|--------|--------|--------|------|
| 1 | `MSG_LOG` / `WMSFE_MSG_LOG` / `LABOR_MSG_DTL` 共 64M 行 | 本项目无独立"系统日志表"故事；H4 通知和 H8 ERP 接口日志只是顺带提及 | **H8 ERP 防腐层加 §消息日志持久化故事**（独立表 + 按月分区 + 7 天保留 + 故障重放）| 🔴 P0 |
| 2 | `P_ANALYZE_PIX_LOG` 24M 行 | 本项目无 PIX 接口日志 | 同上 | 🔴 P0 |
...
| 消息日志（MSG_LOG / WMSFE_MSG_LOG）| 不迁移（保留期 7 天即可，新系统从零开始）|
...
4. **消息日志类表是隐藏的数据爆点**（合计 64M 行 + 24M PIX 日志 = 88M 行），本项目 H8 防腐层必须加独立日志故事。
