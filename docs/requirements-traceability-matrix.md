...
| ERP 连接凭据、路由与审计 | US-H8-001 | 外部 ERP 凭据泄露、跨货主/跨仓误路由、双通道重复投递、越权启停和历史配置不可追溯 | secret alias（`secrets.rs`）、`h8.erp_connector.read/write`、路由唯一、Idempotency-Key、H2 审计、failover 非双写 | E2E 只读 403；Vault alias 测试；`h8-local-integration-evidence.json` | 部分覆盖 | 控制与联调证据已齐；客户正式 ERP S4 回执仍缺。 | 正式厂商联调时归档请求、回执和审计关联。 |
