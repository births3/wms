...
| ERP 连接凭据、路由与审计 | US-H8-001 / US-H8-002 | 外部 ERP 凭据泄露、跨货主/跨仓误路由、双通道重复投递、越权启停和历史配置不可追溯 | secret alias（`secrets.rs`）、`h8.erp_connector.read/write`、路由唯一、不可变版本快照、Idempotency-Key、H2 审计、failover 非双写 | E2E 只读 403；Vault alias 测试；PostgreSQL 历史版本、Worker 原版本读取及人工重放即时接管测试；出站连接货主/目录/仓库收窄与逐消息 `route-resolve` 测试；`h8-local-integration-evidence.json` | 部分覆盖 | 人工重放桥、出站入库完成唯一路由切片和 PostgreSQL 接管已验证；其余出站源仓库标识、真实 MSSQL 运行证据及客户正式 ERP S4 回执仍缺。 | 补其余出站源的仓库路由标识、Docker MSSQL 重放运行证据，并在正式厂商联调时归档请求、回执和审计关联。 |
