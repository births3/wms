...
| H1 PC 三层菜单管理 | US-H1-007 | `apps/web-admin` 基础能力 / 平台能力 / H1 菜单管理 | [h1-menu-management-design.md](h1-menu-management-design.md) | 部分覆盖 | 已覆盖后端菜单表、草稿/发布/版本 API、前端三层菜单消费、菜单管理页、按钮权限点维护、OpenAPI 和 api-client；9002 真实截图归后续证据收口。 | 后续补角色授权矩阵、真实用户权限组合 E2E、9002 截图归档和发布回滚操作证据。 |
| M-CG PC 单据号规则管理 | US-CG-001 / US-CG-002 | `apps/web-admin` 系统管理 / 单据号规则菜单 | 待补真实管理页设计与截图 | 待补证据 | PC 规则管理页、规则预览、动作测试、9002 真实截图和审计展示尚未实现；后端公开 API 与 api-client 已覆盖规则列表、upsert、启停和生成记录查询。 | 后续切片补 PC 管理页、动作测试、真实截图、审计展示和 M2/M3/M4 创建单据接入。 |
| H9 PC 打印模板中心 | US-H9-001 / US-H9-002 / US-H9-003 / US-H9-004 / US-H9-005 | `apps/web-admin` 基础能力 / H9 打印模板菜单 | 已补字段库列表页；既有 `pc-m2-009` 原型仅作历史参考 | 部分覆盖 | H9 独立菜单、字段库读取 API 和字段库列表页已实现；模板类型字典、模板主数据、hiprint 设计器、预览打印、9002 真实截图和货主覆盖仍未实现。 | 后续补模板类型字典接入、模板版本、打印记录、hiprint 设计器、浏览器预览打印、动作测试和真实截图证据。 |
...
| M-CG 单据号生成后端第一切片 | US-CG-001 / US-CG-002 | `backend/crates/api/src/lib.rs` code-generator OpenAPI；`shared/openapi/openapi.json`；`packages/api-client/src/schema.ts` | `backend/crates/api/src/document_numbering_handlers.rs`；`backend/crates/api/src/document_numbering_repository.rs`；`backend/crates/api/src/document_numbering.rs` | `backend/migrations/202607020001_mcg_document_numbering.sql`；`document_number_rules`、`document_number_counters`、`document_number_allocations`；`idempotency_request`；`audit_event` | `backend/crates/api/tests/document_numbering_postgres.rs` | 部分覆盖 | 已覆盖 no-gap 发号、计数器、生成记录、调用方事务接口、规则列表、规则 upsert、规则启停、读写权限、L11 幂等和 OpenAPI/api-client；配置审批、PC 前端、E2E/截图和业务模块接入仍未实现。 | 后续切片补 PC 管理页、动作测试、9002 截图、M2/M3/M4 创建单据接入和专项治理脚本。 |
| H9 打印模板引擎 | US-H9-001 / US-H9-002 / US-H9-003 / US-H9-004 / US-H9-005 | `backend/crates/api/src/lib.rs` print-template OpenAPI；`shared/openapi/openapi.json`；`packages/api-client/src/schema.ts` | `backend/crates/api/src/print_template_handlers.rs`；`backend/crates/api/src/print_template.rs` | `backend/crates/api/src/print_template.rs`；`backend/migrations/202607050002_h9_print_template.sql`；字段库、字段库版本、字段定义表 | `backend/crates/api/tests/print_template_postgres.rs`；`just openapi-check`；`pnpm --filter @wms/web-admin build` | 部分覆盖 | 已覆盖字段库发布、版本不可改写、幂等重放、审计写入、字段库读取 OpenAPI/api-client 和 PC 字段库列表页；模板类型字典、模板主数据、hiprint JSON、打印记录、真实截图和打印预览仍未实现。 | 按 ADR-0036 继续补模板类型字典接入、模板版本、打印记录、hiprint 设计器、浏览器预览打印、9002 真实截图和动作测试。 |
...
| M-CG 单据号生成 | US-CG-001 / US-CG-002 | `cargo test --manifest-path backend/Cargo.toml -p wms-api --test document_numbering_postgres`；`cargo test --manifest-path backend/Cargo.toml -p wms-api --lib openapi_contains_wave1_contract_paths`；`just openapi-check` | `backend/crates/api/tests/document_numbering_postgres.rs`；`backend/crates/api/src/lib.rs`；`shared/openapi/openapi.json`；`packages/api-client/src/schema.ts` | 部分覆盖 | 后端/API 切片已有测试与契约；PC 前端、9002 截图、PDA、M2/M3/M4 创建单据接入和治理脚本专项扫描未覆盖。 | 后续补 PC/E2E/截图、业务接入和专项治理脚本；新增 API 时继续保持 OpenAPI/api-client 同步。 |
| H9 打印模板引擎 | US-H9-001 / US-H9-002 / US-H9-003 / US-H9-004 / US-H9-005 | `cargo test --manifest-path backend/Cargo.toml -p wms-api --test print_template_postgres`；`just openapi-check`；`pnpm --filter @wms/web-admin build`；`python3 scripts/governance/check_admin_page_query_panel.py` | `backend/crates/api/tests/print_template_postgres.rs`；`backend/crates/api/src/lib.rs`；`shared/openapi/openapi.json`；`packages/api-client/src/schema.ts`；`apps/web-admin/src/pages/print-template/H9PrintTemplatePage.tsx` | 部分覆盖 | 字段库发布、版本不可改写、OpenAPI/api-client 同步和 PC 字段库列表页已有验证；前端动作测试、浏览器打印预览、9002 截图和专项截图证据未覆盖。 | H9 后续切片补动作测试、浏览器打印预览截图、hiprint 设计器和真实截图证据。 |
