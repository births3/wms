...
| H8 PC 接口表探查 | US-H8-004 | 基础能力 / H8 集成中心 / H8 接口表探查（`h8-erp-interface-tables`） | `prototypes/e2e/web-admin-h8-interface-tables-real.spec.ts`；列表、`pending` 筛选、详情、权限拒绝 4 张真实截图；`docs/retros/h8-interface-table-software-path-evidence.json` | 已覆盖 | `wms_h8_real` + MSSQL DEMO 数据完成真实列表→筛选→详情与无写动作；仅有 connector.read 的新会话/刷新隐藏入口且 API 403；H2 列表/详情审计持久化并脱敏。 | 软件路径已关闭；客户正式接口库联调为可选增强。 |
...
| H8 ERP 接口表只读探查 | US-H8-004 | `GET /api/v1/h8/erp-interface-tables/connectors`、`GET .../rows` 与 `GET .../rows/{row_id}`（`connector_id`+`table_key`）；OpenAPI/api-client 已同步 | `backend/crates/api/src/h8_erp_interface_tables`：白名单、参数化 SELECT、`updated_at` 窗、表级过滤 400、准确 total、H2 查询审计失败即失败请求 | `h8_erp_connectors` 兼容迁移；专用权限/菜单及最新发布版本回填；Docker SELECT-only 账号；TCP 证据工具 | 真实 MSSQL `DEMO-ASN-001` 列表→筛选→详情、权限 403/菜单隐藏、H2 审计及 4 张截图均已落地 | 已覆盖 | 领域白名单/范围/脱敏、OpenAPI、权限/菜单、只读页面、DML 拒绝和 H2 持久化均已验证。 | 软件路径已关闭；正式接口库为可选增强。 |
...
| H8 ERP 接口表只读探查 | US-H8-004 | 领域/API 单测；审计失败传播单测；页面 self-check；真实 Playwright；`check_probe_readonly.sh` TCP/Docker 模式；OpenAPI、质量矩阵与 `gov-t1` | `DEMO-ASN-001`/`DEMO-PM-001` SELECT、三类 DML 拒绝、列表/筛选/详情、权限 403/入口隐藏、H2 列表/详情持久化与截图均有证据 | 已覆盖 | V0–V3 全部通过；V4 按故事决策不适用。 | 软件路径 `SOFTWARE_PATH_PASS`。 |
