...
| H8 ERP 消息交换与语义转换 | US-H8-002 | 复用 H3 业务 OpenAPI 与 H-INT 契约；不新增通用连接器 API | 已有 `scripts/h8_erp_interface_sync` 五类入站、七类出站和主备切片 | 现有接口表、业务 outbox 与 M-PM 调用需按消息类型逐条核验 | Worker 单测、本地联调和容器 ERP 证据 | 部分覆盖 | 尚缺逐消息类型 canonical 转换、统一错误、完整 PostgreSQL 业务链、档案补录闭环和客户正式 ERP 双向证据。 | 按 US-H8-002 消息目录逐条补契约/业务/幂等测试和正式 ERP S4 回执。 |
...
| ERP 报文、消息日志与故障重放 | US-H8-002 / US-H8-003 | ERP DTO 污染业务域、字段映射绕过、敏感报文泄漏、重复投递、消息丢失、越权重放和无界日志增长 | H8 canonical 边界、M-PM、原 Idempotency-Key、脱敏摘要、append-only 尝试、租约、月分区、受控保留和 H2 审计 | 当前仅有 Worker、本地/容器联调切片 | 部分覆盖 | 缺正式消息/尝试模型、逐消息契约、权限重放、保留保护、性能和客户 ERP 故障恢复证据。 | 按 US-H8-002/003 分别补实现和验收，完成前保持质量矩阵延期。 |
