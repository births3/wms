...
| M-CG PC 单据号规则管理 | US-CG-001 / US-CG-002 | `apps/web-admin` 系统管理 / 单据号规则菜单 | 待补真实管理页设计与截图 | 待补证据 | 规则管理页、规则预览、生成记录查询、启停和审计展示尚未实现。 | 后续切片补公开 API、api-client、PC 管理页、动作测试和真实截图；当前只完成后端内部发号服务。 |
...
| M-CG 单据号生成后端第一切片 | US-CG-001 / US-CG-002 | 不适用：当前仅内部服务，不新增公开 API | `backend/crates/api/src/document_numbering_repository.rs`；`backend/crates/api/src/document_numbering.rs` | `backend/migrations/202607020001_mcg_document_numbering.sql`；`document_number_rules`、`document_number_counters`、`document_number_allocations` | `backend/crates/api/tests/document_numbering_postgres.rs` | 部分覆盖 | 已覆盖 no-gap 发号、计数器、生成记录、幂等和调用方事务接口；规则管理 API、生成记录查询 API、配置审计审批和业务模块接入仍未实现。 | 后续切片补规则管理/查询 API、M2/M4 创建单据接入和 OpenAPI/api-client 同步。 |
...
| M-CG 单据号生成 | US-CG-001 / US-CG-002 | `set -a; source /home/test1/workspace/wms/.env; set +a; cargo test --manifest-path backend/Cargo.toml --test document_numbering_postgres` | `backend/crates/api/tests/document_numbering_postgres.rs` | 已覆盖 | 无 | 后续新增规则管理 API、生成记录查询 API 或业务模块接入时，继续补对应 API / repository / E2E 测试。 |
