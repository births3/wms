...
| H9 打印模板引擎 | US-H9-001 / US-H9-002 / US-H9-003 / US-H9-004 / US-H9-005 | 待补 `backend/crates/api/src/lib.rs` print-template OpenAPI；`shared/openapi/openapi.json`；`packages/api-client/src/schema.ts` | 待补 `backend/crates/api/src/print_template_handlers.rs` / service | 待补 `backend/crates/api/src/print_template.rs`、repository、migration；模板类型、字段库元数据、模板版本、打印记录表 | 待补 H9 Postgres 测试和 OpenAPI 同步证据 | 待补证据 | 当前仅完成 H9 故事和 ADR；后端 API、持久化、审计、幂等和 api-client 尚未实现。 | 按 ADR-0036 先补 OpenAPI、migration、service/repository 和 Postgres 测试，再接 PC 前端。 |
...
| H9 打印模板引擎 | US-H9-001 / US-H9-002 / US-H9-003 / US-H9-004 / US-H9-005 | 待补 `cargo test --manifest-path backend/Cargo.toml -p wms-api --test print_template_postgres`；`just openapi-check`；H9 前端动作测试 | 待补 H9 后端测试、OpenAPI/api-client、9002 截图和打印预览证据 | 待补证据 | 当前仅完成故事、ADR 和 RTM；无可执行 H9 后端/前端测试。 | H9 实现切片必须补后端 Postgres 测试、前端动作测试、浏览器打印预览截图和治理脚本接线。 |
...
| 打印模板版本和字段库治理 | US-H9-001 / US-H9-002 / US-H9-003 / US-H9-004 / US-H9-005 | GSP 纸质留存、历史打印可追溯、敏感字段脱敏、模板版本不可改写 | H9 字段库发布版本、模板版本、打印记录、货主覆盖和 H2 审计追踪 | 待补 `print_template_postgres.rs`、OpenAPI 契约和真实截图证据 | 待补证据 | 当前仅有 H9 设计文档；尚未落后端表、审计写入、权限和真实打印证据。 | H9 实现时必须补模板版本不可改写测试、敏感字段脱敏测试、跨货主权限测试和打印记录证据。 |
