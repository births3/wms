...
| H8 PC 接口表探查 | US-H8-004 | 规划为基础能力 / H8 集成中心 / H8 接口表探查（`h8-erp-interface-tables`） | 无 | 部分覆盖 | 十轮规格与入口规划已闭合；权限/菜单/API/页面/E2E 与 DEMO 种子截图均未实现。未实现前不在矩阵预登记页面或空 E2E。 | 落地权限/菜单/API/页面后，一次性登记页面、Playwright 和截图，并在 `docker-compose.h8-erp-if` 上补 SELECT-only、列表/详情/权限/审计证据。 |
...
| H8 ERP 接口表只读探查 | US-H8-004 | 计划 `GET /api/v1/h8/erp-interface-tables/rows` 与详情 `.../rows/{row_id}`（`connector_id`+`table_key`） | 待实现：独立探查凭据解析、表白名单、参数化 SELECT、`updated_at` 窗、表级过滤 400、准确 total | `h8_erp_connectors` 兼容新增 `interface_probe_db_username/interface_probe_db_password_alias/interface_probe_config_version`；权限迁移 + 菜单；不新增消息业务表 | 十轮规格已闭合；实现与 Docker 联调证据未落地 | 部分覆盖 | 已明确独立凭据和版本、数据库只读、无仓表全仓权限、脱敏限长摘要和 H3/ADR-0018 复用；兼容迁移、API、权限迁移和联调未落地。 | 先补连接字段迁移，再实现 API/页面，以 `docker-compose.h8-erp-if` + DEMO 种子和 DML 拒绝测试关闭软件路径。 |
...
| H8 ERP 接口表只读探查 | US-H8-004 | 计划：`docker compose -f deploy/docker-compose.h8-erp-if.yml` + DEMO 种子 API/E2E + SELECT-only DML 拒绝 + 权限/审计单测 + `gov-t1` | 十轮规格与 `deploy/h8-erp-if/init`；实现命令与截图未落地 | 部分覆盖 | 规格与软件路径验收命令已写；连接字段迁移、API/页面/Playwright/DEMO 与 DML 否定断言未执行。 | 实现后按故事 L1–L10、安全专项与软件路径证据包关闭；不强制客户正式接口库。 |
...
| ERP 接口表只读探查越权与泄漏 | US-H8-004 | 任意 SQL、跨货主读接口表、无仓表跨仓泄漏、凭据回显、原始报文泄漏、误写接口表、非法 `sync_status`/列过滤绕过 | 专用 `h8.erp_interface_table.read`、独立探查凭据、表白名单、参数化 SELECT、货主+仓权过滤、无仓表全仓权限、4096 字节脱敏摘要、SELECT-only 账号、H3 限流、H2 审计 | 十轮规格已有；实现证据未落地 | 部分覆盖 | 控制措施已写入故事与矩阵延期；实现与数据库 DML 拒绝、权限否定用例证据未落地，实现前保持 deferred。 | 落地后用 Docker MSSQL + SELECT-only DML 拒绝 + 权限否定/表级 400 用例关闭软件路径。 |
