...
| H8 PC ERP 连接配置 | US-H8-001 | `apps/web-admin` 基础能力 / H8 集成中心 / H8 ERP 连接（`h8-erp-connectors`） | `prototypes/e2e/web-admin-h8-real.spec.ts`（6 条，含编辑/route-resolve）；`docs/retros/h8-failover-runtime-evidence.json`（含半开回主）；`docs/retros/h8-container-erp-s4-evidence.json` | 部分覆盖 | AC1–16 软件主链已齐；矩阵仍可选延期客户正式 ERP 回执。 | 正式厂商联调归档后关闭 deferred。 |
