...
| H8 PC ERP 连接配置 | US-H8-001 | `apps/web-admin` 基础能力 / H8 集成中心 / H8 ERP 连接（`h8-erp-connectors`） | [user-stories-h8-erp-integration.md](domain/user-stories-h8-erp-integration.md)；`prototypes/e2e/web-admin-h8-real.spec.ts` | 部分覆盖 | 已有独立菜单、专用表、配置 API 与页面；真实 Playwright 全动作 E2E、启用成功截图与 S4 ERP 联调证据仍待补齐。 | 补新建/测试/启用/重叠拒绝/停用/只读真实 E2E 与截图，并完成 secrets 与主备运行时验收。 |
...
| H8 ERP 连接配置与安全接入 | US-H8-001 | 已补 OpenAPI / api-client：`/api/v1/config/erp-connectors*` | `h8_erp_connectors` handler/service；H2 审计摘要 | H8 专用表、路由唯一约束、配置版本；菜单迁移 `h8-erp-connectors` | 本地 Worker/Mock 与领域/API 测试；真实浏览器与 S4 外部证据待补 | 部分覆盖 | 尚缺真实浏览器全动作 E2E、正式 secret 解析、主备降级运行时与真实 ERP S4 证据。 | 补 E2E/截图、secrets、主备运行时与 dev/staging ERP 回执后关闭延期。 |
