...
| H9 PC 打印模板中心 | US-H9-001 / US-H9-002 / US-H9-003 / US-H9-004 / US-H9-005 | `apps/web-admin` 系统管理 / 打印模板菜单 | 待补真实管理页设计与截图；既有 `pc-m2-009` 原型仅作历史参考 | 待补证据 | H9 独立菜单、模板类型字典、字段库元数据、hiprint 设计器、预览打印和货主覆盖尚未实现。 | 先补 H9 OpenAPI / 后端 / 前端管理页，再补 9002 真实截图、动作测试和模板预览打印证据。 |
...
| H9 打印模板引擎 | US-H9-001 / US-H9-002 / US-H9-003 / US-H9-004 / US-H9-005 | 待补 `backend/crates/api/src/lib.rs` print-template OpenAPI；`shared/openapi/openapi.json`；`packages/api-client/src/schema.ts` | 待补 `backend/crates/api/src/print_template_handlers.rs` / service | `backend/crates/api/src/print_template.rs`；`backend/migrations/202607050002_h9_print_template.sql`；字段库、字段库版本、字段定义表 | `backend/crates/api/tests/print_template_postgres.rs` | 部分覆盖 | 已覆盖字段库发布、版本不可改写、幂等重放和审计写入；模板类型字典、模板主数据、hiprint JSON、打印记录、OpenAPI/api-client 和 PC 前端仍未实现。 | 按 ADR-0036 继续补模板类型字典接入、模板版本、打印记录、OpenAPI、api-client、PC 管理页和真实截图。 |
...
| H9 打印模板引擎 | US-H9-001 / US-H9-002 / US-H9-003 / US-H9-004 / US-H9-005 | `cargo test --manifest-path backend/Cargo.toml -p wms-api --test print_template_postgres`；待补 `just openapi-check`；H9 前端动作测试 | `backend/crates/api/tests/print_template_postgres.rs`；待补 OpenAPI/api-client、9002 截图和打印预览证据 | 部分覆盖 | 字段库发布和版本不可改写已有后端测试；OpenAPI、前端动作、浏览器打印预览、截图和治理脚本专项扫描未覆盖。 | H9 后续切片补 OpenAPI/api-client、PC 前端动作测试、浏览器打印预览截图和治理脚本接线。 |
...
| 打印模板版本和字段库治理 | US-H9-001 / US-H9-002 / US-H9-003 / US-H9-004 / US-H9-005 | GSP 纸质留存、历史打印可追溯、敏感字段脱敏、模板版本不可改写 | H9 字段库发布版本、模板版本、打印记录、货主覆盖和 H2 审计追踪 | `backend/crates/api/tests/print_template_postgres.rs`；`backend/migrations/202607050002_h9_print_template.sql`；待补 OpenAPI 契约和真实截图证据 | 部分覆盖 | 字段库发布版本不可改写、幂等和审计已有后端测试；模板主数据、打印记录、权限和真实打印证据仍未覆盖。 | H9 后续切片补模板版本不可改写测试、敏感字段脱敏测试、跨货主权限测试、打印记录证据和真实截图。 |
