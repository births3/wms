...
| H8 PC ERP 连接配置 | US-H8-001 | `apps/web-admin` 基础能力 / H8 集成中心 / H8 ERP 连接（`h8-erp-connectors`） | `prototypes/e2e/web-admin-h8-real.spec.ts`；`docs/retros/h8-local-integration-evidence.json`；`docs/retros/h8-container-erp-s4-evidence.json` | 部分覆盖 | 已有独立菜单、专用权限、配置 API/页面、Playwright 全动作/只读/Vault E2E、主备 failover 与容器 ERP 联调证据。客户指定厂商正式环境回执仍可替换 `ERP_CALLBACK_BASE` 补档。 | 客户厂商正式联调时替换回调地址并归档回执；可选关闭质量矩阵延期项。 |
...
| H8 ERP 连接配置与安全接入 | US-H8-001 | 已补 OpenAPI / api-client：`/api/v1/config/erp-connectors*` | `h8_erp_connectors` + H2 审计；`secrets.rs`；Worker failover | H8 专用表、权限迁移、菜单 `h8-erp-connectors` | Playwright E2E；`just h8-local-integration`；容器 ERP 证据 JSON | 部分覆盖 | 软件与容器厂商联调已齐；客户指定厂商正式环境回执可选。 | 正式厂商联调时替换 `ERP_CALLBACK_BASE` 归档回执。 |
...
| H8 ERP 连接配置与安全接入 | US-H8-001 | `just web-admin-h8-real-e2e`；`just h8-local-integration`；`just h8-container-erp-s4-evidence`；`python3 -m pytest scripts/h8_erp_interface_sync/test_h8_sync_worker.py`；`check_scope_gap_discovery --module H8` | 已有领域/API 单测、浏览器 E2E、主备与容器 ERP 联调证据 | 部分覆盖 | 容器厂商与联调证据已齐；客户正式厂商环境回执可选。 | 客户厂商联调后关闭矩阵延期。 |
...
| ERP 连接凭据、路由与审计 | US-H8-001 | 外部 ERP 凭据泄露、跨货主/跨仓误路由、双通道重复投递、越权启停和历史配置不可追溯 | secret alias（`secrets.rs`）、`h8.erp_connector.read/write`、路由唯一、Idempotency-Key、H2 审计、failover 非双写 | E2E 只读 403；Vault alias 测试；`h8-local-integration-evidence.json` | 部分覆盖 | 控制与联调证据已齐；客户正式厂商环境可选。 | 正式厂商联调时归档回执。 |
