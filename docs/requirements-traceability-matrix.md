...
| H8 ERP 防腐层 | [user-stories-h8-erp-integration.md](domain/user-stories-h8-erp-integration.md) | 1 | 前端体验 RTM / 后端实现 RTM / 测试证据 RTM / 合规风险 RTM |
...
| H8 PC ERP 连接配置 | US-H8-001 | `apps/web-admin` 基础能力 / M1 系统配置中心 / ERP 连接配置域（不新增菜单） | [user-stories-h8-erp-integration.md](domain/user-stories-h8-erp-integration.md) 字段、动作和截图证据规则 | 待补证据 | 当前 M1 配置中心只实现 Feature Flag；尚无 ERP 连接配置域、真实 API 接线、权限组合 E2E 和启用截图。 | 实现 H8 专用连接表与配置 API 后，在既有 M1 配置中心补新建、测试、启用、重叠拒绝、停用和只读权限真实 E2E，并归档启用成功截图。 |
...
| H8 ERP 连接配置与安全接入 | US-H8-001 | 待补配置列表/新建/修改/测试/启用/停用/删除 OpenAPI 与 api-client | 现有 `scripts/h8_erp_interface_sync` 只覆盖本地传输 Worker，不是连接配置 handler/service | 待补 H8 专用连接表、路由唯一约束、配置版本、权限和审计事务 | 现有接口表 Worker/Mock 仅作相邻通道切片证据，不满足 US-H8-001 | 待补证据 | 尚无连接配置 API、handler/service、domain/repository、PostgreSQL 迁移、H1 权限、H2 审计和真实 ERP S4 证据。 | 按故事状态表外向内 TDD：先冻结 OpenAPI 与失败测试，再实现 H8 专用表、service/repository、配置中心接线和真实 ERP dev/staging 验收。 |
...
| H8 ERP 连接配置与安全接入 | US-H8-001 | `python3 scripts/governance/check_user_story_structure.py --json`；`python3 scripts/governance/check_project_rtm.py --json`；`python3 scripts/governance/check_scope_gap_discovery.py --strict --module H8 --json`；实现后补 PostgreSQL、OpenAPI、真实浏览器 E2E 和真实 ERP dev/staging 命令 | 当前证据仅为故事、澄清记录、技术规格和既有本地 Worker；不得据此标记完成 | 待补证据 | 尚无 L1-L11 实现测试、真实数据 E2E、截图和 S4 ERP 回执证据；严格范围检查应保持失败。 | 按 US-H8-001 测试维度补实现证据，全部满足后再登记质量矩阵完成项。 |
...
| ERP 连接凭据、路由与审计 | US-H8-001 | 外部 ERP 凭据泄露、跨货主/跨仓误路由、双通道重复投递、越权启停和历史配置不可追溯 | secret alias、最小 scope、路由唯一约束、配置版本、Idempotency-Key、系统管理员写/仓库主管只读、H2 append-only 审计 | [user-stories-h8-erp-integration.md](domain/user-stories-h8-erp-integration.md)；[ADR-0013](adr/0013-config-secrets.md)；[ADR-0018](adr/0018-resilience-engineering.md)；实现证据待补 | 待补证据 | 控制规则已冻结，但专用表、权限、审计、脱敏测试、主备切换测试和真实 ERP 证据尚未实现。 | 实现时补 PostgreSQL 约束/权限/审计/幂等测试、配置中心真实 E2E 和 S4 外部回执证据。 |
