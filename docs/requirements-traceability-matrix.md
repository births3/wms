...
| H9 PC 打印模板中心 | US-H9-001 / US-H9-002 / US-H9-003 / US-H9-004 / US-H9-005 | `apps/web-admin` 基础能力 / H9 打印模板菜单 | 已补模板类型树和字段库列表页；既有 `pc-m2-009` 原型仅作历史参考 | 部分覆盖 | H9 独立菜单、模板类型字典、字段库读取 API、公共树状导航和字段库列表页已实现；模板主数据、hiprint 设计器、预览打印、9002 真实截图和货主覆盖仍未实现。 | 后续补模板版本、打印记录、hiprint 设计器、浏览器预览打印、动作测试和真实截图证据。 |
