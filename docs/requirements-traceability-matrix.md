...
| H8 PC ERP 连接配置 | US-H8-001 | `apps/web-admin` 基础能力 / H8 集成中心 / H8 ERP 连接（`h8-erp-connectors`） | `prototypes/e2e/web-admin-h8-real.spec.ts`（6 条，含编辑/route-resolve）；`docs/retros/h8-failover-runtime-evidence.json`（含半开回主）；`docs/retros/h8-container-erp-s4-evidence.json` | 部分覆盖 | AC1–16 软件主链已齐；客户正式 ERP dev/staging 回执仍缺，不能标记故事完成。 | 使用客户正式 `ERP_CALLBACK_BASE` 归档请求、回执与审计关联后关闭 deferred。 |
| H8 PC ERP 消息运维 | US-H8-003 | 规划为基础能力 / H8 集成中心 / H8 ERP 消息（`h8-erp-messages`） | 无 | 部分覆盖 | 仅完成故事与入口规划；消息查询、详情、统计、死信重放、只读权限、页面级查询配置和截图均未实现。 | 先补 PostgreSQL 消息/尝试模型和 API，再按列表型页面规则实现 QueryPanel + DataGrid，最后补真实 E2E 与截图。 |
| H8 PC 接口表探查 | US-H8-004 | 基础能力 / H8 集成中心 / H8 接口表探查（`h8-erp-interface-tables`） | `prototypes/e2e/web-admin-h8-interface-tables-real.spec.ts`；列表、`pending,failed` 多选筛选、详情、权限拒绝 4 张真实截图；`docs/retros/h8-interface-table-software-path-evidence.json` | 已覆盖 | `wms_h8_e2e` + MSSQL DEMO 数据完成真实列表→双状态并集→详情与无写动作；仅有 connector.read 的新会话/刷新隐藏入口且 API 403；H2 列表/详情审计持久化并脱敏。 | 软件路径已关闭；客户正式接口库联调为可选增强。 |
...
| H8 ERP 连接配置与安全接入 | US-H8-001 | 已补 OpenAPI / api-client：`/api/v1/config/erp-connectors*` | `h8_erp_connectors` + H2 审计；`secrets.rs`；Worker failover | H8 专用表、权限迁移、菜单 `h8-erp-connectors` | Playwright E2E；`just h8-local-integration`；容器 ERP 证据 JSON | 部分覆盖 | 软件与容器厂商联调已齐；仍缺客户正式 ERP dev/staging 请求、回执和审计关联。 | 使用客户正式 `ERP_CALLBACK_BASE` 归档 S4 证据后关闭延期。 |
| H8 ERP 消息交换与语义转换 | US-H8-002 | 复用 H3 业务 OpenAPI 与 H-INT 契约；不新增通用连接器 API | 已有 `scripts/h8_erp_interface_sync` 五类入站、七类出站和主备切片 | 五类接口表 DTO 已统一转换为 H8 canonical 命令；业务 outbox 与 M-PM 调用仍需按消息类型逐条核验 | Worker canonical/错误分类单测、本地联调和容器 ERP 证据 | 部分覆盖 | 尚缺真实 M-PM 规则查询、完整 PostgreSQL 业务链、档案补录闭环和客户正式 ERP 双向证据。 | 按 US-H8-002 消息目录逐条补映射/契约/业务/幂等测试和正式 ERP S4 回执。 |
| H8 ERP 消息日志与故障重放 | US-H8-003 | 计划新增消息列表、详情、统计和重放 API | 待实现 H8 消息 service/repository 与受控重放 | 待实现消息主记录、append-only 尝试记录、月分区、租约和索引 | 当前无完整 PostgreSQL、并发、页面 E2E 或 S4 故障恢复证据 | 部分覆盖 | 仅完成故事和 RTM；遗留分析记录的消息日志、监控、死信和重放功能尚未实现。 | 先落数据库与并发状态机，再补 API/页面/E2E，最后完成客户 ERP 故障恢复演练。 |
| H8 ERP 接口表只读探查 | US-H8-004 | `GET /api/v1/h8/erp-interface-tables/connectors`、`GET .../rows` 与 `GET .../rows/{row_id}`（`connector_id`+`table_key`）；OpenAPI/api-client 已同步 | `backend/crates/api/src/h8_erp_interface_tables`：白名单、参数化 SELECT、`updated_at` 窗、表级过滤 400、准确 total、H2 查询审计失败即失败请求 | `h8_erp_connectors` 当前字段基线；专用权限/菜单；Docker SELECT-only 账号；TCP 证据工具 | 真实 MSSQL `DEMO-ASN-001` 列表→筛选→详情、权限 403/菜单隐藏、H2 审计及 4 张截图均已落地 | 已覆盖 | 领域白名单/范围/脱敏、OpenAPI、权限/菜单、只读页面、DML 拒绝和 H2 持久化均已验证。 | 软件路径已关闭；正式接口库为可选增强。 |
...
| H8 ERP 连接配置与安全接入 | US-H8-001 | `just web-admin-h8-real-e2e`；`just h8-local-integration`；`just h8-container-erp-s4-evidence`；`python3 -m pytest scripts/h8_erp_interface_sync/test_h8_sync_worker.py`；`check_scope_gap_discovery --module H8` | 已有领域/API 单测、浏览器 E2E、主备与容器 ERP 联调证据 | 部分覆盖 | 容器厂商与联调证据已齐；客户正式厂商环境回执仍是故事完成条件。 | 客户厂商联调并归档请求、回执和审计关联后关闭矩阵延期。 |
| H8 ERP 消息交换与运行治理 | US-H8-002 / US-H8-003 | 现有 Worker 单测与本地/容器联调仅作已实现切片；新增故事尚无完整验收命令 | 现有五类入站、七类出站、主备切换证据；消息运维无完整证据 | 部分覆盖 | 缺逐消息类型 L2/L3/L4/L11、PostgreSQL、租约并发、页面 E2E、性能和客户 ERP 故障恢复证据。 | 实现时分别建立两条验收记录；localhost/Mock/容器不得替代 S4 回执。 |
| H8 ERP 接口表只读探查 | US-H8-004 | 领域/API 单测；审计失败传播单测；页面 self-check；真实 Playwright；`check_probe_readonly.sh` TCP/Docker 模式；OpenAPI、质量矩阵与 `gov-t1` | `DEMO-ASN-001` pending / `DEMO-ASN-002` failed / `DEMO-PM-001` pending SELECT、三类 DML 拒绝、双状态并集、详情、权限 403/入口隐藏、H2 列表/详情持久化与截图均有证据 | 已覆盖 | V0–V3 全部通过；V4 按故事决策不适用。 | 软件路径 `SOFTWARE_PATH_PASS`。 |
...
| ERP 连接凭据、路由与审计 | US-H8-001 / US-H8-002 | 外部 ERP 凭据泄露、跨货主/跨仓误路由、双通道重复投递、越权启停和历史配置不可追溯 | secret alias（`secrets.rs`）、`h8.erp_connector.read/write`、路由唯一、不可变版本快照、Idempotency-Key、H2 审计、failover 非双写 | E2E 只读 403；Vault alias 测试；PostgreSQL 历史版本、Worker 原版本读取及人工重放即时接管测试；出站连接货主/目录/仓库收窄与逐消息 `route-resolve` 测试；M2 入库完成及 M3 库存状态 outbox 仓库身份回读；`h8-local-integration-evidence.json` | 部分覆盖 | 人工重放桥、现有三类出站生产者的仓库路由身份和 PostgreSQL 接管已验证；四类业务生产者、真实 MSSQL 运行证据及客户正式 ERP S4 回执仍缺。 | 接通档案补录、对账差异、发货确认、库存快照生产者，补 Docker MSSQL 重放运行证据，并在正式厂商联调时归档请求、回执和审计关联。 |
| ERP 报文、消息日志与故障重放 | US-H8-002 / US-H8-003 | ERP DTO 污染业务域、字段映射绕过、敏感报文泄漏、重复投递、消息丢失、越权重放和无界日志增长 | H8 canonical 边界、M-PM、原 Idempotency-Key、脱敏摘要、append-only 尝试、租约、月分区、受控保留和 H2 审计 | 五类入站 canonical 边界、消息/尝试模型、Worker、本地/容器联调切片 | 部分覆盖 | 缺真实 M-PM、逐消息契约、生产分区/性能和客户 ERP 故障恢复证据。 | 按 US-H8-002/003 分别补实现和验收，完成前保持质量矩阵延期。 |
| ERP 接口表只读探查越权与泄漏 | US-H8-004 | 任意 SQL、跨货主读接口表、无仓表跨仓泄漏、凭据回显、原始报文泄漏、误写接口表、非法 `sync_status`/列过滤绕过、审计静默失败 | 专用 `h8.erp_interface_table.read`、独立探查凭据、表白名单、参数化 SELECT、货主+仓权过滤、无仓表全仓权限、4096 字节脱敏摘要、SELECT-only 账号、H3 限流、H2 查询审计失败即失败请求 | 领域/API 测试、真实 E2E、connector.read 反向权限、MSSQL SELECT/DML 否定、H2 持久化与敏感词检查均通过 | 已覆盖 | API 无写路径/原始 payload；MSSQL 实际拒绝 UPDATE/DELETE/INSERT；无权限菜单隐藏且 API 403；审计摘要无敏感内容。 | 软件路径已关闭，后续按正式环境常规回归。 |
