...
3. **方案 C（库区定环境大类 + 库位定作业形态 + 容器质量锁复用 M1 系统字典 + 通用 IoT 设备中台，推荐）**：库区定义温区与品类准入大区，库位定义作业形态（存储/箱拣/零拣）；容器支持合格、隔离、不合格三类质量锁，锁原因直接复用 M1 `system_dictionary_items` 系统字典；补货独立配置，硬件插件化接入。
...
1. **空间与属性解耦**：
   ...
   - **库位（Location）**：承载作业形态（`location_type: storage | case_pick | piece_pick`，与标题"库位三级作业形态"统一用语）、容器开关（`allows_container`）、安全特控（双锁柜、外用专位、易串味专位）、多货主隔离（`current_owner_id`，既有 `bound_owner_id` 改名）、三重锁定（`lock_in` 禁入 / `lock_out` 禁出 / `lock_all` 全锁，既有 `status='locked'` 迁移为 `lock_all`）、混品/混批策略（库位为基座、容器类型策略为收紧层，任一禁止即禁止）与 ABC 动销层级（`pick_zone_level`）。
2. **容器三类质量锁 (Container Quality Locks) 与系统字典复用**：
   ...
   - **适用边界**：容器质量锁为容器（整托）粒度——个别件异常先拆托或按批次行质检处理；**散货加锁 = 装箱容器化**（移入周转箱 → 容器加锁 → 上架质量区存储位，走容器锁全流程；未装箱或未上架则波次分配不到）；散货轻量管控走既有 M3 质检流程（批次 `status`/`qty_frozen`），6 维③ 散货位读批次 `status` 兜底。
   - **订单/波次带锁感知**：普通订单波次分配不到任何带锁库存（质量区排除 + 未上架无行 + 锁状态过滤三层兜底）；处置类单据（不合格品出库/隔离处置/报损销毁）允许波次到对应带锁库存并携带锁标记，仍受锁-区域约束；**加锁提交即生效，与 M-QL 审批解耦**（审批为处置结论路径，期间锁持续有效）；加锁时已下发拣选任务标记"质量锁中断"、订单行释放回波次重算单、被中断任务库存不扣减。
   - **加锁即物理隔离（锁-区域强约束）**：锁类别 ↔ 区域硬匹配（隔离锁仅隔离区、不合格锁仅不合格品区，两质量区互斥）；**加锁容器目标位 = 质量区 ∩ 存储位（`allows_container=true`），禁上箱拣/零拣散货位**（散货位自动解绑，将失去容器级管控）；加锁成功同事务生成隔离移库任务（`lock_move`），暂缓期禁一切作业、超时（默认 2 小时）告警；解锁回 `qualified` 同事务生成移回合格区任务，移出前不参与分配；**质量区库位不参与波次算单分配**；批次 `status` ↔ `zone.quality_color` 一致性校验 + 治理对账兜底（详见模型 §2.1 锁-区域映射）。
   - **操作权限**：新增 H1 权限点 `m1.quality-lock.manage`（加锁/换原因/解锁），无权限用户禁止操作；操作日志进审计事件表。
   - **锁事件表为纯审计表（只 INSERT，禁止 UPDATE/DELETE）**：加锁/换原因/解锁均插入新事件行，当前锁冗余在容器主档（`lpn_containers.current_lock_category`）同事务维护；**并发防护：加锁/解锁事务对容器行 `FOR UPDATE` 行锁串行 + 接口 `Idempotency-Key` 幂等**；加锁与解锁必须双人见证（`witness_id` 与 `operated_by` 必须为不同用户），缺见证人或见证人重复拒绝提交。
...
4. **上架 6 维正交校验流水线（叠加于既有上架事务，非替换）**：
   - PDA 上架或系统推荐库位时，在既有上架逻辑（LPN 绑定、散件互斥、行锁加量，见 ADR-0047）之前执行前置校验：① 品类大区隔离校验 ➜ ② 温区环境匹配校验（目标库位温区范围须被商品存储温区范围包含，见模型 §1.1）➜ ③ 容器质量锁状态校验（含目标库区 `zone.quality_color` 匹配，散货位读批次 `status`）➜ ④ 特药双人核验（叠加既有 M-VR 双人策略动态查询，见 US-M2-005 验收 8）➜ ⑤ 包装粒度作业形态防呆（存储位仅接受容器粒度上架，散件先组托或转拣选位；拣选位仅接受散货，容器上架自动解绑）➜ ⑥ 外用易串味互斥与容量防呆（商品侧依据 `products.is_external_use` / `is_fragrant` 字段，与库区 `is_external_use_zone` / `is_fragrant_zone` 互斥校验）；③ 通过后进入既有 LPN/互斥/加量事务。
   - **6 维为固定业务骨架**：属 GSP 合规强制校验，硬编码实现，不允许配置关闭或弱化；仅 ④ 的双人策略经 M-VR 规则引擎动态扩展（US-M-VR-002）。
   - **拒绝留痕**：任一维度校验不通过强阻断时，写入阻断日志表 `putaway_validation_rejection_logs`（id/owner_id/操作人/容器或商品/目标库位/拦截维度/时间/原因），供质量追溯与治理对账；被拦截记录不产生库存变动。
...
7. **独立补货策略系统**：
   ...
   - **Min-Max 数值只存补货策略表**（按货主 + 库位组/品类/**单商品**配置 `min_safety_threshold` / `max_replenish_target`，scope 优先级 product > category > location_group；**补货动线可配置** `source_type → target_type`：storage→case_pick / storage→piece_pick / case_pick→piece_pick），库位字段不存数值，仅挂 `replenish_strategy_id` 引用；
...
- 基础档案库区、库位与容器表扩展作业形态、容器质量锁、安全特控、容量参数、多货主与 AGV/PTL 字段。
...
- **错误码规划（实施时在 `docs/error-codes.md` 登记并过 check_error_codes）**：`M1_QUALITY_LOCK_*`（容器状态非 in_use/见证人缺失或重复/M-QL 未办结或驳回状态异常/目标区域 `quality_color` 不匹配/库位形态不符等）、`M3_REPLENISH_*`（任务领取冲突/超量下架/取消前置不满足/来源可用量不足/确认幂等冲突等）、`M2_PUTAWAY_*`（6 维校验 ①-⑥ 逐维阻断码）。
- **Phase 1 基线同步清单**（v1 前直接改基线，不做兼容过渡）：① `warehouse_zones.temperature_zone` 与 `products.storage_condition` 两处温区 CHECK 值域同步迁移至五温区新编码；`products` 新增 `is_external_use` / `is_fragrant` 两字段（6 维⑥互斥校验的商品侧依据）；`warehouse_zones` 新增 `allowed_categories` / `is_external_use_zone` / `is_fragrant_zone` / `is_special_drug_zone` 四字段；② `warehouse_locations.status` 值域收敛为 `available/occupied/disabled`，存量 `locked` 迁移为 `lock_status='lock_all'`，`bound_owner_id` 改名 `current_owner_id`；`warehouse_locations` 新增 `allows_container` / `mix_product_policy` / `mix_batch_policy` / `lock_status` / `pick_zone_level` 五字段；**`location_type` CHECK 值域扩展加 `staging`（集货位，见 wave-model §5.2）**；③ `inventory_batches` 按 §4 迁移清单改名/新增字段（`product_code→product_id`、`quality_status→status`、`qty_locked→qty_frozen`、新增 `zone_id/container_lpn` 与在途双字段），并同步改 M3 召回/质检作业引用；④ 新增 `container_quality_lock_events` 纯审计表（含 `quality_liaison_id` 挂接列）与 `lpn_containers` 当前锁冗余字段；⑤ 新增补货策略表（含库位组两张表）与 `warehouse_locations.replenish_strategy_id` 引用、**库位路径顺序字段 `pick_sequence_no`/`putaway_sequence_no`（批量向导生成，拣选任务与上架推荐排序依据）**、`location_device_bindings` 绑定表与 AGV 预留字段（`is_agv_managed`/`agv_pod_code`/`agv_unreachable_at`）；⑥ 设备中台四表（`iot_devices`/`wcs_tasks`/`iot_event_logs` 于 Phase 2 建齐结构，无生成入口，见模型 §6.6）。
