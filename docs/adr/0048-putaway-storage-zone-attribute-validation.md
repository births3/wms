...
2. **容器三类质量锁 (Container Quality Locks) 与系统字典复用**：
   ...
   - **加锁即物理隔离（锁-区域强约束）**：锁类别 ↔ 区域硬匹配（隔离锁仅隔离区、不合格锁仅不合格品区，两质量区互斥）；加锁成功同事务生成隔离移库任务（`lock_move`），暂缓期禁一切作业、超时（默认 2 小时）告警；解锁回 `qualified` 同事务生成移回合格区任务，移出前不参与分配；**质量区库位不参与波次算单分配**；批次 `status` ↔ `zone.quality_color` 一致性校验 + 治理对账兜底（详见模型 §2.1 锁-区域映射）。
