...
| [ADR-0025](0025-audit-storage-model.md) | 审计存储模型（PG append-only）| Proposed | SPIKE-002 验证 accept；audit_event 表 + 月分区 (PARTITION BY RANGE) + trigger + wms_app 角色权限 + JSONB diff (jsonb_path_ops 索引) + 每日封档 hash chain (方案 C) + 5 年保留与归档 |
