...
1. 误把开发完成等同于可预发布。
2. 在没有稳定 dev/staging 或外部系统时，用 localhost / stub / mock / fake / example 证据替代真实证据。
...
- 不用本地 PostgreSQL、localhost、stub、mock、fake、example 替代真实 dev/staging 或外部系统证据。
...
4. 所有 evidence 引用必须指向真实 dev/staging、真实硬件或真实外部系统日志 / 工单 / 对账记录，禁止 local/mock/fake/stub/example。
