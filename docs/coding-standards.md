...
任何代码改动都先查现有实现，再决定是否新增。查找范围包括业务故事、分层文档、现有模块、类型、接口客户端、工具函数、测试夹具和治理脚本。

| 层级 | 先复用什么 | 没有时怎么补 |
|------|------------|--------------|
| 后端 domain | 实体、值对象、不变量、错误类型 | 补到对应限界上下文的 `domain`，保持无 IO |
| 后端 service | 命令、查询、状态流转、审计 helper | 补聚焦用例服务，不把业务规则塞进 handler |
| 后端 repository | trait、查询 helper、SQL 映射 | 补到 repository 或持久化辅助模块，避免复制 SQL |
| 前端 page / feature | 页面局部组件、hook、view model、api-client 调用 | 补页面私有组件或 feature hook，页面只编排 |
| 前端 UI | `@wms/ui` 基础控件、业务复合组件、设计 token | 通用 UI 进 `@wms/ui`，业务复合按组件规范注册 |
| 测试 | 既有 fixture、builder、mock server 数据 | 补可复用 fixture，避免每个测试手写一套数据 |

新增标准单元必须满足：

1. 当前任务真实使用，不为“将来可能”预留空抽象。
2. 名称表达业务语义或技术职责，不能绑定一次性页面变量名。
3. 放置位置符合 `docs/layered-design.md`，不能为复用破坏依赖方向。
4. 最小公开接口；后续出现 3 个以上相似用例时再提升为更通用层。
5. 最终汇报说明复用了什么、新增了什么、为什么放在该层、跑了哪些验证。

禁止事项：

- 禁止复制相似 handler、service、SQL、表单、页面、hook 后只改字段名。
- 禁止为少量本地逻辑新增依赖。
- 禁止绕过 `@wms/api-client`、OpenAPI 类型、审计 helper 或既有测试夹具。
...
- API 版本放 URL 路径（`/api/v1/...`）；首个正式版本前 breaking change 直接同步当前契约，正式版本后才走新主版本 `/api/v2/` 和退役窗口（与 H3-001 §4、ADR-0016 一致）
