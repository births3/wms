...
- W2.G runtime evidence：待稳定 dev/staging 后按 `docs/runbooks/wave-2-runtime-evidence.md` 采集，写入 `docs/retros/wave-2-runtime-evidence.json`，并通过 `just wave-2-runtime-evidence-validate`
- W1.B / W1.D runtime evidence：仍按 Wave 1 runbook 在预发布前补齐
...
- 配置中心版 Feature Flag 的真实灰度链路未在 dev/staging 验证，预发布前必须补证据，不能使用 local / mock / fake / example 替代。
