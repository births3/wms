...
|---|---|---|---|---|---|
| AR-01 | 确认鉴权存储故障时的高风险写操作策略 | 阻断决策 | P2 | 无 | 待确认 |
| AR-02 | 分离传输无关的操作人/货主上下文 | 高 | P1+P2 | AR-01 可并行决策 | 已完成（`b3bee45`） |
| AR-03 | 修复 M4 截断数据上的客户端查询 | 阻断缺陷 | P0+P1 | 无 | 代码与真实 M4 E2E 完成（查询命中默认窗口外订单） |
| AR-04 | 建立 M4 发运应用服务边界 | 高 | P1+P2 | AR-02；保持既有幂等语义或等待 AR-06 明确 | 已完成（`2bbb4bd`） |
| AR-05 | 让一条 M4 关键链路真正复用 H6 状态规则 | 高 | P2 | AR-04、状态确认 | 待确认 |
| AR-06 | 收敛 L11 幂等语义和重复实现 | 高 | P1+P2 | 保持 ADR-0034；确认存储权威 | 首切片完成；管理菜单、库存状态配置、系统字典、任务引擎、对账、质量联系单、报损报溢与双人策略模块已迁移；23 项遗留访问待迁移 |
| AR-07 | 恢复生产前端 OpenAPI 客户端唯一入口 | 高 | P0+P1 | 无 | 代码与 M3 真实 E2E 已完成（`a80a103`）；客户平台真实 E2E 待独立数据库 |
| AR-08A | 收敛菜单、视图和故障回退边界 | 中高 | P1+P2 | 菜单故障策略确认 | 待确认 |
| AR-08B | 消除 M4 模型与组件循环依赖 | 中高 | P1 | AR-03 可独立先做 | 代码、自检、构建与 M4 真实 E2E 完成（`2149b85`） |
| AR-09 | 解除 Render Worker 对核心 API 启动的硬阻塞 | 高 | P1+P2 | 部署行为确认 | 待确认 |
| AR-10 | 修正 H5 与同类虚假真实 E2E/截图证据 | 阻断证据 | P1 | 可用测试 PostgreSQL | 规则、真实 spec 与矩阵纠偏完成（`dd699b8`）；真实 H5 E2E 待测试数据库 |
| AR-11 | 阻止新增数字 `include!` 并改造一个代表聚合 | 中 | P1+P2 | AR-04/AR-05 后独立执行 | 待确认 |
| AR-12 | 复核首版前 migration 基线与表所有权 | 中 | P2 | 数据库方案确认 | 生成链首切片完成；基线决策与空库证据待补 |
| KG-01A | 补齐确定性可追溯图谱关系 | 中高 | P1+P2 | 实现归属确认；ALTER 关系等待 AR-12 | 待确认 |
| KG-01B | 修正业务域拓扑 | 中 | P1 | 既有计划 G2 完成 | 阻塞 |
| KG-02 | 确认并修正图谱新鲜度语义 | 中高 | P2 | 新鲜度方案确认 | 待确认 |
| REDIS-01 | 评估 Redis 是否仍是必要基础设施 | 中高 | P2 | 无；不得先改运行时 | 方向已确认：不新增 Redis；鉴权替代待独立任务（`fdfbb9f`） |
...
| AR-08A | 已完成 | 采用 fail-closed 菜单回退（用户确认 A）；集合检查与 H1 真实 E2E 通过（`79d2688`、`3be857f`）。 |
...
- [ ] 项目主人和安全/业务负责人确认候选方案及适用 operation 清单。
- [ ] 若故障语义改变，新增 ADR 并明确读取、普通写入、高风险写入的故障行为；新 ADR
      Accepted 后局部取代 ADR-0024 §2.3.1，ADR-0024 其余鉴权决策继续有效。若确认维持
      现状，则记录有批准人的复核结论，不制造无变化 ADR。
- [ ] ADR 索引、H1 用户故事、错误码和运行手册按最终决策同步。
- [ ] 明确 PostgreSQL 回查失败、Redis 抖动、恢复后的行为和告警级别。
- [ ] 测试分别覆盖 Redis 正常、撤销命中、Redis 不可用和恢复四种路径。
- [ ] 若继续全量 fail-open，记录最长窗口、Redis HA 现状和正式发布阻断条件，不把风险写成已消除。
- [ ] 新增策略矩阵测试，覆盖 operation 风险级别 × Redis 正常/撤销命中/不可用/恢复；
...
- [ ] 新增或修订用户故事，明确本切片由 H6 提供共享纯状态规则、M4 service 负责事务执行；
      若要改为 H6 唯一执行器，另建 P2 故事/ADR 和独立验收。
- [ ] 业务负责人确认 M4 状态、事件、初始态、终态和短拣分支。
- [ ] H6 定义与 M4 实际持久化状态一致，不再存在注册表未知状态。
- [ ] M4 service 在进程内调用同一纯转换函数，不通过 HTTP 回调自身。
- [ ] 非法跳转在写数据库前失败；合法跳转的业务状态、H2 审计和领域事件同事务完成。
- [ ] 测试覆盖完整正常链、短拣链、非法跳转、重复请求和事务回滚。
- [ ] RTM 和质量矩阵不再把“定义可读”写成“业务执行已接入”。
...
- [ ] 项目主人确认菜单故障回退方案，并同步菜单设计文档。
- [ ] 菜单请求失败、返回空树、当前 view 被移除和低/无权限用户都有明确且不越权的页面状态。
- [ ] `AdminView`、必要的本地 view ID、开发菜单种子和 renderer 处理集合由表驱动检查验证一致；
      该检查不复制服务端权限表。
- [ ] 服务端成功返回菜单时，前端不使用本地结构覆盖其顺序、启用状态或权限结果。
- [ ] 没有真实聚合接口前，Dashboard 显示未接入/不可用状态，不展示硬编码业务数量；
