...
- 验收基线：2026-07-27 当前工作区（组套版本、四层解析、就绪/失败策略、外部权威文件引用与实例快照）
- 验收层级：`S3`
- 质量矩阵状态：`stories`
...
| AC-5 发票完整=全部源订单被有效发票覆盖；药检完整=全部必需商品+批号被有效报告覆盖 | V1 / V2 / V3 | 服务端可计算就绪检查：按组订单发票号与 `outbound_order_lines` 商品+批号，对照 `h9_ingested_document_files` 有效文件逐项判定并输出缺口原因 | `required_not_ready_applies_frozen_policy_and_completeness`（缺文件→缺口原因；播种文件→就绪）；E2E `发票（外部文件，必需）：就绪，绑定 1 个权威文件` | PASS | H-FILE 摄取未实现，文件引用经 `h9_ingested_document_files` 占位登记表以 SQL 播种验证（见缺口清单） |
| AC-6 引用 H-FILE 文件 ID/稳定来源；禁止临时外部 URL 当长期事实源 | V0 / V1 | 配置层 `external_file_ref` 受控校验：必须 `h-file:` 前缀、拒绝 http(s) URL；实例层保存权威文件 ID+版本+内容哈希，不落 URL | `validate_print_suite`（InvalidExternalFileRef）；`binding_and_category_validation_is_mode_specific` 临时 URL 拒绝；`PrintSuiteFileBinding{file_id,file_ref,file_version,content_hash}` | PASS | 按需短期访问 URL 生成属 H-FILE/US-H9-009 范围，本故事不实现 |
...
| AC-8 实例保存组套版本、规则版本、源单据快照；rendered 项存模板版本、external_file 项存权威文件 ID 与版本 | V1 / V2 / V3 | 截单事务内按解析结果创建实例：`suite_snapshot`/`aggregation_rule_version_*`/`source_documents` 固化，实例项存 `template_version_id` 或 `file_bindings`；无发布组套时按当前基线只生成归集组 | `instance_snapshot_and_policies_are_frozen`（快照 UPDATE 被触发器拒绝、停用组套后快照不变）；`suite_resolution_...`（无组套→无实例）；E2E 实例行 `V1 待打印 1.delivery_note✓ → 2.invoice✓` | PASS | - |
...
- V1 / V2：`cargo test -p wms-api --test h9_print_suite_postgres` → **6 passed; 0 failed**；
...
1. **H-FILE 摄取未实现**：`external_file` 的权威文件经新增占位登记表
   `h9_ingested_document_files`（稳定 file_ref + 版本 + 内容哈希 + 发票号/商品批号业务键）承载；
   Postgres 测试与 E2E 种子以直接 SQL 播种文件引用验证 AC5 完整性判定。H-FILE 真正的
   上传/摄取、按需短期 URL 与文件权限（L8 文件读取审计）随 H-FILE / US-H9-009 落地后，
   本表应由 H-FILE 权威登记替代或映射，判定逻辑无需变更。
2. **就绪策略的运行时执行**：`pause_agent_queue` 在本故事固化为实例 `hold_scope='agent_queue'`
   标记；真正暂停 Agent 队列/放行后续实例的调度行为属 US-H9-010/012 队列与 Agent 运行时。
3. **实例后续状态机**：实例当前落 `waiting_documents/queued/cancelled` 子集；H6 全量
   `h9_print_suite` 状态机注册与转换事件在 US-H9-009/010 实现渲染与任务队列时接入。
4. **组套实例创建入口**：按设计仅由截单事务自动创建；无发布组套时按当前基线只生成归集组；
   未提供独立的"补建实例"端点，等待 US-H9-009 明确重试语义后再评估。
...
- 范围声明：本记录不代表 H9 套打中心全部完成；US-H9-009～010、012～015 继续逐故事验收，
