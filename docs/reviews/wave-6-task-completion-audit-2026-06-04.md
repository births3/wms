...
| 项 | 结论 | 证据 |
|---|---|---|
| Wave 1 开发完成 | 已完成 | `just wave-1-complete-check` 通过；预发布 runtime evidence 单独后移 |
| Wave 2 开发完成 | 已完成 | `just wave-2-complete-check` 通过；W2.G runtime evidence 单独后移 |
...
| Wave 6 预发布收口 | 未完成 | `just wave-6-evidence-check` 失败：7 个真实 evidence gate 未关闭；`just wave-6-complete-check` 失败：另缺 Wave 6 retro |
...
| 区域 | 完成情况 | 核对结论 |
|---|---:|---|
| 当前 Wave 6 | 8/22 | 8 个工具链 / 范围项完成；14 个真实 evidence / 外部依赖 / retro 项未完成 |
...
| 类别 | 项目 |
|---|---|
| Wave 6 预发布 evidence / 外部依赖 | 当前 Wave 6 中 14 个未勾选项，见下方逐项表；另有 Wave 3 PDA 生产端后续项由 W6.D 承接 |
...
| TODO 行 | 任务 | 当前状态 | 核对依据 |
|---|---|---|---|
| 15 | 稳定 dev/staging 环境仍未就绪 | 未完成 | Wave 1 / 2 / 6 runtime validators 均缺真实 dev/staging evidence；`just wave-6-evidence-check` 失败 |
...
| 26 | W6.A Wave 1 H2 runtime evidence | 未完成 | `just wave-1-runtime-evidence-validate` 缺 `docs/retros/wave-1-h2-runtime-evidence.json` |
| 27 | W6.B Wave 1 W1.D 自动回滚 evidence | 未完成 | `just wave-1-runtime-evidence-validate` 缺 `docs/retros/wave-1-runtime-evidence.json` |
| 28 | W6.C tooling | 已完成 | `record_wave2_runtime_evidence.py` 与 `just wave-2-runtime-evidence-record` 已登记；`W6-tooling` 通过 |
| 29 | W6.C Wave 2 配置中心 Feature Flag evidence | 未完成 | `just wave-2-runtime-evidence-validate` 缺 `docs/retros/wave-2-runtime-evidence.json` |
...
Wave 6 当前缺 7 个 evidence gate，对应 8 个真实 evidence JSON 文件：

| Gate | 缺失文件 |
|---|---|
| W6.A | `docs/retros/wave-1-h2-runtime-evidence.json` |
| W6.B | `docs/retros/wave-1-runtime-evidence.json` |
| W6.C | `docs/retros/wave-2-runtime-evidence.json` |
...
此外，Wave 6 最终完成还缺 `docs/retros/wave-6-retro.md`。该 retro 只能在上述 evidence 全部通过后编写。
