...
| US-H8-001 ERP 连接配置与安全接入 | H8 | 已补 H8 专用表、领域规则、配置 API、OpenAPI/api-client、H2 审计、专用权限、集成中心菜单、secrets 解析、真实浏览器 E2E、主备降级 failover 运行时（REST 失败→接口表）与本地 ERP mock 证据 docs/retros/h8-failover-runtime-evidence.json。尚无外部真实 ERP 厂商 dev/staging 回执作为正式 S4，不得标记完成。 |
