...
| US-M1-004b 容器质量锁管理 | M1 | S3 | L1、L2、L3、L4、L5、L7、L8、L9、L10、L11 | 方案与验收标准已定稿（ADR-0048、storage-location-model §2，含锁-区域映射/装箱加锁/带锁感知/在途拣选收口）；未实施。待 Phase 1 实施：事件表/主档冗余字段迁移、加锁/换原因/解锁接口、权限点 m1.quality-lock.manage、双人见证、M-QL 挂接与解锁前置校验（closed/rejected）、批次状态联动、锁-区域移库任务、管理端容器页操作与 PDA 扫码。 |
