...
| US-H8-001 ERP 连接配置与安全接入 | H8 | 已补 H8 专用表、领域规则、配置 API、OpenAPI/api-client、H2 审计摘要、M1 配置中心 ERP 连接页与 Worker 非双写对齐；本地传输切片仍在。尚无真实浏览器 E2E/截图、正式 secret 解析、主备降级运行时与真实 ERP dev/staging S4 证据，不得标记完成。 |
