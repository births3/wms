...
| US-H8-002 ERP 消息交换与 WMS 语义转换 | H8 | 已补受控消息目录/信封/错误分类/配置版本绑定 domain 与单测，Worker 本地入站五类+出站七类仍在；尚缺逐消息类型 M-PM 规整、完整业务 API 幂等链与客户正式 ERP 双向 S4。 |
| US-H8-003 ERP 消息日志、监控与故障重放 | H8 | 已补消息主记录/尝试表迁移、状态机与租约 domain、list/detail/stats/replay API、h8-erp-messages 菜单页、mock 与 Playwright 截图；尚缺月分区挂载、P95 指标与客户 ERP 死信重放 S4。 |
