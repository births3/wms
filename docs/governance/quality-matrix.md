...
| US-H8-001 ERP 连接配置与安全接入 | H8 | 已补 H8 专用表、领域规则、配置 API、OpenAPI/api-client、H2 审计摘要、基础能力/H8 集成中心独立菜单（h8-erp-connectors）与 Worker 非双写对齐；本地传输切片仍在。尚无全动作真实浏览器 E2E、正式 secret 解析、主备降级运行时与真实 ERP dev/staging S4 证据，不得标记完成。 |
