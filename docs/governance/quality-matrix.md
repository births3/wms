...
| US-H8-001 ERP 连接配置与安全接入 | H8 | 已补 H8 专用表、领域规则、配置 API、OpenAPI/api-client、H2 审计、专用权限 h8.erp_connector.read/write、集成中心独立菜单与 Worker 非双写对齐；本地传输切片与 alias 探测（WMS_H8_SECRET_ALIASES）已有。尚无全动作真实浏览器 E2E、正式 Vault secret 解析、主备降级运行时与真实 ERP dev/staging S4 证据，不得标记完成。 |
