...
| US-H8-002 ERP 消息交换与 WMS 语义转换 | H8 | ASN V2、本地主备、入站唯一路由、首次配置绑定、Worker 重试主动读取原绑定、既有幂等消息绑定切换拒绝、schema_version 严格拒绝、预检失败 receive/final_failure H2 审计、审计失败阻断业务、五类入站业务 API 的 503/422 错误分类、共享错误出口凭据脱敏、未知 storage_condition 写入前拒绝，以及 route-resolve 对已注入单仓范围的默认绑定和越权 403 已验证；仍缺 canonical/M-PM 实际映射、历史配置快照读取与人工重放认领链、JWT 用户多仓范围与 Worker 身份接线、出站唯一路由、逐消息 L2-L4/L11 与自研 ERP 双向 S4。 |
