...
| US-H8-001 ERP 连接配置与安全接入 | H8 | 已补 H8 专用表、领域规则、配置 API、OpenAPI/api-client、H2 审计、专用权限、集成中心菜单、真实浏览器新建/测试/启用/停用/重叠拒绝 E2E 与截图登记；本地传输切片与 alias 探测已有。尚无只读权限组合 E2E、正式 Vault secret 解析、主备降级运行时与真实 ERP dev/staging S4 证据，不得标记完成。 |
