...
| US-H8-002 ERP 消息交换与 WMS 语义转换 | H8 | ASN V2、本地主备、入站唯一路由、首次配置绑定、schema_version 严格拒绝和 Worker 错误重试分类已验证；仍缺 canonical/M-PM、重试复用原绑定、主体仓库范围、出站唯一路由、预检失败审计、错误脱敏、逐消息 L2-L4/L11 与自研 ERP 双向 S4。 |
| US-H8-003 ERP 消息日志、监控与故障重放 | H8 | 已补消息/尝试存储、awaiting_receipt 两级回执状态、Worker claim 租约、stats P95、保留策略 purge、菜单页 E2E 与分区准备迁移；新增的 Worker 心跳、按连接+方向暂停恢复、加密短期报文保留尚未实现，并缺生产月分区和自研 ERP 故障恢复 S4。 |
