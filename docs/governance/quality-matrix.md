...
| US-H9-013 多 Agent 分配与故障降级 | H9 | S4 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | 地址/客户/线路优先级、同站点主备及 ADR-0041 在线交接、租约策略、assignment_epoch/lease_token fencing 已确认；分配规则、租约、故障转移、页面和多 Agent S4 证据未实现。 |
...
| US-M2-002 PDA/PC Web 收货 | M2 | S4 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | PC 数量闭环、GSP 现场字段、整单拒收主链、一 ASN 一 receipt、收货不写库存已有口径（澄清 #103-#113）；追溯码绑码、LPN 绑定、PDA 离线不是 002 完成条件。仍缺：超温继续强制 H-FILE 稳定性报告附件、收货节点 M-VR 双人策略（US-VR-006）。 |
...
| US-M2-005 PDA/PC Web 智能上架 | M2 | S4 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | PC 推荐、合法性校验、部分上架、LPN 落库、上架 ERP outbox 本地闭环与真实 E2E 切片已有；全自动模式、上架节点 M-VR 双人/审批、外部 ERP 真投递、PDA 离线冲突及真机证据未闭环。 |
| US-M2-006 收货异常处理 | M2 | S4 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | 整单拒收、数量闭环、短少强制关闭、销售退货批号级部分拒收及条件式 M-QL/H4 记录已有实现与测试；但联络类型未配置时会静默跳过，且 M2 repository 直接写跨上下文表，结构化异常分类、稳定性报告、真实企业微信通知和按供应商/商品/类型/批号统计仍未闭环。 |
...
| US-M1-001 商品档案管理 | M1 | S2 | L1、L2、L3、L4、L5、L7、L8、L9、L10、L11 | 已完成结构化商品纵向切片：spec 在 M1/H8 Rust、Python canonical、MSSQL、OpenAPI/api-client、管理端 query 与 dev mock 全链为必填非空字符串，缺规格 REST 以 422 且 H8/M1 零写入；货主内 UDI、物理属性、多级包装、显式 null、事务审计和 L11 幂等均有真实 PostgreSQL 证据。H8 REST 与接口表 Worker 统一复用共享防腐层；M1 商品 POST/PATCH/DELETE/batch-sync 对 JWT 与 API Key 主体统一返回 403，GET 保持可用；ERP 必需映射未命中不得写接口表，WMS 二次校验以 422/H8 死信/M1 零写入拒绝，Rust Worker 将违规接口行置死信。管理端与开发 Mock 均为 ERP 权威投影只读页，浏览器 E2E 验证无新增/编辑/导入动作并展示完整商品契约。故事仍保持延期：ERP 发布器的真实发布前校验证据、变更请求及批量预览逐字段 M-PM 结果、特殊药品分类变更双人审批、PDA 离线扫码查询和自研 ERP dev/staging S4 尚未闭环。 |
| US-M1-002 供应商资质档案 | M1 | S2 | L1、L2、L3、L4、L5、L7、L8、L9、L10、L11 | 已完成供应商批量 API 同步后端切片：整批 PostgreSQL 事务、重复幂等回放/冲突、货主隔离、API Key 中间件鉴权、请求审计和原子回滚均有真实 PostgreSQL 回归证据；供应商创建、批量同步和更新入口现统一执行 GB 32100 字符集、长度、校验码与大写归一化校验，前后端共用同一算法语义并覆盖 422 契约；管理端批量导入已直接调用原子批量接口，并由一次性 PostgreSQL 浏览器 E2E 验证两条请求、页面回显和本地门户截图；截图按门禁不入库，长期证据仍须由 PR 附件或 CI artifact 归档；故事仍不能整体关闭，资质证照有效期/上传、经营范围标签、到期预警、过期供应商收货拦截、印章及样张备案、完整供应商更新/查询、PDA、Excel 原生导入和完整外部系统联调尚未闭环。 |
| US-M1-003 客户/门店档案 | M1 | S2 | L1、L2、L3、L4、L5、L7、L8、L9、L10、L11 | 已补客户多地址及客户/门店联系方式、经营范围、资质证照、连锁归属的 PostgreSQL 读写、默认地址互斥、货主隔离、幂等和审计，并新增客户批量同步 API：整批事务、重复幂等回放/冲突、货主隔离、API Key 中间件鉴权、请求审计和原子回滚有真实 PostgreSQL 回归证据；管理端批量导入已直接调用原子批量接口，并由一次性 PostgreSQL 浏览器 E2E 验证两条请求、页面回显和本地门户截图；截图按门禁不入库，长期证据仍须由 PR 附件或 CI artifact 归档；仍缺 Excel 原生导入与完整外部系统联调，不能标记完成。 |
| US-M1-005 客商开票信息 | M1 | - | - | 当前实现和证据不足以证明该故事全部验收标准，禁止以局部页面、接口或静态文件标记完成。 |
| US-M1-006 用户与权限管理 | M1 | S2 | L1、L2、L3、L4、L5、L7、L8、L11 | 已补用户创建后端切片、生产菜单和管理端弹窗/API 接线：用户名/姓名/手机号/密码/角色校验、货主隔离、多角色原子绑定、幂等、密码哈希、审计、真实浏览器创建用户与角色绑定和本地门户截图均有证据，并已登记 OpenAPI；截图需由 PR 附件或 CI artifact 长期归档。仍缺完整角色/用户/权限矩阵验收和发布证据，不能标记完成。 |
...
| US-M4-002 波次规划 | M4 | - | - | 已补 M4 PC 波次列表、详情、创建、刷新和未拣选取消真实 API：后端在同一事务内校验已确认订单、按合格库存进行货主隔离锁定、生成按库位排序的拣选明细和 M-TE 拣选任务、更新订单入波次，取消时释放库存锁定并恢复订单状态，且写入幂等和审计；独立 PostgreSQL 回归与临时数据库浏览器 E2E 已验证列表读取、创建响应、库存分配、拣选任务、刷新回显、详情读取、取消响应和真实页面截图。故事仍不能整体关闭：容量/完整路径规则、任务执行回写、异常回滚和完整波次规则证据尚未闭环；另波次模板（货主级+平台默认继承、5 种类型、合单条件组、容量七项、触发方式、变更直接生效仅审计、创建即全字段快照）、波次状态机（pending/allocating/released/picking/reviewing/completed + partial_failed/cancelled）与随货同行单维度参数化（order/wave）已随 wave-model.md 定稿未实施（登记见 US-M4-002b）。 |
| US-M4-002b 波次模板与出库箱 | M4 | S3 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | wave-model.md 已定稿未实施：波次模板（货主级+平台默认继承、5 种类型、合单条件组、容量上限七项、触发方式、变更直接生效仅审计、创建即全字段快照）、出库箱（lpn_containers 的 outbound_box 容器 + box_no/LPN 合一 + 1..N 周转箱绑定 + 一条箱明细只能绑定一个周转箱）、拣选入箱双模式（direct_box/bound_tote，pick_container_mode 模板字段）、复核双模式（review_mode box/tote）、缺口处理三策略（wait_and_split/hold_wave/fail_order）、集货/装箱装车/发运扫码（staging_scan_mode/loading_scan_mode）及随货同行单维度参数化（order/wave）均未实施。 |
| US-M4-003 PDA 拣选作业 | M4 | S3 | L1、L2、L3、L4、L5、L6、L8、L9、L10、L11 | 已补 PDA 批量拣货提交与周转箱/追溯码绑定接口（POST /api/v1/outbound/pick-tasks/batch-complete）、波次拣货最优动线聚合接口（GET /api/v1/outbound/waves/{wave_id}/pick-summary）、物理操作时间戳与单据幂等审计留痕，并通过 E2E 测试套件验证；实体手持终端硬件扫码交互、整件/零件混拣动态调度、离线冲突决议和真实 PDA 运行证据尚未全部闭环，暂在 deferred_stories 中保留并跟踪。 |
| US-M4-004 出库复核 | M4 | S3 | L1、L2、L3、L4、L5、L7、L8、L9、L10、L11 | 已补出库复核查询/提交闭环：状态、逐行商品/数量、拣选与复核分离、短拣、货主隔离、幂等和审计；PC 页面提交前查询 M-VR 出库/复核策略，服务端按整单最严格策略强制第二复核员资质和已通过的 H4 主管审批，并在 outbound_review_records 落规则/双人/审批证据；全量复核完成后原子创建 M-TE 装车任务，短拣不提前创建。一次性 PostgreSQL 真实浏览器 E2E 已验证策略展示、第二人提交与成功回显。故事仍不能整体关闭：装车任务执行回写、PDA 整件/零件扫码、离线冲突、追溯码和真实 PDA/外部运行证据尚未闭环；另复核双模式（review_mode：box 按出库箱聚合 / tote 按周转箱逐箱，问题箱 exception 不阻塞）、集货位（location_type='staging' + STG 位号编排 + staging_scan_mode 扫码强度）、装箱+装车合并（明细转移+数量校验+周转箱释放回 idle，装箱确认失败 exception）与发运（loading_scan_mode 默认 mandatory、随货同行单核对）已随 wave-model.md 定稿未实施（登记见 US-M4-002b）。 |
...
| US-M5-001 冷链设备台账 | M5 | S3 | L1、L2、L3、L4、L5、L8、L9、L10、L11 | 已补冷链设备台账的 PostgreSQL 创建、列表、更新和停用切片：设备类型受控、同货主编码唯一、货主隔离、监控中停用门禁、幂等重放和审计均有真实数据库测试及 OpenAPI 契约；故事仍不能整体关闭，校准到期预警/企业微信通知、验证报告附件、外部冷链同步、前端台账和真实浏览器证据尚未闭环。 |
...
| US-M6-003 业务报表 | M6 | S2 | L1、L2、L3、L4、L8、L9、L10 | 已补 POST /api/v1/reports/query 的出入库汇总真实 PostgreSQL 查询，按货主统计 receiving_orders/outbound_orders，并覆盖未知报表错误契约和两个货主的数据隔离；故事仍不能整体关闭：前端报表菜单/页面、完整日报月报与周转率/岗位绩效/合规维度、图表、Excel/PDF、定时邮件/企业微信推送、导出审计和正式 E2E/截图证据尚未闭环。 |
...
| US-M9-001 计费规则配置 | M9 | S2 | L1、L2、L3、L4、L5、L8、L9、L10、L11 | 已补计费规则的计费项、计量单位、周期、非负费率、生效窗口与合同窗口校验、同货主合同隔离、并发窗口冲突、Idempotency-Key 幂等回放/冲突、规则写入与审计同事务 PostgreSQL 证据，并新增正式菜单迁移、PC 配置页面、页面级 self-check 和真实浏览器 E2E/截图。仍缺阶梯价格、H-APV 配置审批、真实 GET 历史查询和正式发布证据，不能标记完成。 |
...
| US-PK-002 装箱作业 | PK | S3 | L1、L2、L3、L4、L5、L8、L9、L10、L11 | 已补周转箱条码状态秒级预检接口（GET /api/v1/packing/totes/{tote_code}/status）、可用/占用/封箱状态校验、OpenAPI 契约及 E2E 测试。故事仍保留延期：包装工位物理电子秤、称重校验联动、冷链保温箱选型及真实包装台操作证据待 Wave 5/6 实施。 |
...
| US-MPM-004 映射执行 API | MPM | - | - | 已落地 PostgreSQL 持久单值映射 API、全局/货主规则优先级、四类匹配查询、未命中去重队列、L11 幂等、权限与同事务 H2 审计，并由真实 PostgreSQL 验证服务重建后规则仍生效；当前仅接通 H8 商品储存条件，仍缺批量接口、缓存失效、规则维护入口、完整字段追溯及其余调用方证据，故事保持延期。 |
...
| US-QL-001 质量联系单类型与审批模板配置 | QL | S3 | L1、L2、L3、L4、L5、L8、L9、L10、L11 | 已实现货主级类型读取/写入、指定审批人、H4 模板、超时时长、启停、权限、审计与幂等配置后端；仍缺预置类型及默认时长、按角色/按货主审批人规则、PC 配置页和超时提醒执行器，故不得整体关闭。 |
| US-QL-002 创建质量联系单 | QL | S3 | L1、L2、L3、L4、L5、L8、L9、L10、L11 | 已实现手工创建、M-CG 发号、H4 审批记录同事务创建、详情查询、货主隔离、权限、审计与幂等；仍缺 M-VR/M2/M3 自动触发、档案补录与召回专用字段校验、PC/PDA 入口及真实企业微信推送。 |
| US-QL-003 质量联系单审批 | QL | S3 | L1、L2、L3、L4、L5、L8、L9、L10、L11 | 已实现指定审批人、同意/拒绝、必填意见、H4 与 M-QL 状态原子回写、审计和幂等；档案补录批准后进入待 ERP 同步并原子生成 H8 outbox。仍缺企业微信外部回调签名验真、真实通知送达、审批超时提醒、ERP 落地回调及外部联调证据。 |
| US-QL-004 质量联系单与业务流程联动 | QL | S3 | L1、L2、L3、L4、L5、L7、L8、L9、L10、L11 | 已实现审批通过后同事务创建 M-SA 销毁报损单，并由 M-SA 执行链调用 M-VR 销毁节点双人/H4 策略；档案补录审批已在同一事务内校验真实 ASN、收货记录、货主、仓库、商品和照片，进入待 ERP 同步并生成 H8 outbox；错误来源或缺少照片会回滚 H4 与 M-QL 状态，同键重放不重复 outbox。仍缺校验放行、验收不合格其他处置、养护异常、库存状态调整、档案补录 ERP 回写后的 M1/M-QL/M2 解锁和召回完整状态联动。 |
...
| US-SA-001 报损单管理 | SA | S4 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | 已完成报损单手工/ERP 创建、M-CG 发号、ERP 外部单号去重、待审批/待执行/执行中/已完成主状态、质量联系单审批回写入口、报损/销毁节点 M-VR 三档双人策略、两名保管员资格与 H4 主管审批门禁、M3 原子扣减、召回销毁标记解除、审计、幂等、权限、查询及 ERP 反馈 outbox，并通过一次性 PostgreSQL 测试。仍缺 M-QL 联系单实体及企业微信真实审批回调、ERP outbox 投递/失败重试 worker 与真实 ERP 回执、PDA 扫码/离线同步和真机证据，故不得整体关闭。 |
| US-SA-002 报溢单管理 | SA | S4 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | 已完成报溢单手工/ERP 创建、M-CG 发号、ERP 外部单号并发去重、待审批/待执行/执行中/已完成主状态、质量联系单审批回写入口、报溢节点 M-VR 三档双人策略、两名保管员资格与 H4 主管审批门禁、M2 温区/色标/容量校验、M3 库存与库位容量原子增加、审计、幂等、权限、状态查询及 ERP 反馈 outbox，并通过 PostgreSQL 测试。仍缺 M-QL 联系单实体及企业微信真实审批回调、ERP outbox 投递/失败重试 worker 与真实 ERP 回执、PDA 扫码/离线同步和真机证据，故不得整体关闭。 |
...
| US-TE-003 任务创建（各模块触发） | TE | S3 | L1、L2、L3、L4、L5、L8、L9、L10、L11 | 已实现统一任务模型、业务源幂等、待分配初态和审计，并接入 M2 验收通过→上架、M4 波次下发→拣选、最终复核→装车三条原子触发链。仍缺补货、移库、盘点、销退上架等其余业务触发源以及全链路发布证据。 |
| US-TE-005 任务分配 | TE | S3 | L1、L2、L3、L4、L5、L7、L8、L9、L10、L11 | 已实现手工分派、按资格有效期、当前活跃负荷和最大并发容量自动分派，以及改派、下发、召回；状态与分配事件原子写入，PC 调度页已通过一次性 PostgreSQL 真实浏览器 E2E 和本地门户截图，截图需由 PR 附件或 CI artifact 长期归档。仍缺就近定位、冷库技能匹配和真实 PDA 推送证据。 |
| US-TE-007 任务合并 | TE | - | - | 当前实现和证据不足以证明该故事全部验收标准，禁止以局部页面、接口或静态文件标记完成。 |
| US-TE-008 任务执行（PDA） | TE | S4 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | 已实现 PDA 面向的本人统一待办 API，以及已下发→执行中→完成/异常状态机、本人执行权限、数量差异拦截、幂等和审计。受 ADR-0027 约束尚未启动生产 PDA 应用，扫码三/四步、SQLite 离线缓存、冲突待主管处理和真机证据仍缺。 |
| US-TE-009 任务跟踪看板 | TE | S1 | L1、L2、L3、L7、L8、L10 | 已补按状态、类型、仓库、人员与优先级展示的 PC 任务调度列表和主管干预；支持关闭或 5/15/30 秒自动刷新，并按任务预计时长高亮执行超时和未接单超时，真实 PostgreSQL 浏览器 E2E/截图已验证。仍缺超时企业微信通知和 PAD 视图。 |
...
| US-VR-006 双人策略矩阵规则 | VR | S4 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | 已完成 6 流程 12 节点、8 类默认模板、全局/货主/仓库优先级、双人确认、M1 字典双向同步、幂等、审计、权限、查询缓存与变更失效；PC 矩阵、M2 验收、M4 复核及 M-SA 报损/报溢/销毁执行均已接入三档策略，M-QL 审批通过可同事务创建销毁报损单并进入该执行链，规则/第二人/H4 审批证据可回查。仍缺正式环境 P95 指标，以及 ADR-0027 解禁后的 PDA 离线 24 小时缓存/恢复校验和真实 PDA 证据，故不得整体关闭。 |
| US-H8-001 ERP 连接配置与安全接入 | H8 | S4 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | 开发实现层 PASS：US-H8-001 AC1-16 的配置/运行时主链、route-resolve、scope、幂等/乐观锁、在途 pause/resume、测试重叠校验、熔断半开和真实浏览器 E2E 已覆盖。外部证据层 DEFERRED：尚缺客户指定厂商正式 ERP dev/staging 回执（ERP_CALLBACK_BASE）；该缺口不阻塞普通开发迭代，但阻塞故事整体完成与正式发布。 |
| US-H8-002 ERP 消息交换与 WMS 语义转换 | H8 | S4 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | 开发实现层 PASS：五类入站共享 H8 REST 防腐层与接口表 canonical 主流程；商品 spec 已收紧为 Rust/Python/MSSQL/OpenAPI/api-client 全链必填并覆盖缺失零写入，M1 四类商品直写拒绝 API Key、GET 保留；商品必需映射未命中时 WMS 以 422/H8 死信/M1 零写入拒绝且 Rust Worker 将违规接口行置死信，不再创建 pending_mapping 商品；完整商品/包装契约、M-PM 映射溯源、七类出站生产者和主动对账拉取均已覆盖。目录驱动测试逐类验证双通道 L2-L4，真实 Docker MSSQL 验证商品约束及七类出站降级 L11。外部证据层 DEFERRED：ERP 发布器真实的 INSERT 前映射校验、自研 ERP dev/staging 双向请求、两级回执、重试和审计关联留待外部环境具备后执行，不阻塞普通开发迭代。 |
| US-H8-003 ERP 消息日志、监控与故障重放 | H8 | S4 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | 开发实现层 PASS：消息分区与全局唯一、日统计、两级回执、claim 租约、重试时间、精确查询、稳定游标、多仓裁剪、Worker 健康、暂停恢复、权限审计和加密报文保留均有 PostgreSQL、隔离 MSSQL 或真实浏览器证据。预发布运行层 DEFERRED：待补单货主单月 10M 消息+10M 完成尝试的列表/统计 P95 原始证据。外部证据层 DEFERRED：待补自研 ERP dev/staging 故障恢复、回执、重放和审计关联；两类延期均不阻塞普通开发迭代。 |
| US-M1-004b 容器质量锁管理 | M1 | S3 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | Phase 1 后端与管理端容器页加锁/换原因/解锁已落地；PDA 扫码、波次三层兜底/拣选中断、质量区 7/3 天滞留时限仍待补齐。 |
| US-M3-012 补货作业 | M3 | S3 | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | Phase 2 已实施：策略/任务 API、PC 页、execute 列表契约与 H4 种子已落地。PDA 生产 UI 受 ADR-0027 阻断，真实 E2E 截图仍待补齐。明确不做：波次缺口不按字面二次扣减本波次占用（GWT 4 qty=7）；作业冻结挂起走 200+suspended 不走 422（GWT 17）。 |
...
| US-M1-004 仓库与库位管理 | M1 | S2 | write、api_change、frontend_interaction | L1、L2、L3、L4、L5、L7、L8、L9、L11 | m1-warehouses、m1-zones、m1-locations | GET /api/v1/master-data/warehouses<br>POST /api/v1/master-data/warehouses<br>PATCH /api/v1/master-data/warehouses/{id}<br>GET /api/v1/master-data/warehouse-zones<br>POST /api/v1/master-data/warehouse-zones<br>PATCH /api/v1/master-data/warehouse-zones/{id}<br>GET /api/v1/master-data/locations<br>GET /api/v1/master-data/locations/by-code/{location_code}<br>POST /api/v1/master-data/locations<br>POST /api/v1/master-data/locations/batch-create<br>PATCH /api/v1/master-data/locations/{id} | requirement:verified<br>fields:verified<br>frontend:verified<br>api:verified<br>backend:verified<br>database:verified<br>security:verified<br>audit:verified<br>tests:verified<br>evidence:verified<br>docs:verified<br>governance:verified |
| US-M1-004a 容器（LPN）管理 | M1 | S3 | write、api_change、frontend_interaction、concurrent_resource | L1、L2、L3、L4、L5、L6、L7、L8、L9、L11 | m1-lpn-containers | GET /api/v1/master-data/lpn-containers<br>POST /api/v1/master-data/lpn-containers<br>POST /api/v1/master-data/lpn-containers/batch-create<br>GET /api/v1/master-data/lpn-containers/{id}<br>PATCH /api/v1/master-data/lpn-containers/{id}<br>DELETE /api/v1/master-data/lpn-containers/{id}<br>GET /api/v1/master-data/lpn-container-type-policies<br>PUT /api/v1/master-data/lpn-container-type-policies | requirement:verified<br>fields:verified<br>frontend:verified<br>api:verified<br>backend:verified<br>database:verified<br>security:verified<br>audit:verified<br>tests:verified<br>evidence:verified<br>docs:verified<br>governance:verified |
...
| US-M3-005 库存盘点 | M3 | S3 | write、api_change、inventory_change、concurrent_resource、audit_compliance、frontend_interaction | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | m3-counts | GET /api/v1/inventory/counts<br>POST /api/v1/inventory/counts<br>POST /api/v1/inventory/counts/quick-spot-count<br>GET /api/v1/inventory/counts/{id}<br>POST /api/v1/inventory/counts/{id}/lines/{line_id}/submit<br>POST /api/v1/inventory/counts/{id}/approve | requirement:verified<br>fields:verified<br>frontend:verified<br>api:verified<br>backend:verified<br>database:verified<br>security:verified<br>audit:verified<br>tests:verified<br>evidence:verified<br>docs:verified<br>governance:verified |
...
| US-M1-010 设备中台（AGV/PTL/DWS/RFID） | M1 | S3 | write、api_change、frontend_interaction、concurrent_resource、audit_compliance | L1、L2、L3、L4、L5、L6、L7、L8、L9、L10、L11 | m1-devices、m1-device-dashboard | GET /api/v1/device-dashboard<br>GET /api/v1/iot-events<br>GET /api/v1/iot-devices<br>POST /api/v1/iot-devices<br>GET /api/v1/iot-devices/{id}<br>PATCH /api/v1/iot-devices/{id}<br>POST /api/v1/iot-devices/{id}/heartbeat<br>POST /api/v1/iot-devices/{id}/events<br>POST /api/v1/location-device-bindings<br>POST /api/v1/location-device-bindings/{id}/unbind<br>GET /api/v1/wcs-tasks<br>GET /api/v1/wcs-tasks/{id}<br>POST /api/v1/wcs-tasks<br>POST /api/v1/wcs-tasks/{id}/dispatch<br>POST /api/v1/wcs-tasks/{id}/receipt<br>POST /api/v1/wcs-tasks/{id}/resend<br>POST /api/v1/wcs-tasks/{id}/void<br>POST /api/v1/wcs-tasks/{id}/confirm-skip | requirement:verified<br>fields:verified<br>frontend:verified<br>api:verified<br>backend:verified<br>database:verified<br>security:verified<br>audit:verified<br>tests:verified<br>evidence:verified<br>docs:verified<br>governance:verified |
