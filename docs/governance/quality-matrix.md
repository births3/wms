...
| US-H8-003 ERP 消息日志、监控与故障重放 | H8 | 消息/尝试存储、两级回执、claim 租约、普通 Worker 成功/失败完成尝试、服务端精确高级查询、created_at+id 稳定游标分页、按连接/通道/消息类型统计及 P95、Worker 心跳健康、暂停恢复、权限审计、完整报文短期加密保留/授权解密/no-store/到期清理，以及本地 Docker MSSQL 死信/人工重放状态闭环均有真实 PostgreSQL 或关闭 dev-mock 的浏览器证据；仍缺按 ADR-0018 持久化 next_retry_at、JWT 仓库范围接线、生产约定数据量 P95、生产 RANGE 月分区和自研 ERP 故障恢复 S4。 |
