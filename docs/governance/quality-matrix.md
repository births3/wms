...
| US-AL-001 告警定义注册 | AL | 已完成 M-CG 同事务 no-gap 生成服务、M2 ASN 自动编号，以及 M4 出库订单按 M1 单据类型字典校验并自动编号的 PostgreSQL 后端切片；仍缺 M4 真实前端创建接口接入、M3/其他单据创建方接入、配置审批和全链路真实 E2E 证据，不能关闭故事。 |
...
| US-M1-010 特殊药品分类字典管理 | M1 | 已补齐特殊药品字典的 8 类预置、合规属性矩阵、通用字典维护和自定义启用分类被商品真实 PostgreSQL 写入路径消费；仍缺分类字段/合规属性修改的仓库主管+质量负责人双人审批及企业微信生效回调、M-PM 枚举同步、M-VR 规则模板自动重生成、专用台账与专柜专库运行联动、真实浏览器 E2E 和完整 GSP 回查证据，禁止标记完成。 |
...
| US-M2-003 PDA/PC Web 验收 | M2 | 已补验收累计量不得超过实际收货量、零数量拒绝、签字前总量闭合及每条入库明细必须存在验收记录，并用内存、PostgreSQL、审计回放和真实浏览器 E2E（含超量 422 失败路径与截图）验证；故事仍不能整体关闭：验收岗角色与 M-VR 动态策略/规则 ID、逐商品/逐批号前端作业、多商品 ASN、质量联系单/企业微信、不合格分流、档案补录、PDA 离线和完整权限/审计断言尚未闭环。 |
| US-M2-004 双人验收签字 | M2 | 已补服务端双签资格门禁：内存路径限制第一签字人为当前调用者，PostgreSQL 路径要求两名签字人均为同一货主下启用的 receiving_clerk 验收岗并拥有 m2.write，跨货主/未绑定用户返回 M2_VERIFIER_UNAUTHORIZED；已用内存、PostgreSQL、处理器、幂等和审计回放测试验证。故事仍不能整体关闭：M-VR 三档策略与规则 ID、待第二人签字状态、签字结论/备注、30 分钟超时、电子药检单附件、真实前端双签 E2E/截图和完整审计断言尚未闭环。 |
| US-M2-005 PDA/PC Web 智能上架 | M2 | 已补推荐库位 API 与既有 PC Web 上架确认的同货主、温区/质量颜色、容量、SKU 数量、验收数量、幂等和审计约束，并以真实 PostgreSQL 覆盖推荐排序、跨货主、非法状态/数量和角色权限；故事仍不能整体关闭：m2-putaway 页面尚未接入真实接口，M-VR 动态上架策略、LPN/任务引擎、全自动模式、PDA 离线冲突和真实浏览器 E2E/截图证据尚未闭环。 |
...
| US-M3-001 实时库存查询 | M3 | 已补实时库存查询的库位类型 location_type、库区编码 zone_code 过滤、库位/库区关联查询和货主隔离，并以真实 PostgreSQL 覆盖组合条件、空值条件、未知条件和跨货主数据；故事仍不能整体关闭：内存 mock 没有库位主数据、库位专项视图及温区/库位元信息、批次专项视图、Excel 导出、开放 API 调用凭据和完整真实 E2E 证据尚未闭环。 |
| US-M4-002 波次规划 | M4 | 已补 M4 PC 波次列表、详情、创建、刷新和未拣选取消真实 API：后端在同一事务内校验已确认订单、按合格库存进行货主隔离锁定、生成按库位排序的拣选任务、更新订单入波次，取消时释放库存锁定并恢复订单状态，且写入幂等和审计；独立 PostgreSQL 回归与临时数据库浏览器 E2E 已验证列表读取、创建响应、库存分配、拣选任务、刷新回显、详情读取、取消响应和真实页面截图。故事仍不能整体关闭：容量/完整路径规则、真正下发状态、拣选任务执行、异常回滚和完整波次规则证据尚未闭环。 |
| US-M4-003 PDA 拣选作业 | M4 | 当前实现和证据不足以证明该故事全部验收标准，禁止以局部页面、接口或静态文件标记完成。 |
| US-M4-004 出库复核 | M4 | 已补出库复核查询/提交最小闭环：复核岗权限、订单状态、逐行商品和数量一致性、拣选人与复核人分离、短拣状态、货主隔离、幂等重放和审计 diff，并接入 m4-review PC 页面真实 GET/POST API；修正复核页状态筛选，使用临时 PostgreSQL 数据库完成真实浏览器详情读取、弹窗展示、提交复核和截图验证。故事仍不能整体关闭：包装站/PDA 三模式扫码、M-VR 动态双人策略与主管审批、离线冲突、追溯码、完整复核记录以及真实外部运行证据尚未闭环。 |
...
| US-TE-001 任务类型配置 | TE | 已补任务类型配置后端和 PC 管理端切片：预置拣选/上架/补货/移库/盘点/装车/退货上架并支持扩展编码，维护默认优先级、预计耗时、可合并、可插单和启停状态；支持货主隔离、角色权限、幂等、审计、真实 PostgreSQL、真实 PC API E2E 和截图，并已挂载 OpenAPI 与菜单迁移。仍缺任务引擎创建/分配/执行链路、跨模块触发、PDA 和发布证据，不能标记完成。 |
