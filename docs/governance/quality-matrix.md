...
| US-H8-002 ERP 消息交换与 WMS 语义转换 | H8 | 故事与验收边界已补齐；现有 Worker 只证明本地五类入站、七类出站和双通道切片，尚缺逐消息类型 canonical/M-PM 契约、完整 PostgreSQL 业务链、统一错误与客户正式 ERP S4 证据。 |
| US-H8-003 ERP 消息日志、监控与故障重放 | H8 | 已补独立故事，但消息主记录/尝试记录、月分区、查询统计、并发租约、死信重放 API、H8 ERP 消息页面和真实浏览器/客户 ERP 故障恢复证据尚未实现。 |
