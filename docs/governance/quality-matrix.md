...
| US-M4-003 PDA 拣选作业 | M4 | - | - | 当前实现和证据不足以证明该故事全部验收标准，禁止以局部页面、接口或静态文件标记完成。拣选入箱双模式（direct_box 直装=箱级等待 / bound_tote 绑定周转箱=任务级等待，pick_container_mode 模板字段）、出库箱（outbound_box 容器 + box_no/LPN 合一 + 拣选任务挂 box_id 按箱聚合）、周转箱绑定（1..N、一条箱明细只能绑定一个周转箱）与缺口处理三策略（wait_and_split 默认 / hold_wave 窗口 / fail_order + urgent 补货联动）已随 wave-model.md 定稿未实施（登记见 US-M4-002b）。 |
