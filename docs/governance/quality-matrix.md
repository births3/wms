...
| US-H8-003 ERP 消息日志、监控与故障重放 | H8 | 消息/尝试存储、两级回执、claim 租约、普通 Worker 成功/失败完成尝试、ADR-0018 L2 next_retry_at 与 MSSQL 到期认领、档案补录 L3 的 5 次/5 分钟/24 小时边界、服务端精确高级查询、created_at+id 稳定游标分页、按连接/通道/消息类型统计及 P95、JWT 用户多仓列表/统计/详情裁剪、API Key 单仓操作约束、Worker 心跳健康、暂停恢复、权限审计、完整报文短期加密保留/授权解密/no-store/到期清理，以及本地 Docker MSSQL 死信/人工重放状态闭环均有真实 PostgreSQL、隔离 MSSQL 或关闭 dev-mock 的浏览器证据；仍缺生产约定数据量 P95、生产 RANGE 月分区和自研 ERP 故障恢复 S4。 |
