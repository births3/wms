...
| **H-INT** | **统一外部集成能力（v3.2 新增，契约先行）** | **integration** | **所有外部对接（H4/H5/H8/M5/M10/M-TC）统一接入契约；强制复用 ADR-0018 弹性 + M-PM 规整 + ADR-0013 凭证 + H2 审计；引擎延后，详见 ADR-0030** |
| **H-FILE** | **统一附件/文件能力（v3.3 新增）** | **file-attachment** | **附件上传/下载/关联/留存统一收口；MinIO/S3 + attachments 表 + presigned URL（详见 infra/file-storage.md）；GSP 附件 ≥5 年；详见 ADR-0031** |
| **H-APV** | **审批引擎（v3.4 新增，契约先行）** | **approval-engine** | **审批编排统一收口（谁审/几级/阈值升级/拒绝回退/留痕）；复用 M3-003 approval_source 锚点 + H4 通道 + H6 状态机；引擎延后，详见 ADR-0032** |
| **H-SCH** | **调度引擎（v3.5 新增，契约先行）** | **scheduler** | **系统级定时/周期触发统一注册/防重/监控；复用 ADR-0018 重试 + H-AL 告警 + H2 审计；不接管 M-TE 作业调度与 H10 备份；引擎延后，详见 ADR-0033** |
...
| 编号 | 任务 | 当前状态 |
|-----|------|------|
| W1.A | H1 权限/多租户基础 | 开发完成 |
| W1.B | H2 审计追踪基础设施 | 开发完成；真实 dev PostgreSQL 压测 + 7 天封档证据为预发布 gate |
| W1.C | H3 OpenAPI 契约工具链（utoipa + gen-api） | 完成 |
| W1.D | 文件版 Feature Flag 治理 + dev/staging 自动回滚链路 | 开发完成；真实 dev/staging 自动回滚证据为预发布 gate |
| W1.E | `apps/web-admin` 壳工程接入 H1/H2/H3 与 `@wms/ui` / `@wms/api-client` | 开发完成 |
| W1.F | H-INT 统一外部集成能力契约段 | 完成 |
| W1.G | H-APV 审批引擎契约段 | 完成 |
| W1.H | H-SCH 调度引擎契约段 | 完成 |

H1 / H2 / H3 三者完全独立，可同时进行。**Wave 1 开发完成是后续所有业务模块的前置条件；当前完成状态以 `just wave-1-complete-check` 为准。**
...
| 2026-05-29 | v3.2 | 横向能力 12 → 13：新增 H-INT 统一外部集成能力（契约先行，引擎延后）；登记关联 ADR-0030 |
