...
export function MCGDocumentNumberingPage() {
  ...
  return (
    <ListPageTemplate
      notice={
        notice
          ? { kind: notice.type, text: notice.text }
          : documentTypeOptionsQuery.isError
            ? { kind: "error", text: errorText(documentTypeOptionsQuery.error, "读取 M1 单据类型字典失败") }
            : null
      }
      queryFields={mcgDocumentNumberQueryFields}
      coreQueryFieldKeys={mcgDocumentNumberCoreQueryFieldKeys}
      ...
      dialogs={
        <>
          {/* DisableDialog: 单据号规则启停经过确认 */}
          <Dialog open={disableDialogOpen} onOpenChange={(open) => !busy && setDisableDialogOpen(open)}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle>{disableRuleTarget?.enabled ? "停用单据号规则" : "启用单据号规则"}</DialogTitle>
                <DialogDescription>
                  确认{disableRuleTarget?.enabled ? "停用" : "启用"}规则“{disableRuleTarget?.rule_name}”？{disableRuleTarget?.enabled ? "停用后新单据无法使用此规则生成编号。" : "启用后新单据将按此规则生成编号。"}
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
                <DialogClose asChild><Button type="button" variant="outline" disabled={busy}>取消</Button></DialogClose>
                <Button type="button" disabled={busy || !disableRuleTarget} onClick={() => void confirmToggleRule()}>{busy ? LOADING_PROCESSING : "确认"}</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
              ...
              <form className="grid gap-4" onSubmit={submitForm}>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="grid gap-1 text-sm">
                    <span className="font-medium">规则编码 *</span>
                    <Input required disabled={Boolean(editingRule)} value={form.ruleCode} onChange={(event) => updateForm("ruleCode", event.target.value)} placeholder="purchase-inbound" />
                  </label>
                  <label className="grid gap-1 text-sm">
                    <span className="font-medium">规则名称 *</span>
                    <Input required value={form.ruleName} onChange={(event) => updateForm("ruleName", event.target.value)} placeholder="采购入库单号" />
                  </label>
                  <label className="grid gap-1 text-sm">
                    <span className="font-medium">{COLUMN_DOCUMENT_TYPE} *</span>
                    <select className="h-9 rounded-md border border-input bg-background px-3 text-sm" value={form.documentType} onChange={(event) => updateForm("documentType", event.target.value)} aria-label={COLUMN_DOCUMENT_TYPE} disabled={!dictionaryReady}>
                      <option value="">请选择</option>
                      {documentTypeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
                    </select>
                  </label>
                  <label className="grid gap-1 text-sm">
                    <span className="font-medium">重置策略</span>
                    <select className="h-9 rounded-md border border-input bg-background px-3 text-sm" value={form.resetPolicy} onChange={(event) => updateForm("resetPolicy", event.target.value)} aria-label="重置策略">
                      <option value="daily">每日重置</option>
                      <option value="continuous">连续递增</option>
                    </select>
                  </label>
                  <label className="grid gap-1 text-sm">
                    <span className="font-medium">流水位数 (1-12) *</span>
                    <Input required type="number" min="1" max="12" step="1" value={form.sequenceWidth} onChange={(event) => updateForm("sequenceWidth", event.target.value)} />
                  </label>
                ...
                <label className="grid gap-1 text-sm">
                  <span className="font-medium">编码模板 *</span>
                  <span className="text-xs text-muted-foreground">可用占位符：{`{OWNER}`}、{`{YYYY}`}、{`{YY}`}、{`{MM}`}、{`{DD}`}、{`{SEQ}`}</span>
                  <textarea required className="min-h-24 rounded-md border border-input bg-background px-3 py-2 font-mono text-sm" value={form.template} onChange={(event) => updateForm("template", event.target.value)} placeholder="{OWNER}-ASN-{YYYY}{MM}{DD}-{SEQ}" />
                </label>
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="grid gap-1 text-sm">
                    <span className="font-medium">生效时间</span>
                    <Input type="datetime-local" value={form.effectiveFrom} onChange={(event) => updateForm("effectiveFrom", event.target.value)} />
                  </label>
                  <label className="grid gap-1 text-sm">
                    <span className="font-medium">失效时间</span>
                    <Input type="datetime-local" value={form.effectiveTo} onChange={(event) => updateForm("effectiveTo", event.target.value)} />
                  </label>
                </div>
                <DialogFooter>
                  <DialogClose asChild><Button type="button" variant="outline" disabled={busy}>取消</Button></DialogClose>
          ...
          <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
            <DialogContent className="sm:max-w-xl">
              <DialogHeader>
                <DialogTitle>单据号示例预览</DialogTitle>
                <DialogDescription>预览只生成示例，不会占用真实流水号。</DialogDescription>
              </DialogHeader>
              {previewRule ? (
                <div className="grid gap-4 text-sm">
                  <div className="rounded-md border bg-muted/30 p-4 text-center">
                    <div className="mb-2 text-xs text-muted-foreground">示例单据号</div>
                    <code className="break-all font-mono text-lg font-bold tracking-wider text-primary">
                      {renderPreview(previewRule.template, previewRule.sequence_width)}
                    </code>
                  </div>
                  <dl className="grid grid-cols-[110px_1fr] gap-x-4 gap-y-2">
                    <dt className="text-muted-foreground">规则编码</dt><dd>{previewRule.rule_code}</dd>
                    <dt className="text-muted-foreground">单据类型</dt><dd>{documentTypeLabel(previewRule.document_type, documentTypeOptions)}</dd>
                    <dt className="text-muted-foreground">流水策略</dt><dd>{previewRule.reset_policy === "daily" ? "每日重置" : "连续递增"} / {previewRule.sequence_width} 位</dd>
                    <dt className="text-muted-foreground">{COLUMN_STATUS}</dt><dd>{previewRule.enabled ? STATUS_ENABLED : STATUS_DISABLED}</dd>
                  </dl>
                </div>
              ) : null}
              <DialogFooter>
    ...
    >
      <Card className="flex flex-1 flex-col min-h-0 overflow-hidden rounded-lg shadow-sm">
