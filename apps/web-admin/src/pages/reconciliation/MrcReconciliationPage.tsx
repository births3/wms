    ...
    <section className="flex w-full flex-col gap-5 px-4 py-8 lg:px-8">
      ...
      {ruleQuery.isError && (
        <div
          ...
          role="alert"
        >
          读取对账频率失败：{errorText(ruleQuery.error, "请检查权限和服务状态")}。为避免覆盖真实配置，当前禁止维护。
        </div>
      )}
      {notice && (
        <div
      ...
      <QueryPanel
        fields={mRcReconciliationQueryFields}
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          {resolveMutation.isError && (
            <div className="text-sm text-destructive" role="alert">
              处理库存对账差异失败：{errorText(resolveMutation.error, "请稍后重试")}
            </div>
          )}
          <DialogFooter>
            <DialogClose asChild>
              <Button type="button" variant="outline" disabled={resolveMutation.isPending}>取消</Button>
            </DialogClose>
            <Button
              type="button"
              disabled={
