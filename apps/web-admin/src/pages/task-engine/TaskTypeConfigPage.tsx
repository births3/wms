...
import {
  Button, Card, CardContent, Dialog, DialogClose, DialogContent, DialogDescription,
  DialogFooter, DialogHeader, DialogTitle, Input, ListPageTemplate, StatusBadge,
  formatDateTime, type DataGridColumn, type DataGridCreateAction,
...
export function TaskTypeConfigPage() {
  ...
  const disableAction: DataGridDisableAction = {
    ...
    description: "切换任务类型状态",
    disabled: (ctx) => ctx.selectedRowKeys.length !== 1 || busy,
    onClick: () => {
      if (selectedRow) {
        setToggleTarget(selectedRow);
        setToggleOpen(true);
      }
    },
  };
  ...
  return (
    <ListPageTemplate
      banner={<PriorityRuleCard />}
      notice={notice}
      queryFields={queryFields}
      coreQueryFieldKeys={mteTaskTypeCoreQueryFieldKeys}
      queryValue={draftQuery}
      onQueryValueChange={setDraftQuery}
      onQuery={() => applyQuery(draftQuery)}
      onReset={resetQuery}
      gridProps={{
        storageKey: "mte.task-types",
        columns,
        data: rows,
        rowKey: (r) => r.task_type_code,
        selectable: true,
        selectedRowKeys: selected,
        onSelectedRowKeysChange: setSelected,
        caption: query.isPending ? "加载任务类型..." : undefined,
        emptyTitle: query.isError ? "读取任务类型失败" : "暂无任务类型",
        emptyDescription: query.isError ? errorText(query.error, ERROR_AUTH_API_CHECK) : "暂无可用任务类型",
        exportFileBaseName: "M-TE-task-types",
        refreshAction,
        createAction,
        editAction,
        disableAction,
      ...
      dialogs={
        <>
          {/* TaskTypeDisableDialog: 启停写操作统一经过确认 */}
          <Dialog open={toggleOpen} onOpenChange={(open) => !busy && (setToggleOpen(open), !open && setToggleTarget(null))}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle>{toggleTarget?.enabled ? "停用任务类型" : "启用任务类型"}</DialogTitle>
                <DialogDescription>确认{toggleTarget?.enabled ? "停用" : "启用"}任务类型“{toggleTarget?.task_type_name ?? ""}”？</DialogDescription>
              </DialogHeader>
              <DialogFooter>
                <DialogClose asChild><Button type="button" variant="outline" disabled={busy}>取消</Button></DialogClose>
                <Button type="button" disabled={busy || !toggleTarget} onClick={() => void confirmToggle()}>{busy ? LOADING_PROCESSING : "确认"}</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
