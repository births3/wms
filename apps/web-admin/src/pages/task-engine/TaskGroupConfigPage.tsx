...
import {
  ...
  DialogFooter, DialogHeader, DialogTitle, Input, ListPageTemplate, Select, SelectContent,
...
export function TaskGroupConfigPage() {
  ...
  return (
    <ListPageTemplate
      notice={notice}
      queryFields={mteTaskGroupQueryFields}
      coreQueryFieldKeys={mteTaskGroupCoreQueryFieldKeys}
      queryValue={draftQuery}
      onQueryValueChange={setDraftQuery}
      onQuery={() => applyQuery(draftQuery)}
      onReset={resetQuery}
      gridProps={{
        storageKey: "mte.task-groups",
        columns,
        data: rows,
        rowKey: (row) => row.id,
        selectable: true,
        ...
        caption: groupsQuery.isPending ? "加载任务组..." : undefined,
        emptyTitle: groupsQuery.isError ? "读取任务组失败" : "暂无任务组",
        emptyDescription: groupsQuery.isError ? errorText(groupsQuery.error, "请检查鉴权和 API 服务") : "业务创建首个任务时会生成仓库默认任务组，也可手工新增",
        refreshAction,
        createAction,
        editAction,
        exportFileBaseName: "M-TE-task-groups",
