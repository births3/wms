...
import {
  ...
  DialogFooter, DialogHeader, DialogTitle, ListPageTemplate, Select, SelectContent,
  SelectItem, SelectTrigger, SelectValue, StatusBadge, formatDateTime,
...
const mteTaskDispatchCoreQueryFieldKeys = ["keyword", "status"];
...
export function TaskDispatchPage() {
  ...
  const filters = { status: queryString(appliedQuery.status), taskTypeCode: queryString(appliedQuery.taskTypeCode), warehouseId: queryString(appliedQuery.warehouseId) };
  ...
  const {
    ...
    openWith: openConfirmWith,
    setOpen: setConfirmOpen,
    setTarget: setConfirmAction,
  } = useDialogState<{ task: WarehouseTask; action: TaskTransitionAction }>();
  ...
  const toolbarActions: DataGridToolbarAction[] = [
    { key: "release", label: "释放", description: "按任务类型释放条件放入待分配池", icon: <Send className="size-4" aria-hidden />, disabled: () => !selectedTask || selectedTask.status !== "pending_release" || busy, onClick: () => { if (selectedTask) { openConfirmWith({ task: selectedTask, action: "release" }); } } },
    { key: "auto-assign", label: "自动分派", description: "按任务组资格和当前负荷选择执行人", icon: <UserPlus className="size-4" aria-hidden />, disabled: () => !selectedTask || selectedTask.status !== "pending_assignment" || busy, onClick: () => { if (selectedTask) { openConfirmWith({ task: selectedTask, action: "assign" }); } } },
    ...
    { key: "dispatch", label: "下发", description: "下发到保管员 PDA 待办", icon: <Send className="size-4" aria-hidden />, disabled: () => !selectedTask || selectedTask.status !== "assigned" || busy, onClick: () => { if (selectedTask) { openConfirmWith({ task: selectedTask, action: "dispatch" }); } } },
    { key: "recall", label: "召回", description: "从已分配或已下发状态召回待分配池", icon: <RotateCcw className="size-4" aria-hidden />, disabled: () => !selectedTask || !["assigned", "dispatched"].includes(selectedTask.status) || busy, onClick: () => { if (selectedTask) { openConfirmWith({ task: selectedTask, action: "recall" }); } } },
    { key: "expedite", label: "手动加急", description: "按优先级规则一次性提升任务优先级", icon: <Zap className="size-4" aria-hidden />, disabled: () => !selectedTask || selectedTask.manually_expedited || ["completed", "cancelled"].includes(selectedTask.status) || busy, onClick: () => { if (selectedTask) { openConfirmWith({ task: selectedTask, action: "expedite" }); } } },
    { key: "resolve", label: "处置完成", description: "主管确认异常任务完成", icon: <CheckCircle2 className="size-4" aria-hidden />, disabled: () => !selectedTask || selectedTask.status !== "exception" || busy, onClick: () => { if (selectedTask) { openConfirmWith({ task: selectedTask, action: "resolve_complete" }); } } },
    { key: "cancel", label: "取消", description: "取消待释放、待分配或异常任务", icon: <XCircle className="size-4" aria-hidden />, variant: "destructive", disabled: () => !selectedTask || !["pending_release", "pending_assignment", "exception"].includes(selectedTask.status) || busy, onClick: () => { if (selectedTask) { openConfirmWith({ task: selectedTask, action: "cancel" }); } } },
  ];

  return (
    <ListPageTemplate
      notice={notice}
      queryFields={mteTaskDispatchQueryFields}
      coreQueryFieldKeys={mteTaskDispatchCoreQueryFieldKeys}
      ...
      gridProps={{
        storageKey: "mte.task-dispatch",
        columns: taskColumns(workerNames),
        data: rows,
        rowKey: (row) => row.id,
        selectable: true,
        selectedRowKeys: selected,
        onSelectedRowKeysChange: setSelected,
        caption: tasksQuery.isPending ? "加载任务队列..." : undefined,
        emptyTitle: tasksQuery.isError ? "读取任务队列失败" : "暂无任务",
        emptyDescription: tasksQuery.isError ? errorText(tasksQuery.error, "请检查鉴权和 API 服务") : "业务事件触发后任务会进入待分配池",
        refreshAction,
        toolbarActions,
        exportFileBaseName: "M-TE-tasks",
        ...
        <>
          <Dialog open={assignOpen} onOpenChange={(open) => !busy && setAssignOpen(open)}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle>{assignAction === "assign" ? "分派任务" : "改派任务"}</DialogTitle>
                <DialogDescription>仅展示任务组“{selectedTask?.task_group_code ?? ""}”内的有效成员。</DialogDescription>
              </DialogHeader>
              <label className="grid gap-1 text-sm">
                执行人
                <Select value={assigneeId} onValueChange={setAssigneeId}>
                  <SelectTrigger><SelectValue placeholder="请选择合格成员" /></SelectTrigger>
                  <SelectContent>
              ...
              {qualifiedUsers.length === 0 && (
                <div className="rounded-md border border-wms-warning/30 bg-wms-warning/10 px-3 py-2 text-sm text-wms-warning" role="status">
                  该任务组尚未配置成员，请先在“任务组与人员资格”页面维护。
                </div>
              )}
              <DialogFooter>
                <DialogClose asChild><Button type="button" variant="outline" disabled={busy}>取消</Button></DialogClose>
                <Button type="button" disabled={busy || !selectedTask || !assigneeId} onClick={() => selectedTask && void mutate(selectedTask, assignAction, assigneeId)}>
                  {busy ? "处理中..." : "确认分派"}
                </Button>
              </DialogFooter>
            </DialogContent>
          ...
          <Dialog open={confirmOpen} onOpenChange={(open) => !busy && setConfirmOpen(open)}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  {confirmAction ? `确认对任务 ${confirmAction.task.task_no} 执行“${actionLabels[confirmAction.action]}”？此操作会立即更新任务并写入审计。` : "请确认任务操作。"}
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
                <DialogClose asChild><Button type="button" variant="outline" disabled={busy}>取消</Button></DialogClose>
                <Button type="button" variant={confirmAction?.action === "cancel" ? "destructive" : "default"} disabled={busy || !confirmAction} onClick={() => confirmAction && void mutate(confirmAction.task, confirmAction.action)}>
                  {busy ? "处理中..." : "确认执行"}
                </Button>
              </DialogFooter>
            </DialogContent>
...
function taskAttention(task: WarehouseTask, now = Date.now()) {
