...
import { CheckCircle2, RotateCcw, Send, UserPlus, XCircle } from "lucide-react";
...
export function TaskDispatchPage() {
  ...
  return <section className="flex w-full flex-col gap-5 px-4 py-8 lg:px-8">
    ...
    <Dialog open={confirmOpen} onOpenChange={(open) => !busy && setConfirmOpen(open)}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>确认任务操作</DialogTitle><DialogDescription>{confirmAction ? `确认对任务 ${confirmAction.task.task_no} 执行“${actionLabels[confirmAction.action]}”？此操作会立即更新任务状态并写入审计。` : "请确认任务操作。"}</DialogDescription></DialogHeader><DialogFooter><DialogClose asChild><Button type="button" variant="outline" disabled={busy}>取消</Button></DialogClose><Button type="button" variant={confirmAction?.action === "cancel" ? "destructive" : "default"} disabled={busy || !confirmAction} onClick={() => confirmAction && void mutate(confirmAction.task, confirmAction.action)}>{busy ? "处理中..." : "确认执行"}</Button></DialogFooter></DialogContent></Dialog>
  </section>;
  ...
  async function mutate(task: WarehouseTask, action: TaskTransitionAction, assigneeUserId?: string) {
    ...
    try {
      ...
      setAssignOpen(false); setConfirmOpen(false); setConfirmAction(null); setSelected([result.id]); setNotice({ kind: "success", text: `任务 ${result.task_no} 已更新为${statusLabels[result.status] ?? result.status}` });
    } catch (error) { setNotice({ kind: "error", text: errorText(error, "任务状态更新失败") }); }
  }
}

const actionLabels: Record<TaskTransitionAction, string> = { assign: "自动分派", dispatch: "下发", reassign: "改派", recall: "召回", start: "开始", complete: "完成", report_exception: "上报异常", resolve_complete: "处置完成", cancel: "取消" };
