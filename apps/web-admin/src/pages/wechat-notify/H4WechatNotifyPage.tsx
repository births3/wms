...
export function H4WechatNotifyPage({ mode }: H4WechatNotifyPageProps) {
  ...
  if (mode === "settings") {
    return (
      <ListPageTemplate
        notice={notice ? { kind: notice.type, text: notice.text } : null}
        banner={settingsQueryResult.error ? <ErrorPanel message={settingsQueryResult.error.message} /> : null}
        queryFields={h4WechatSettingsQueryFields}
        coreQueryFieldKeys={h4WechatSettingsCoreQueryFieldKeys}
        queryValue={settingsQuery}
        onQueryValueChange={(next) => setSettingsQueryState(normalizeSettingsQuery(next))}
  ...
  if (mode === "records") {
    return (
      <ListPageTemplate
        notice={notice ? { kind: notice.type, text: notice.text } : null}
        banner={recordsQuery.error ? <ErrorPanel message={recordsQuery.error.message} /> : null}
        queryFields={h4NotificationRecordQueryFields}
        coreQueryFieldKeys={h4NotificationRecordCoreQueryFieldKeys}
        queryValue={recordQuery}
        onQueryValueChange={(next) => setRecordQuery(normalizeRecordQuery(next))}
  ...
  return (
    <ListPageTemplate
      notice={configDialog.open || sendDialog.open ? null : notice ? { kind: notice.type, text: notice.text } : null}
      banner={configsQuery.error ? <ErrorPanel message={configsQuery.error.message} /> : null}
      queryFields={h4NotificationConfigQueryFields}
      coreQueryFieldKeys={h4NotificationConfigCoreQueryFieldKeys}
      queryValue={configQuery}
      onQueryValueChange={(next) => setConfigQuery(normalizeConfigQuery(next))}
