...
import { CheckCircle2, Loader2, Route, Send } from "lucide-react";
...
export function TmsRoutePlanPage() {
  ...
  const errorMessage = validationError ?? (mutation.error ? `${mutation.error.message} (HTTP ${mutation.error.status})` : null);

  return (
    <ConfigPageTemplate
      notice={errorMessage ? { kind: "error", text: errorMessage } : null}
      configSlot={
        <Card>
          <CardHeader>
      ...
      feedbackSlot={receivedPlan ? <ReceivedPlan plan={receivedPlan} /> : null}
    />
  );
}
