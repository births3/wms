...
export function ErpConnectorConfigPage({
  ...
  return (
    <ListPageTemplate
      ...
      queryFields={h8ErpConnectorQueryFields}
      coreQueryFieldKeys={h8ErpConnectorCoreQueryFieldKeys}
      queryValue={draftQuery}
      onQueryValueChange={(next) => setDraftQuery(normalizeQuery(next))}
      ...
      dialogs={
        <>
              ...
              >
                ...
                <label className="grid gap-1 text-sm">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        channel_mode: e.target.value as H8ErpConnector["channel_mode"],
                      }))
                    }
                  >
                ...
                <label className="grid gap-1 text-sm">
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm((f) => ({ ...f, bearer_secret_alias: e.target.value }))}
                  />
                </label>
                <label className="grid gap-1 text-sm">
                  接口表探查账号（只读）
                  <Input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, interface_probe_db_username: e.target.value }))
                    }
                  />
                </label>
                <label className="grid gap-1 text-sm">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, interface_probe_db_password_alias: e.target.value }))
                    }
                  />
                </label>
                <DialogFooter>
          ...
          <Dialog open={editOpen} onOpenChange={(open) => !busy && setEditOpen(open)}>
            {/* 表单较长，限制高度并允许滚动，避免“保存”按钮溢出视口不可点 */}
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
              <form
                className="grid gap-4"
                onSubmit={(event) => {
              ...
              >
                ...
                <label className="grid gap-1 text-sm">
                  ...
                  <select
                    ...
                    onChange={(e) => setEditForm((f) => ({ ...f, channel_mode: e.target.value as H8ErpConnector["channel_mode"] }))}
                  >
                    <option value="rest">rest</option>
                    <option value="interface_table">interface_table</option>
                    <option value="rest_primary_table_fallback">rest_primary_table_fallback</option>
