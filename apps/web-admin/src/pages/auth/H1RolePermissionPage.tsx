...
export function H1RolePermissionPage({ currentUser }: { currentUser: CurrentUser }) {
  ...
  if (!canManage) {
    return (
      <section className="flex w-full flex-1 min-h-0 flex-col gap-5 px-4 py-8 lg:px-8">
        <PageHeader />
  ...
  return (
    <MasterDetailPageTemplate
      ...
      queryFields={h1RoleQueryFields}
      coreQueryFieldKeys={h1RoleCoreQueryFieldKeys}
      queryValue={draftQuery}
      onQueryValueChange={(next) => setDraftQuery(normalizeRoleQuery(next))}
      onQuery={() => applyQuery(draftQuery)}
      onReset={resetQuery}
      leftSlot={
        <Card className="flex flex-1 flex-col min-h-0 min-w-0 overflow-hidden rounded-lg shadow-sm">
          <CardContent className="flex flex-1 flex-col min-h-0 space-y-3 p-4">
