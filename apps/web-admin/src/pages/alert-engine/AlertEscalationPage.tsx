...
import {
  ...
  DialogFooter, DialogHeader, DialogTitle, Input, ListPageTemplate,
  StatusBadge, formatDateTime, type DataGridColumn,
...
export function AlertEscalationPage() {
  ...
  return (
    <ListPageTemplate
      ...
      queryFields={alertEscalationQueryFields}
      coreQueryFieldKeys={alertEscalationCoreQueryFieldKeys}
      queryValue={draftQuery}
      onQueryValueChange={setDraftQuery}
      onQuery={() => applyQuery(draftQuery)}
      onReset={resetQuery}
      gridProps={{
        storageKey: "hal.alert-escalation-rules",
        columns,
        data: rows,
        rowKey: (row) => row.id,
        selectable: true,
        ...
        caption: query.isPending ? "加载升级规则..." : undefined,
        emptyTitle: query.isError ? "读取升级规则失败" : "暂无升级规则",
        emptyDescription: query.isError ? errorText(query.error, "请检查权限和 API 服务") : "新增规则后可在告警定义中引用",
        tableClassName: "min-w-[1770px]",
        exportFileBaseName: "H-AL 告警升级规则",
        refreshAction,
        createAction,
        editAction,
      ...
      dialogs={
        <EscalationRuleDialog
          ...
          errorMessage={formError ?? upsert.error?.message}
