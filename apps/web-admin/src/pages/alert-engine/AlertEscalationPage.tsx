...
export function AlertEscalationPage() {
  ...
  const [editing, setEditing] = React.useState<AlertEscalationRule | null>(null);
  ...
  return <section className="flex w-full flex-col gap-5 px-4 py-8 lg:px-8">
    ...
    <QueryPanel fields={alertEscalationQueryFields} defaultVisibleFieldKeys={alertEscalationCoreQueryFieldKeys} value={draftQuery} onValueChange={setDraftQuery} onQuery={() => setAppliedQuery(normalizeQuery(draftQuery))} onReset={resetQuery} />
    <Card><CardContent className="p-5"><DataGrid storageKey="hal.alert-escalation-rules" columns={columns} data={rows} rowKey={(row) => row.id} selectable selectedRowKeys={selected} onSelectedRowKeysChange={setSelected} caption={query.isPending ? "加载升级规则..." : undefined} emptyTitle={query.isError ? "读取升级规则失败" : "暂无升级规则"} emptyDescription={query.isError ? errorText(query.error, "请检查权限和 API 服务") : "新增规则后可在告警定义中引用"} tableClassName="min-w-[1770px]" exportFileBaseName="H-AL 告警升级规则" refreshAction={refreshAction} createAction={createAction} editAction={editAction} queryState={appliedQuery} querySummaryItems={buildQueryPanelSummaryItems(alertEscalationQueryFields, appliedQuery)} onApplyQueryState={(value) => { const next = normalizeQuery(value); setDraftQuery(next); setAppliedQuery(next); }} onClearQueryState={resetQuery} /></CardContent></Card>
...
function filterRules(rows: AlertEscalationRule[], query: QueryPanelValue) { const keyword = text(query.keyword).trim().toLocaleLowerCase(); const enabled = text(query.enabled); const role = text(query.handlerRole).trim().toLocaleLowerCase(); return rows.filter((row) => (!keyword || `${row.rule_code} ${row.rule_name}`.toLocaleLowerCase().includes(keyword)) && (!enabled || row.enabled === (enabled === "true")) && (!role || row.off_hours_handler_roles.some((value) => value.toLocaleLowerCase().includes(role)))); }
function defaultQuery(): QueryPanelValue { return { keyword: "", enabled: "", handlerRole: "" }; }
function normalizeQuery(value: unknown): QueryPanelValue { const row = value && typeof value === "object" ? value as QueryPanelValue : {}; return { keyword: text(row.keyword), enabled: text(row.enabled), handlerRole: text(row.handlerRole) }; }
