...
import {
  ...
  type DataGridRefreshAction, type QueryPanelField, type QueryPanelRangeValue,
...
export function AlertDashboardPage() {
  ...
  return <section className="flex w-full flex-col gap-5 px-4 py-8 lg:px-8">
    ...
    <QueryPanel fields={alertDashboardQueryFields} defaultVisibleFieldKeys={alertDashboardCoreQueryFieldKeys} value={draftQuery} onValueChange={setDraftQuery} onQuery={() => setAppliedQuery(normalizeQuery(draftQuery))} onReset={resetQuery} />
    <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><CardTitle>活动告警</CardTitle><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={!selectedAlert || alertAction.isPending} onClick={() => openAction("acknowledge")}>确认接警</Button><Button variant="outline" disabled={!selectedAlert || alertAction.isPending} onClick={() => openAction("handling")}>记录处理</Button><Button disabled={!selectedAlert || alertAction.isPending} onClick={() => openAction("close")}>关闭</Button><Button variant="destructive" disabled={!selectedAlert || alertAction.isPending} onClick={() => openAction("ignore")}>忽略</Button></div></div></CardHeader><CardContent><DataGrid storageKey="hal.active-alerts" columns={alertColumns} data={rows} rowKey={(row) => row.id} selectable selectedRowKeys={selected} onSelectedRowKeysChange={setSelected} caption={active.isPending ? "加载活动告警..." : "严重告警优先，数据最长延迟 5 秒"} emptyTitle={active.isError ? "读取活动告警失败" : "当前没有活动告警"} emptyDescription={active.isError ? errorText(active.error, "请检查权限和仓库范围") : "新告警会在触发后自动出现"} tableClassName="min-w-[1430px]" exportFileBaseName="H-AL 活动告警" refreshAction={refreshAction} queryState={appliedQuery} querySummaryItems={buildQueryPanelSummaryItems(alertDashboardQueryFields, appliedQuery)} onApplyQueryState={(value) => { const next = normalizeQuery(value); setDraftQuery(next); setAppliedQuery(next); }} onClearQueryState={resetQuery} /></CardContent></Card>
  ...
  function resetQuery() { const next = defaultQuery(); setDraftQuery(next); setAppliedQuery(next); }
...
function normalizeQuery(value: unknown): QueryPanelValue { const row = value && typeof value === "object" ? value as QueryPanelValue : {}; return { alertCode: text(row.alertCode), severity: text(row.severity), status: text(row.status), warehouseId: text(row.warehouseId), triggeredAt: range(row.triggeredAt) }; }
function toFilters(value: QueryPanelValue): AlertInstanceFilters { const dates = range(value.triggeredAt); return { alert_code: optional(value.alertCode), severity: optional(value.severity), status: optional(value.status), warehouse_id: optional(value.warehouseId), from: dateBoundary(dates.from, false), to: dateBoundary(dates.to, true) }; }
function range(value: QueryPanelValue[string]): QueryPanelRangeValue { return value && typeof value === "object" && !Array.isArray(value) ? { from: text(value.from), to: text(value.to) } : { from: "", to: "" }; }
...
function optional(value: unknown) { const result = text(value).trim(); return result || undefined; }
