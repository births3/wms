...
import {
  Button, DataGrid, Dialog, DialogClose, DialogContent,
  DialogDescription, DialogFooter, DialogHeader, DialogTitle, ListPageTemplate, PageHeader, QueryPanel,
...
export function AlertDefinitionPage() {
  ...
  return (
    <ListPageTemplate
      notice={notice}
      queryFields={alertDefinitionQueryFields}
      coreQueryFieldKeys={alertDefinitionCoreQueryFieldKeys}
      queryValue={draftQuery}
      onQueryValueChange={setDraftQuery}
      onQuery={() => applyQuery(draftQuery)}
      onReset={resetQuery}
      gridProps={{
        storageKey: "hal.alert-definitions",
        columns,
        data: rows,
        rowKey: (row) => row.id,
        selectable: true,
        ...
        caption: query.isPending ? "加载告警定义..." : undefined,
        emptyTitle: query.isError ? "读取告警定义失败" : "暂无告警定义",
        emptyDescription: query.isError ? errorText(query.error, ERROR_AUTH_API_CHECK) : "请新增首个货主告警定义",
        tableClassName: "min-w-[1595px]",
        exportFileBaseName: "H-AL 告警定义",
        refreshAction,
        createAction,
        detailAction,
        editAction,
