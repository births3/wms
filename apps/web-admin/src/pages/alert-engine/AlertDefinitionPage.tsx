...
import {
  ...
  StatusBadge, buildQueryPanelSummaryItems, type DataGridColumn, type DataGridCreateAction,
...
export const alertDefinitionQueryFields: QueryPanelField[] = [
  ...
  { key: "severity", label: "级别", type: "select", options: [{ label: "全部", value: "" }, { label: "提示", value: "info" }, { label: "警告", value: "warning" }, { label: "严重", value: "critical" }] },
];
...
const columns: DataGridColumn<AlertDefinition>[] = [
  ...
  { key: "event_type", header: "事件类型", width: 190, minWidth: 140, mono: true, filterValue: (row) => row.event_type, copyValue: (row) => row.event_type, filter: { type: "text" } },
  ...
  { key: "enabled", header: "状态", width: 110, minWidth: 90, render: (row) => <StatusBadge status={row.enabled ? "completed" : "isolated"} label={row.enabled ? "启用" : "停用"} size="sm" /> },
  { key: "is_gsp_forced", header: "监管属性", width: 120, minWidth: 100, render: (row) => row.is_gsp_forced ? "GSP 强制" : "一般" },
  { key: "version", header: "版本", width: 90, minWidth: 80, mono: true, sortable: true, sortValue: (row) => row.version },
  { key: "created_at", header: "创建时间", width: 175, minWidth: 145, sortable: true, sortValue: (row) => row.created_at, filterValue: (row) => row.created_at, copyValue: (row) => row.created_at, filter: { type: "dateRange" }, render: (row) => formatDateTime(row.created_at) },
  { key: "updated_at", header: "更新时间", width: 175, minWidth: 145, sortable: true, sortValue: (row) => row.updated_at, filterValue: (row) => row.updated_at, copyValue: (row) => row.updated_at, filter: { type: "dateRange" }, render: (row) => formatDateTime(row.updated_at) },
];
...
export function AlertDefinitionPage() {
  ...
  const refreshAction: DataGridRefreshAction = { label: "刷新", description: "刷新告警定义列表", disabled: query.isFetching, onClick: () => void query.refetch() };
  ...
  const disableAction: DataGridDisableAction = { label: selectedRow?.enabled ? "停用" : "启用", description: "提交告警定义启停审批", disabled: (context) => context.selectedRowKeys.length !== 1 || busy || cannotToggle(selectedRow), onClick: () => selectedRow && openConfirm("set_enabled", selectedRow) };
