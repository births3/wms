...
import {
  buildInboundOrderColumns,
  inboundPrivateActions,
} from "./M2InboundOrderTable";
...
export function M2InboundPage({ mode, currentOwner }: M2InboundPageProps) {
  ...
  async function refreshInbound(message?: string) {
  ...
  function clearGridQueryState(key?: string) {
    if (key) {
      ...
      applyQuery(normalizeM2InboundQueryValue(next, defaultQuery, mode));
      setDraftQuery(normalizeM2InboundQueryValue(next, defaultQuery, mode));
    } else {
      resetQuery();
    }
    setSelectionClearedByUser(false);
  ...
  const tableDetailAction: DataGridDetailAction = {
    label: "详情",
    disabled: !selectedOrder,
    onClick: () => {
      if (selectedOrder) openRowDetail(selectedOrder.id);
    },
  };
  const tablePrivateActions = inboundPrivateActions(mode, selectedOrder, openRowDialog, (id) => void releaseOrder(id));
  const tablePrintAction: DataGridPrintAction = {
    label: "打印",
    description: mode === "receiving" ? "打印 ASN 单" : "打印验收记录单",
    disabled: (context) => context.selectedRowKeys.length !== 1 || !selectedOrder,
    onClick: () => {
      if (selectedOrder) openRowPrint(selectedOrder.id);
    },
  };
  const orderColumns = React.useMemo(
    () => buildInboundOrderColumns({ mode, currentOwner, onOpenDetail: openRowDetail }),
    [mode, currentOwner],
  );
  ...
  return (
    <ListPageTemplate
      ...
      queryFields={m2InboundQueryFields}
      coreQueryFieldKeys={m2InboundCoreQueryFieldKeys}
      queryValue={draftQuery}
      loading={ordersQuery.isPending || ordersQuery.isFetching}
      onQueryValueChange={(next) => setDraftQuery(normalizeM2InboundQueryValue(next, defaultQuery, mode))}
      onQuery={() => {
      ...
      gridProps={{
        ...
        selectedRowKeys,
        onSelectedRowKeysChange: selectOrderKeys,
        ...
        detailAction: tableDetailAction,
        printAction: tablePrintAction,
        toolbarActions: tablePrivateActions,
