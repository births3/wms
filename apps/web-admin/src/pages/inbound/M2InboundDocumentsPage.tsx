...
export function M2InboundDocumentsPage() {
  ...
  return (
    <ListPageTemplate
      notice={notice}
      queryFields={m2InboundDocumentsQueryFields}
      coreQueryFieldKeys={m2InboundDocumentsCoreQueryFieldKeys}
      ...
      gridProps={{
        ...
        onApplyQueryState: (value) => {
          const next = normalizePanelValue(value);
          setDraftQuery(next);
          setAppliedQuery(next);
        },
      ...
      dialogs={
        activeRow && (
          <Dialog open onOpenChange={(open) => !open && closeEntry()}>
            <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-4xl">
              <DialogHeader>
                <DialogTitle>录入资料 · {activeRow.receiptNo}</DialogTitle>
                <DialogDescription>
                  采购订单 {activeRow.purchaseOrderNo} · {activeRow.supplierName} · 所有上传、复用和版本修改均写入审计。
                </DialogDescription>
              </DialogHeader>
              {notice && (
                <div role={notice.kind === "error" ? "alert" : "status"} className={cn("rounded-md border px-3 py-2 text-sm", notice.kind === "error" ? "border-destructive/30 bg-destructive/10 text-destructive" : "border-wms-success/30 bg-wms-success/10 text-wms-success")}>
                  {notice.text}
                </div>
              )}
              <Tabs value={activeTab} onValueChange={(value) => { setActiveTab(value as EntryTab); setFiles([]); setReason(""); setNotice(null); resetImagePreview(); }}>
                <TabsList>
                  <TabsTrigger value="drug">药检单</TabsTrigger>
                  <TabsTrigger value="upstream">上游随货同行单</TabsTrigger>
                </TabsList>
                <TabsContent value="drug" className="mt-4 grid gap-4">
                  <div className="grid gap-4 md:grid-cols-2">
                    <Field label={COLUMN_BATCH_NO}>
                      <select className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={selectedBatch} onChange={(event) => setSelectedBatch(event.target.value)}>
                        {activeRow.batchNos.length ? activeRow.batchNos.map((batchNo) => <option key={batchNo}>{batchNo}</option>) : <option value="">待批号</option>}
                      </select>
                    </Field>
                    <Field label="当前版本"><Input readOnly value={activeRow.drugInspectionVersion ? `v${activeRow.drugInspectionVersion}` : "未录入"} /></Field>
                    <Field label="录入方式">
                      <select className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={drugSource} onChange={(event) => { setDrugSource(event.target.value as "upload" | "reuse"); setFiles([]); resetImagePreview(); }}>
                        <option value="upload">上传新文件</option>
                        <option value="reuse">复用已有药检单</option>
                      </select>
                    </Field>
                    {drugSource === "upload" && (
                      <Field label="图片处理方式">
                        <select className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={processingMode} onChange={(event) => changeProcessingMode(event.target.value)}>
                          <option value="none">不处理</option>
                          <option value="color_enhance">原色增强</option>
                          <option value="black_white_enhance">黑白增强</option>
                        </select>
                      </Field>
                    )}
                    {drugSource === "upload" && (
                      <Field label="报告编号"><Input required value={reportNo} onChange={(event) => setReportNo(event.target.value)} placeholder="请输入药检报告编号" /></Field>
                    )}
                    {drugSource === "upload" && (
                      <Field label="检验结论">
                        <select className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={qualified ? "qualified" : "unqualified"} onChange={(event) => setQualified(event.target.value === "qualified")}>
                  ...
                  {drugSource === "upload" ? (
                    <Field label="药检单文件">
                      <Input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(event) => void selectDrugInspectionFile(event.target.files?.[0])} />
                      <span className="text-xs text-muted-foreground">PDF 不超过 50MB；JPG/PNG 不超过 5MB。图片上传后通过真实 H-FILE 和处理服务生成预览，权威原件不覆盖。</span>
                    </Field>
                  ) : (
                  ...
                  {drugSource === "upload" && originalPreviewUrl && (
                    <div className="grid gap-3 rounded-md border bg-muted/20 p-3 md:grid-cols-2" data-testid="drug-inspection-image-preview">
                      ...
                      <ImagePreview title={`处理后预览${previewDimensions ? ` · ${previewDimensions}` : ""}`} src={processedPreviewUrl} pending={previewStatus === "uploading"} error={previewStatus === "error"} />
                    </div>
                  )}
                  {drugSource === "upload" && (
                    <Field label="修改原因"><Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="当前批号已有版本时必填；新批号可留空" /></Field>
                  )}
                  <VersionNote row={activeRow} kind="drug" />
                  <DialogFooter>
                    <DialogClose asChild><Button type="button" variant="outline">取消</Button></DialogClose>
                    <Button type="button" disabled={!selectedBatch || saveDrugInspection.isPending || previewStatus === "uploading" || previewStatus === "error"} onClick={() => void submitDrugInspection()}>
                      <FileUp className="size-4" aria-hidden />{saveDrugInspection.isPending ? LOADING_SAVING : previewStatus === "uploading" ? "生成预览中..." : "保存药检单"}
                    </Button>
                  </DialogFooter>
                </TabsContent>
                <TabsContent value="upstream" className="mt-4 grid gap-4">
                  ...
                  <div className="rounded-md border">
                    ...
                    <div className="max-h-44 overflow-y-auto p-3">
                      {candidates.map((row) => (
                        <label key={row.id} className="flex items-center gap-3 border-b py-2 last:border-0">
                          <Checkbox checked={selectedAsnIds.includes(row.id)} onCheckedChange={(checked) => toggleCandidate(row.id, checked === true)} />
                          <span className="font-mono text-sm">{row.receiptNo}</span>
                          <span className="text-sm text-muted-foreground">{row.actualReceivedAt ? toWmsBusinessDate(row.actualReceivedAt) : "未收货"} · {upstreamStatusLabel(row.upstreamDeliveryStatus)}</span>
                        </label>
                      ))}
                  ...
                  <DialogFooter>
                    <DialogClose asChild><Button type="button" variant="outline">取消</Button></DialogClose>
                    <Button type="button" disabled={saveUpstreamDelivery.isPending} onClick={() => void submitUpstreamDelivery()}><FileUp className="size-4" aria-hidden />{saveUpstreamDelivery.isPending ? "上传中..." : "上传并完成录入"}</Button>
                  </DialogFooter>
                </TabsContent>
              </Tabs>
            </DialogContent>
