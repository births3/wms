...
function buildM3BatchQueryFields(qualityStatusOptions: QualityStatusOption[]): QueryPanelField[] {
  return [
    { key: "keyword", label: FIELD_KEYWORD, type: "text", placeholder: "批号 / 商品编码 / 库位" },
    ...
    { key: "locationCode", label: "库位", type: "text", placeholder: "按库位编码模糊查询" },
    { key: "zoneCode", label: "库区", type: "text", placeholder: "按库区编码查询" },
    { key: "temperatureZone", label: COLUMN_TEMP_ZONE, type: "text", placeholder: "normal / cool / cold / frozen" },
    { key: "qualityStatus", label: COLUMN_QUALITY_STATUS, type: "multiSelect", options: qualityStatusOptions },
    {
      key: "recallFlag",
      label: "召回",
      type: "multiSelect",
...
export function M3BatchManagementPage({ onOpenLocationHistory }: M3BatchManagementPageProps) {
  ...
  const statusAction: DataGridToolbarAction = {
    ...
    disabled: ({ selectedRowKeys }) => {
      ...
      return (
        ...
        || statusOptionsFor(batch?.status ?? "", qualityStatusOptions, statusTransitions).length === 0
      );
    },
    onClick: ({ selectedRowKeys }) => {
      const batch = batches.find((item) => item.id === selectedRowKeys[0]);
  ...
  const targetStatusOptions = statusOptionsFor(statusBatch?.status ?? "", qualityStatusOptions, statusTransitions);
  ...
  return (
    <ListPageTemplate
      ...
      queryFields={m3BatchQueryFields}
      coreQueryFieldKeys={m3BatchCoreQueryFieldKeys}
      queryValue={draftQuery}
      loading={batchesQuery.isPending || batchesQuery.isFetching}
      onQueryValueChange={(next) => setDraftQuery(normalizeM3BatchQueryValue(next))}
      onQuery={() => {
            ...
            <DialogContent className="sm:max-w-lg">
              <form className="grid gap-4" onSubmit={submitStatus}>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    {statusBatch?.batch_no ?? "库存批次"} 当前为{qualityStatusLabel(statusBatch?.status ?? "", qualityStatusOptions)}，状态变更必须提供审批来源。
                  </DialogDescription>
                </DialogHeader>
                ...
                <label className="grid gap-1 text-sm">
                  审批来源
                  <Input required value={statusForm.approvalSource} onChange={(event) => setStatusForm((value) => ({ ...value, approvalSource: event.target.value }))} placeholder="例如 温度超标事件" />
                </label>
                <label className="grid gap-1 text-sm">
                  审批编号
                  <Input required value={statusForm.approvalId} onChange={(event) => setStatusForm((value) => ({ ...value, approvalId: event.target.value }))} placeholder="例如 TEMP-001" />
                </label>
                <label className="grid gap-1 text-sm">
                  变更原因
                  <Input required value={statusForm.reason} onChange={(event) => setStatusForm((value) => ({ ...value, reason: event.target.value }))} placeholder="说明状态变更原因" />
                </label>
                <DialogFooter>
                  <DialogClose asChild><Button type="button" variant="outline" disabled={statusMutation.isPending}>取消</Button></DialogClose>
                  <Button type="submit" disabled={statusMutation.isPending || !statusForm.targetStatus}>
                    {statusMutation.isPending ? LOADING_SUBMITTING : "确认变更"}
                  </Button>
                </DialogFooter>
              </form>
            </DialogContent>
...
function statusFormFor(
  ...
  return {
    ...emptyStatusForm,
    targetStatus: statusOptionsFor(batch.status, qualityStatusOptions, transitions)[0]?.value ?? "",
  };
}
...
function filterBatches(batches: InventoryBatch[], query: QueryPanelValue, warningDays: number) {
  ...
  return batches.filter((batch) => {
    ...
    return (
      ...
      (qualityStatuses.size === 0 || qualityStatuses.has(batch.status)) &&
...
function toInventoryBatchQuery(query: QueryPanelValue): InventoryBatchQuery {
  ...
  return {
    ...
    status: qualityStatuses.length === 1 ? qualityStatuses[0] : undefined,
...
function InventoryDimensionSummary({
  ...
  const statusSummary = Object.entries(
    dimensionBatches.reduce<Record<string, number>>((summary, batch) => {
      summary[batch.status] = (summary[batch.status] ?? 0) + Number(batch.qty_on_hand);
      return summary;
    }, {}),
