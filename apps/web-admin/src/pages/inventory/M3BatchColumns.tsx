...
export function buildBatchColumns(
  ...
  return [
    ...
    },
    {
      ...
      copyValue: (row) => `现存 ${row.qty_on_hand} / 锁定 ${row.qty_frozen} / 可用 ${availableQty(row)}`,
      filter: { type: "numberRange" },
      render: (row) => (
        <div className="text-sm">
          <div className="font-medium">{row.qty_on_hand} 件</div>
          <div className="text-xs text-muted-foreground">
            锁定 {row.qty_frozen} / 可用 {availableQty(row)}
          </div>
        </div>
      ),
    },
    {
      key: "status", header: "质量状态", width: 150, minWidth: 130, sortable: true,
      sortValue: (row) => qualityStatusLabel(row.status, qualityStatusOptions),
      filterValue: (row) => row.status,
      copyValue: (row) => qualityStatusLabel(row.status, qualityStatusOptions),
      filter: { type: "multiSelect", options: qualityStatusOptions },
      render: (row) => (
        <StatusBadge
          status={qualityStatusKey(row.status, row.recall_flag)}
          label={qualityStatusLabel(row.status, qualityStatusOptions)}
          size="sm"
        />
      ),
    },
