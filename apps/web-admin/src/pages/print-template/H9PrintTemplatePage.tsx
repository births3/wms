...
export function H9PrintTemplatePage({ currentUser }: { currentUser: CurrentUser }) {
  ...
  const errorMsg = templateTypesQuery.error?.message ?? librariesQuery.error?.message ?? templatesQuery.error?.message;
  ...
  return (
    <MasterDetailPageTemplate
      ...
      banner={errorMsg ? <div className="rounded-md border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive">{errorMsg}</div> : null}
      queryFields={h9PrintTemplateQueryFields}
      coreQueryFieldKeys={h9PrintTemplateCoreQueryFieldKeys}
      queryValue={draftQuery}
      onQueryValueChange={(next) => setDraftQuery(normalizeH9QueryValue(next))}
      ...
      rightSlot={
        <DataGrid
          ...
          columns={columns}
          data={rows}
          rowKey={(row: PrintTemplateRow) => row.id}
          selectable
