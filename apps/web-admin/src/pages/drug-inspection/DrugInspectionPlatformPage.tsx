...
const queryFields: QueryPanelField[] = [{
  key: "status", label: "平台状态", type: "select", options: [
    { label: "全部", value: "" }, { label: "已对接", value: "connected" },
    { label: "测试中", value: "testing" }, { label: "停用", value: "disabled" },
  ],
}];
...
const columns: DataGridColumn<DrugInspectionPlatform>[] = [
  ...
  { key: "status", header: "状态", width: 120, render: (row) => <StatusBadge status={statusVariant(row.status)} label={statusLabel(row.status)} size="sm" /> },
  { key: "created_at", header: "创建时间", width: 180, render: (row) => formatDateTime(row.created_at) },
  { key: "updated_at", header: "更新时间", width: 180, render: (row) => formatDateTime(row.updated_at) },
];
...
export function DrugInspectionPlatformPage({ currentUser }: { currentUser: CurrentUser }) {
  ...
  const refreshAction: DataGridRefreshAction = { label: "刷新", description: "刷新当前货主药检平台列表", disabled: platformsQuery.isFetching, onClick: () => void refresh() };
  ...
  return <section className="flex w-full flex-col gap-5 px-4 py-8 lg:px-8">
    ...
    {dialog && <Dialog open onOpenChange={(open) => !busy && !open && setDialog(null)}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
      {dialog === "status" ? <form className="grid gap-4" onSubmit={submitStatus}><DialogHeader><DialogTitle>维护药检平台状态</DialogTitle><DialogDescription>{selected?.platform_name}。状态变更会写入审计追踪。</DialogDescription></DialogHeader><Field label="状态"><select className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={form.status} onChange={(event) => update("status", event.target.value)}><option value="connected">已对接</option><option value="testing">测试中</option><option value="disabled">停用</option></select></Field><DialogFooter><DialogClose asChild><Button type="button" variant="outline">取消</Button></DialogClose><Button type="submit" disabled={busy}>{busy ? "保存中..." : "保存状态"}</Button></DialogFooter></form> : <form className="grid gap-4" onSubmit={submit}><DialogHeader><DialogTitle>{dialog === "create" ? "新增药检平台" : "编辑药检平台"}</DialogTitle><DialogDescription>凭证只提交 Vault 引用；列表和编辑表单不会回显敏感值。编辑时请重新填写凭证引用。</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><Field label="平台编码"><Input required value={form.platform_code} onChange={(event) => update("platform_code", event.target.value)} /></Field><Field label="平台名称"><Input required value={form.platform_name} onChange={(event) => update("platform_name", event.target.value)} /></Field><Field label="API 地址" className="sm:col-span-2"><Input required type="url" placeholder="https://inspection.example/api" value={form.api_url} onChange={(event) => update("api_url", event.target.value)} /></Field><Field label="认证方式"><select className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={form.auth_method} onChange={(event) => update("auth_method", event.target.value)}><option value="api_key">API Key</option><option value="username_password">账号密码</option></select></Field><Field label="超时（秒）"><Input required type="number" min="1" max="300" value={form.timeout_seconds} onChange={(event) => update("timeout_seconds", event.target.value)} /></Field>{form.auth_method === "api_key" ? <Field label="API Key Vault 引用" className="sm:col-span-2"><Input required placeholder="vault://wms/di/platform/api-key" value={form.api_key_alias} onChange={(event) => update("api_key_alias", event.target.value)} /></Field> : <><Field label="账号"><Input required value={form.username} onChange={(event) => update("username", event.target.value)} /></Field><Field label="密码 Vault 引用"><Input required placeholder="vault://wms/di/platform/password" value={form.password_alias} onChange={(event) => update("password_alias", event.target.value)} /></Field></>}<Field label="状态"><select className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={form.status} onChange={(event) => update("status", event.target.value)}><option value="connected">已对接</option><option value="testing">测试中</option><option value="disabled">停用</option></select></Field></div><DialogFooter><DialogClose asChild><Button type="button" variant="outline">取消</Button></DialogClose><Button type="submit" disabled={busy}>{busy ? "保存中..." : "保存配置"}</Button></DialogFooter></form>}
