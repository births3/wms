...
import {
  ...
  DialogFooter, DialogHeader, DialogTitle, Input, ListPageTemplate, StatusBadge,
  cn, type DataGridColumn, type DataGridCreateAction,
  type DataGridEditAction, type DataGridRefreshAction, type DataGridToolbarAction,
  type ListPageNotice, type QueryPanelField, type QueryPanelValue,
} from "@wms/ui";
...
type Notice = ListPageNotice | null;
type Form = {
...
export function DrugInspectionPlatformPage() {
  ...
  async function refresh() {
    ...
    setNotice(result.error ? { kind: "error", text: errorMessage(result.error, "读取药检平台列表失败") } : { kind: "success", text: "药检平台列表已刷新" });
  }
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setDialog(null); setSelectedRowKeys([]); setNotice({ kind: "success", text: "药检平台配置已保存" });
    } catch (error) { setNotice({ kind: "error", text: errorMessage(error, "保存药检平台配置失败") }); }
  }
  async function submitStatus(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setDialog(null); setSelectedRowKeys([]); setNotice({ kind: "success", text: "药检平台状态已更新" });
    } catch (error) { setNotice({ kind: "error", text: errorMessage(error, "更新药检平台状态失败") }); }
  }
  ...
  return (
    <ListPageTemplate
      ...
      coreQueryFieldKeys={mDiPlatformCoreQueryFieldKeys}
      queryValue={draftQuery}
      onQueryValueChange={setDraftQuery}
      onQuery={() => setAppliedQuery(draftQuery)}
      onReset={() => {
        setDraftQuery({ status: "" });
        setAppliedQuery({ status: "" });
      }}
      gridProps={{
        storageKey: "m-di.drug-inspection-platforms",
        columns,
        data: rows,
        rowKey: (row) => row.id,
        selectable: true,
        ...
        caption: platformsQuery.isPending ? "加载药检平台..." : undefined,
        emptyTitle: platformsQuery.isError ? "读取药检平台失败" : "暂无药检平台配置",
        emptyDescription: platformsQuery.isError
          ? errorMessage(platformsQuery.error, "请检查鉴权和数据库连接")
          : "请新增药检平台对接配置",
        refreshAction,
        createAction,
        editAction,
        ...
        onApplyQueryState: (value) => {
          const next = { status: queryString((value as QueryPanelValue).status) };
          setDraftQuery(next);
      ...
      dialogs={
        dialog && (
          <Dialog open onOpenChange={(open) => !busy && !open && setDialog(null)}>
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
              {dialog === "status" ? (
                <form className="grid gap-4" onSubmit={submitStatus}>
                  <DialogHeader>
                    <DialogTitle>维护药检平台状态</DialogTitle>
                    <DialogDescription>{selected?.platform_name}。状态变更会写入审计追踪。</DialogDescription>
                  </DialogHeader>
                ...
                <form className="grid gap-4" onSubmit={submit}>
                  <DialogHeader>
                    <DialogTitle>{dialog === "create" ? "新增药检平台" : "编辑药检平台"}</DialogTitle>
                    <DialogDescription>
                      凭证只提交 Vault 引用；列表和编辑表单不会回显敏感值。编辑时请重新填写凭证引用。
                    </DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label="平台编码">
                      <Input
                        ...
                        onChange={(event) => update("platform_code", event.target.value)}
                      />
                    </Field>
                    <Field label="平台名称">
                      <Input
                        ...
                        onChange={(event) => update("platform_name", event.target.value)}
                      />
                    </Field>
                    <Field label="API 地址" className="sm:col-span-2">
                      <Input
                        required
                        type="url"
                        placeholder="https://inspection.example/api"
                        value={form.api_url}
                    ...
                    {form.auth_method === "api_key" ? (
                      <Field label="API Key Vault 引用" className="sm:col-span-2">
                        <Input
                          required
                          placeholder="vault://wms/di/platform/api-key"
                          value={form.api_key_alias}
                          onChange={(event) => update("api_key_alias", event.target.value)}
                        />
                      </Field>
                    ) : (
                      <>
                        ...
                        <Field label="密码 Vault 引用">
                          <Input
                            required
                            placeholder="vault://wms/di/platform/password"
                            value={form.password_alias}
                            onChange={(event) => update("password_alias", event.target.value)}
                          />
                        </Field>
                      </>
                    )}
