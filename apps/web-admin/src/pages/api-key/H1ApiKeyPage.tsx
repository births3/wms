...
export function H1ApiKeyPage({ currentUser }: { currentUser: CurrentUser }) {
  ...
  const [notice, setNotice] = React.useState<ListPageNotice | null>(null);
  ...
  async function refresh() {
    ...
    setNotice(result.error ? { kind: "error", text: result.error.message } : { kind: "success", text: "API Key 列表已刷新" });
  }
  ...
  async function revoke(id: string) {
    ...
    try {
      ...
      setSelectedRowKeys([]);
      setNotice({ kind: "success", text: "API Key 已吊销；重复吊销保持幂等" });
    } catch (error) {
      setNotice({ kind: "error", text: errorText(error, "吊销 API Key 失败") });
    }
  }
  ...
  async function submitCreate(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setCreateOpen(false);
      setNotice({ kind: "success", text: `创建成功。明文 secret 只展示一次：${created.secret ?? "未返回"}` });
    } catch (error) {
      setNotice({ kind: "error", text: errorText(error, "创建 API Key 失败") });
    }
  }
  ...
  async function submitRotate(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!Number.isInteger(days) || days < 0) {
      setNotice({ kind: "error", text: "宽限期必须是大于等于 0 的整数天" });
      return;
    }
    try {
      ...
      setRotateOpen(false);
      setSelectedRowKeys([]);
      setNotice({ kind: "success", text: `轮换成功。新 secret 只展示一次：${rotated.new_key.secret ?? "未返回"}` });
    } catch (error) {
      setNotice({ kind: "error", text: errorText(error, "轮换 API Key 失败") });
    }
  }
  ...
  return (
    <ListPageTemplate
      notice={notice}
      queryFields={apiKeyQueryFields}
      coreQueryFieldKeys={apiKeyCoreQueryFieldKeys}
      ...
      gridProps={{
        ...
        emptyDescription: apiKeysQuery.isError
          ? errorText(apiKeysQuery.error, "请检查鉴权和数据库连接")
          : "请使用创建 Key 录入受控调用方",
