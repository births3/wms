...
import { toReceivingOrderListRow, type ReceivingOrderListRow } from "./receiving-order-list-row";

export type { ReceivingOrderListRow } from "./receiving-order-list-row";
...
async function listReceivingOrders(): Promise<ReceivingOrderListRow[]> {
  ...
  return result.data.data.map(toReceivingOrderListRow);
}
...
export function useReceivingOrdersQuery() {
  return useQuery<ReceivingOrderListRow[], ApiError>({
