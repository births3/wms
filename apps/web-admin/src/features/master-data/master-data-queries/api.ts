...
const systemDictionaryDefinitions = [
  ...
  { code: "container_quarantine_reason", name: "容器隔离原因" },
  { code: "container_rejected_reason", name: "容器不合格原因" },
] as const;
...
export async function batchGenerateLocations(
  request: components["schemas"]["BatchGenerateLocationsRequest"],
): Promise<MasterDataRow[]> {
  const [locationTypeOptions, warehouseRefs] = await Promise.all([
    listSystemDictionaryItemOptions("location_type"),
    listWarehouseRefs(),
  ]);
  const result = await api.POST("/api/v1/master-data/locations/batch-generate", {
    params: {
      header: { "Idempotency-Key": idempotencyKey("web-m1-location-batch-generate") },
    },
    body: request,
  });
  if (!result.data) {
    throw new ApiError(result.error, "批量生成库位失败", result.response.status);
  }
  const locationTypeLabels = new Map(locationTypeOptions);
