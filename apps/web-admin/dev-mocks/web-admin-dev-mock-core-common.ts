...
export async function handleLocationUpdate(req: IncomingMessage, res: ServerResponse, id: string) {
  ...
  const updated: DevLocation = {
    ...
    current_owner_id: asNullableString(body.current_owner_id) ?? location.current_owner_id,
...
export function allDevOrders(): DevOrder[] {
  return [...seedOrders(), ...devCreatedOrders].map((order) => {
    const receipts = model.devReceivingPrintData.get(order.id)?.receipts ?? [];
