...
import { handleH8ErpMessageDevMock } from "./erp-message-dev-mock";
import { handleH8ErpInterfaceTableDevMock } from "./erp-interface-table-dev-mock";
...
import { handleLpnContainerDevMock } from "./lpn-container-dev-mock";
import { handleReconciliationDevMock } from "./reconciliation-dev-mock";
import { handleReplenishmentDevMock } from "./replenishment-dev-mock";
...
async function tryHandleDevMockRoute(
  ...
  if (await handleReplenishmentDevMock(req, res, pathname)) {
    return true;
  }
  ...
  if (pathname.startsWith("/api/v1/integration/erp-messages")) {
    return handleH8ErpMessageDevMock(req, res, pathname);
  }
  if (pathname.startsWith("/api/v1/h8/erp-interface-tables")) {
    return handleH8ErpInterfaceTableDevMock(req, res, pathname);
  }
  ...
  if (await handleLpnContainerDevMock(req, res, pathname)) return true;
...
function devLocationFromCreateRequest(body: Record<string, unknown>): DevLocation | null {
  ...
  return {
    ...
    current_owner_id: asNullableString(body.current_owner_id),
    status: "available",
    created_at: now,
    updated_at: now,
  };
...
function isValidLocationInput(body: Record<string, unknown>) {
  ...
  const boundOwnerId = body.current_owner_id === null || body.current_owner_id === undefined
    ? null
    : asString(body.current_owner_id, "");
