...
assert.match(page, /<QueryPanel|<ListPageTemplate/, "M-CG 必须使用公共查询组件");
...
assert.match(page, /<DataGrid[\s\S]*storageKey="mcg\.document-number-rules"|storageKey:\s*"mcg\.document-number-rules"/, "规则列表必须使用 DataGrid");
...
assert.match(page, /previewDocNumber|renderPreview/, "规则维护必须提供预览");
