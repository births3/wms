...
for (const relativePath of menuPages) {
  ...
  assert.match(source, /(?:lg:px-8|ListPageTemplate|MasterDetailPageTemplate|ConfigPageTemplate|DashboardPageTemplate)/, `${relativePath} 桌面页面左侧留白应为 32px`);
  if (source.includes("<DataGrid") && source.includes("<section")) {
    assert.match(source, /flex-1 min-h-0/, `${relativePath} 列表页必须声明 flex-1 min-h-0 以自适应撑满到底部`);
  }
}
