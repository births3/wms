...
assert.match(page, /(?:<DataGrid\b|<ListPageTemplate\b)/, "M3 批号管理页应复用 DataGrid 或 ListPageTemplate");
assert.match(page, /exportFileBaseName[:=]\s*["']M3 批号管理["']/, "M3 库存查询应使用可识别的 Excel 导出文件名");
assert.doesNotMatch(page, /showExportAction=\{false\}/, "M3 库存查询不得关闭 DataGrid 标准 Excel 导出");
assert.match(page, /detailAction[:=]\s*\{?gridDetailAction\}?/, "M3 批号管理应提供标准详情动作");
assert.match(page, /toolbarActions[:=]\s*(?:\{\s*\[|\[)[\s\S]*statusAction,[\s\S]*recallAction,[\s\S]*cancelRecallAction,[\s\S]*canPrint \? \[printLpnAction\]/, "M3 批号管理应按权限提供标签打印，并保留状态、召回和撤回动作");
...
assert.match(page, /emptyTitle[:=]\s*["']暂无库存批次["']/, "M3 应保持 emptyTitle");
