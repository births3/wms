...
for (const token of [
  "US-H8-004",
  /(?:<QueryPanel|<ListPageTemplate)/,
  /(?:<DataGrid|<ListPageTemplate)/,
...
]) assert.match(page, token instanceof RegExp ? token : new RegExp(token));
for (const label of [
  "同步状态",
  "写入时间（最近 7 天）",
  "外部单据号",
...
assert.match(page, /storageKey:\s*`h8\.erp-interface-tables\.\$\{tableKey\}`/);
assert.match(page, /(?:<DataGrid\s+key=\{tableKey\}|<ListPageTemplate)/);
