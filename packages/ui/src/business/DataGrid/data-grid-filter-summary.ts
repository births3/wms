import { getOperatorLabel, type DataGridAdvancedFilterState } from "./data-grid-operators";
import type {
...
export function buildDataGridFilterSummaryItems(
  ...
  if (advancedFilters?.items && advancedFilters.items.length > 0) {
    ...
    for (const item of advancedFilters.items) {
      const field = fieldByKey.get(item.columnKey);
      if (!field) continue;

      const opLabel = getOperatorLabel(item.operator, true);
      let valStr = "";
      ...
      if (
        ...
        item.operator === "today" ||
        item.operator === "thisWeek" ||
        item.operator === "thisMonth"
      ) {
        valStr = "";
      } else if (item.operator === "between" && typeof item.value === "object" && item.value !== null) {
        const range = item.value as DataGridRangeFilter;
        valStr = `${range.from ?? ""} ~ ${range.to ?? ""}`;
      ...
      const text = valStr ? `${field.label} ${opLabel} "${valStr}"` : `${field.label} ${opLabel}`;
