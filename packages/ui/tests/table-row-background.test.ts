...
const columnsSource = readFileSync(new URL("../src/business/DataGrid/DataGridColumns.tsx", import.meta.url), "utf8");
...
// 表头字段栏（TableHeader / TableHead / 冻结列）必须为 100% 实体纯色背景（bg-muted），严禁使用带透明度的 bg-muted/40 等
assert.match(tableSource, /<thead[^>]*bg-muted(?!\/)/, "ui/table TableHeader must use solid bg-muted without opacity modifier");
assert.doesNotMatch(tableSource, /<thead[^>]*bg-muted\/\d+/, "ui/table TableHeader must NOT use semi-transparent bg-muted/xx");
assert.match(dataTableSource, /<TableHeader[^>]*sticky top-0 z-20 bg-muted/, "DataTable sticky TableHeader must be opaque bg-muted");
assert.match(columnsSource, /className: cn\("z-30 bg-muted"/, "DataGridColumns frozen column header must use solid bg-muted");
