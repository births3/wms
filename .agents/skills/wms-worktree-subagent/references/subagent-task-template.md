...
- 本轮切片覆盖：<本轮实际要做的层>
- 本轮明确不覆盖：<不做的层及原因>
- 禁止结论：如果还有未覆盖层，最终只能写“本切片可合并”，不能写“模块已完成”。
...
14. 运行 `git diff --check`、`just gov-t1` 和任务相关测试；任一失败时写“不可合并”。
15. 使用 wms-review-fix-commit 做 review→修复→review；验证通过且 Git 元数据可写时必须本地分组提交。子 worktree 本地提交是任务收口要求，禁止以“用户未明确要求提交”为理由跳过；仍然禁止推送或合并主分支。
16. 提交 scope 必须使用当前治理白名单允许值；模块名不在白名单时按变更层选择既有 scope，例如后端 API / OpenAPI 用 `接口`，管理端页面用 `管理端`，治理脚本用 `治理`。
17. 不推送，不运行 `git clean -f`、`git reset --hard` 或删除分支。
18. 最终报告只贴摘要、状态、验证和 `git diff --stat`；禁止粘贴大段 patch/diff。需要审查 diff 时交给主代理运行 `git diff`。

最终输出：
- 子 worktree 路径
- 提交哈希；未提交时写 `无` 并说明阻塞
- 完整性结论：`模块完成`、`本切片可合并`、`不可合并` 三选一
...
只读校准任务没有可合并 diff，`是否可合并` 默认写“否：只读不适用合并”。
