...
用于替代旧的 tmux 投递式 issue-agent 流程。核心规则：先提案，后执行；一次确认只对应一次方案；用户验收反馈不对时，必须重新提修正方案并等待新的“确认方案”。
...
6. `codex exec` prompt 必须带上 issue、评论、附件 URL、根因文件、确认对应的 proposal 评论 URL、允许写入范围、验证命令和禁止项。
