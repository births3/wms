...
pub async fn seed_e2e_data(pool: &PgPool) -> Result<(), Box<dyn Error>> {
    ...
    sqlx::query(
        ...
        INSERT INTO warehouse_locations (
            ...
            max_volume_cm3, used_volume_cm3, max_sku_count, location_type, current_owner_id, status
        )
        VALUES
        (
    ...
    sqlx::query(
        ...
        ON CONFLICT (owner_id, product_code, batch_no, location_id, status)
    ...
    sqlx::query(
        ...
        UPDATE print_field_library_versions versions
         ...
         WHERE libraries.id = versions.library_id
           AND libraries.library_code = 'm2_acceptance_record'
           AND versions.version_no = 1
    ...
    crate::wms_api_e2e_seed_lpn::seed_lpn_putaway_order(pool).await?;
    Ok(())
}
