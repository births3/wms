...
#[path = "wms_api_e2e_seed_h9/print_suite_samples.rs"]
mod print_suite_samples;
...
use print_suite_samples::seed_h9_print_suite_samples;
...
pub async fn seed_h9_delivery_note_aggregation(pool: &PgPool) -> Result<(), Box<dyn Error>> {
    ...
    sqlx::query(
        ...
        VALUES ($1, $2, 'sales_outbound', 'OUT-H9-E2E-006', 'ERP-H9-E2E-006',
                $3, $4, '{}'::jsonb, $5, 'confirmed', '2026-07-26T08:00:00Z')
        ON CONFLICT (id) DO UPDATE
        SET status = CASE
            WHEN EXISTS (
    ...
    for (order_uuid, order_no, erp_no, invoice_no) in [
        ...
        sqlx::query(
            ...
            VALUES ($1, $2, 'sales_outbound', $3, $4,
                    $5, $6, '{}'::jsonb, $7, $8, 'confirmed', '2026-07-26T08:05:00Z')
            ON CONFLICT (id) DO UPDATE
