...
async fn inventory_batch_query_combines_filters_and_keeps_owner_scope(pool: PgPool) {
    ...
    for (row_owner_id, batch_no, quality_status) in [
        ...
        sqlx::query(
            "INSERT INTO inventory_batches (id, owner_id, product_code, batch_no, production_date, expiry_date, qty_on_hand, qty_frozen, status, location_id, location_code) VALUES ($1, $2, 'P-QUERY', $3, '2026-01-01', '2028-01-01', 10, 0, $4, $5, 'A-01-01')",
        )
        .bind(Uuid::new_v4())
        .bind(row_owner_id)
        .bind(batch_no)
