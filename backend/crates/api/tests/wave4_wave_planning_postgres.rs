...
async fn seed_inventory(pool: &PgPool, owner_id: Uuid, now: chrono::DateTime<Utc>) {
    ...
    sqlx::query(
        "INSERT INTO warehouse_zones (id, owner_id, warehouse_id, zone_code, zone_name, temperature_zone, quality_color, status) VALUES ($1, $2, $3, $4, '波次计划测试区', 'normal_10_30', 'qualified_green', 'active')",
    )
    .bind(zone_id)
    .bind(owner_id)
    .bind(warehouse_id)
...
async fn outbound_wave_release_persists_tasks_locks_audit_and_replays(pool: PgPool) {
    ...
    let evidence: (i64, i64, Option<String>, i64, i64) = sqlx::query_as(
        r#"SELECT
             ...
             (SELECT qty_frozen::BIGINT FROM inventory_batches WHERE owner_id = $1 AND product_code = 'P-WAVE-001' AND batch_no = 'B-WAVE-001'),
