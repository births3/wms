...
async fn seed_product(pool: &PgPool, category: &str) -> (Uuid, Uuid, Uuid) {
    ...
    sqlx::query(
        "INSERT INTO products (id, owner_id, product_code, product_name, specification, storage_condition, special_drug_category, status) VALUES ($1, $2, $3, '双人策略测试商品', '1 unit', 'normal_10_30', $4, 'active')",
    )
    .bind(product_id)
    .bind(owner_id)
