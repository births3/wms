...
async fn seed_outbound_inventory(
    ...
    sqlx::query(
        "INSERT INTO warehouse_zones (id, owner_id, warehouse_id, zone_code, zone_name, temperature_zone, quality_color, status) VALUES ($1, $2, $3, $4, 'M4 动作测试区', 'normal_10_30', 'qualified_green', 'active')",
    )
    .bind(zone_id)
    .bind(owner_id)
    .bind(warehouse_id)
...
async fn m4_release_wave_locks_inventory_creates_pick_tasks_and_rejects_released(pool: PgPool) {
    ...
    let evidence: (String, String, i64, i64, i64, i64) = sqlx::query_as(
        r#"SELECT
             ...
             (SELECT qty_frozen::BIGINT FROM inventory_batches WHERE owner_id = $1 AND product_code = 'P-RL-001' AND batch_no = 'B-RL-001'),
...
async fn m4_release_wave_rejects_orders_not_confirmed(pool: PgPool) {
    ...
    let evidence: (String, i64, i64) = sqlx::query_as(
        r#"SELECT
             (SELECT status FROM outbound_waves WHERE owner_id = $1 AND id = $2),
             (SELECT COUNT(*) FROM outbound_pick_tasks WHERE owner_id = $1 AND wave_id = $2),
             (SELECT COALESCE(SUM(qty_frozen), 0)::BIGINT FROM inventory_batches WHERE owner_id = $1)"#,
    )
    .bind(owner_id)
    .bind(wave_id)
    .fetch_one(&pool)
