...
async fn products_are_read_from_postgres_by_owner(pool: PgPool) {
    ...
    let now = Utc
        ...
        .single()
        .expect("valid time");
    seed_product(&pool, owner_id, "P-M1-001", "冷藏胰岛素", "cold_2_8", now).await;
    seed_product(
        &pool,
        other_owner_id,
        "P-M1-002",
    ...
    let (rows, total) = PgMasterDataReadRepository::new(pool.clone())
    ...
    assert_eq!(
        rows[0].attrs,
        json!({"storage_condition": "cold_2_8", "source": "api_import"})
    );
}
...
async fn product_list_route_reads_postgres_by_owner(pool: PgPool) {
    ...
    let now = Utc
        ...
        .single()
        .expect("valid time");
    seed_product(&pool, owner_id, "P-M1-101", "接口冷藏胰岛素", "cold_2_8", now).await;
    seed_product(
        &pool,
        other_owner_id,
        "P-M1-102",
...
async fn controlled_product_create_writes_source_and_audit(pool: PgPool) {
    ...
    let mut request =
        controlled_product_request("P-M1-CREATE", "新建冷链商品", "none", "cold_2_8", "manual");
    ...
    assert_eq!(product.attrs["storage_condition"], "cold_2_8");
