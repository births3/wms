...
async fn m1_product_reads_require_master_data_read_permission(pool: PgPool) {
    ...
    sqlx::query(
        "INSERT INTO products (id, owner_id, product_code, product_name, specification, storage_condition, special_drug_category, status) VALUES ($1,$2,'P-READ-PERM','权限测试商品','1盒','normal','none','active')",
    )
    .bind(product_id)
    .bind(owner_id)
    .execute(&pool)
