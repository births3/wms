...
async fn location_with_stock_cannot_be_disabled(pool: PgPool) {
    ...
    .await;
    sqlx::query(
        "INSERT INTO inventory_batches (id, owner_id, product_code, batch_no, production_date, expiry_date, qty_on_hand, qty_frozen, status, location_id, location_code, created_at, updated_at) VALUES ($1, $2, 'P-M1-STOCK', 'B-M1-STOCK', DATE '2026-01-01', DATE '2028-01-01', 1, 0, 'qualified', $3, 'A01-01-01-01', now(), now())",
    )
    .bind(Uuid::new_v4())
    .bind(owner_id)
    .bind(location.id)
