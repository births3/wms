    ...
    let zone: WarehouseZone = json_response(
        ...
        request_json(
            ...
            json!({
                ...
                "temperature_zone": "normal_10_30", "quality_color": "qualified_green"
            }),
        ),
    )
    .await;
    let location: Location = json_response(
        ...
        request_json_with_key(
            ...
            json!({
                ...
                "max_sku_count": 1, "location_type": "storage", "current_owner_id": null
            }),
...
async fn product_supplier_customer_updates_persist_and_append_audit(pool: PgPool) {
    ...
    let product = repository
        .create_product(
            &auth,
            controlled_product_request("P-LIFE-01", "原商品", "none", "normal_10_30", "erp_rest"),
            Utc::now(),
            "product-life-create",
        )
        .await
...
async fn warehouse_zone_rejects_disabled_dictionary_value(pool: PgPool) {
    ...
    .await;
    sqlx::query("UPDATE system_dictionary_items SET enabled=FALSE WHERE dict_code='temperature_zone' AND item_code='normal_10_30' AND owner_id IS NULL")
