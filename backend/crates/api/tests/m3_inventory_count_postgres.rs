...
async fn seed_inventory(pool: &PgPool, owner_id: Uuid, qty: i64) -> (Uuid, Uuid, Uuid, Uuid) {
    ...
    sqlx::query(
        "INSERT INTO warehouse_zones (id, owner_id, warehouse_id, zone_code, zone_name, temperature_zone, quality_color, status) VALUES ($1, $2, $3, 'M3-COUNT-ZONE', 'M3 盘点区', 'normal_10_30', 'qualified_green', 'active')",
    )
    .bind(zone_id)
    .bind(owner_id)
    .bind(warehouse_id)
