...
async fn outbound_wave_can_cancel_before_picking_and_release_inventory(pool: PgPool) {
    ...
    let state: (wms_domain::Quantity, String, String) = sqlx::query_as(
        "SELECT (SELECT qty_frozen FROM inventory_batches WHERE owner_id = $1 AND product_code = $2), (SELECT status FROM outbound_orders WHERE owner_id = $1 AND id = $3), (SELECT status FROM outbound_waves WHERE owner_id = $1 AND id = $4)",
    )
    .bind(owner_id)
