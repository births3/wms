...
async fn outbound_complete_pick_review_ship_replays_and_deducts_inventory(pool: PgPool) {
    ...
    sqlx::query(
        "INSERT INTO products (id, owner_id, erp_goods_id, product_code, product_name, specification, storage_condition, special_drug_category, status) VALUES ($1, $2, 1001, 'P-OUT-001', '出库复核策略商品', '1 unit', 'normal_10_30', 'none', 'active')",
    )
    .bind(Uuid::new_v4())
    .bind(owner_id)
    .execute(&pool)
    ...
    let locked: (i64, i64, i64) = sqlx::query_as(
        r#"
        SELECT
            (SELECT qty_frozen::BIGINT FROM inventory_batches
    ...
    sqlx::query(
        "UPDATE products SET special_drug_category = 'narcotic', storage_condition = 'cold_2_8' WHERE owner_id = $1 AND product_code = 'P-OUT-001'",
    )
    .bind(owner_id)
    .execute(&pool)
    .await
