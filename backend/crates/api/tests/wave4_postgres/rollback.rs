...
async fn outbound_ship_rolls_back_all_side_effects(pool: PgPool) {
    ...
    sqlx::query(
        "INSERT INTO products (id, owner_id, product_code, product_name, specification, storage_condition, special_drug_category, status) VALUES ($1, $2, $3, '发运回滚商品', '1 unit', 'normal', 'none', 'active')",
    )
    .bind(Uuid::new_v4())
    .bind(owner_id)
    .bind(product_code)
