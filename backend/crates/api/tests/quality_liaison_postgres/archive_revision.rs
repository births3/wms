...
async fn approved_archive_revision_publishes_and_completed_callback_unlocks_asn(pool: PgPool) {
    ...
    sqlx::query(
        "INSERT INTO products (id, owner_id, product_code, product_name, specification, storage_condition, approval_no, status) VALUES ($1, $2, 'P-ARCHIVE-001', '档案补录商品', '1 unit', 'normal_10_30', 'OLD-001', 'active')",
    )
    .bind(product_id)
    .bind(owner_id)
    .execute(&pool)
