...
async fn upsert_task_priority_rule_applies_configured_factors_sort_waiting_and_audit(pool: PgPool) {
    ...
    let (normal_product_id, normal_product_code) =
        seed_product(&pool, owner_id, "normal_10_30").await;
    let (_cold_product_id, cold_product_code) = seed_product(&pool, owner_id, "cold_2_8").await;
