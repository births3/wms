...
use wms_api::{
    ...
    wechat_notify_service::{
        H4RecordQuery, PgWechatNotifyService, WechatProvider, WechatProviderError,
        WechatProviderFuture, WechatProviderRequest,
    },
};
use wms_domain::{
...
struct ScriptedWechatProvider {
    calls: Arc<Mutex<Vec<WechatProviderRequest>>>,
}

impl WechatProvider for ScriptedWechatProvider {
    fn send<'a>(&'a self, request: WechatProviderRequest) -> WechatProviderFuture<'a> {
        let calls = Arc::clone(&self.calls);
        Box::pin(async move {
            let retryable = request.recipient == "retry";
            calls
                .lock()
...
#[sqlx::test(migrations = "../../migrations")]
async fn h4_provider_boundary_persists_success_retry_and_final_idempotent_response(pool: PgPool) {
    let service = PgWechatNotifyService::new();
    let ctx = ctx();
    let now = Utc::now();
    service
        .upsert_wechat_settings(
            &pool,
            &ctx,
            UpsertH4WechatSettingsRequest {
                corp_id: "ww-corp-provider".to_string(),
    ...
    service
        .upsert_config(
            ...
            UpsertH4NotificationConfigRequest {
                ...
                enabled: true,
                template: "订单 {{order_no}} 已处理".to_string(),
                recipient_rule: serde_json::json!({ "users": ["manager", "retry"] }),
                channels: vec!["wechat".to_string()],
            },
            now,
            "h4-provider-config",
    ...
    let provider = ScriptedWechatProvider::default();
    let request = SendH4NotificationRequest {
        event_type: "provider_probe".to_string(),
        dedupe_key: "provider-probe-1".to_string(),
        recipients: vec!["manager".to_string(), "retry".to_string()],
        payload: serde_json::json!({ "order_no": "ASN-001" }),
    };
    let sent = service
        ...
        .await
        .expect("controlled provider should complete the send boundary");
    assert_eq!(sent.value[0].status, "success");
    assert_eq!(sent.value[1].status, "retrying");
    assert_eq!(sent.value[1].retry_count, 0);
    assert_eq!(
        sent.value[1].failure_reason.as_deref(),
        Some("企微返回临时错误")
    );
    {
        let calls = provider
        ...
        assert_eq!(calls[0].corp_id.as_deref(), Some("ww-corp-provider"));
        assert_eq!(
            calls[0].secret_alias.as_deref(),
            Some("h4/wechat/agent_secret")
        );
    }

    let replay = service
        .send_notification_with_provider(&pool, &ctx, request, now, "h4-provider-send", &provider)
        .await
        .expect("same idempotency key should replay final provider response");
    assert!(replay.replayed);
    assert_eq!(replay.value[0].status, "success");
    assert_eq!(replay.value[1].status, "retrying");
    assert_eq!(
        provider
            .calls
            .lock()
...
async fn h4_record_query_limits_read_only_users_to_their_notifications(pool: PgPool) {
    ...
    .fetch_one(&pool)
    .await
    .expect("scope system admin role should be seeded");
