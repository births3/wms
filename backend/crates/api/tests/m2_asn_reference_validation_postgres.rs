...
async fn seed_references(
    ...
    let product_id: Uuid = sqlx::query_scalar(
        "INSERT INTO products (id, owner_id, product_code, product_name, specification, storage_condition, status) VALUES ($1, $2, $3, 'Reference Product', '1 unit', 'normal', 'active') RETURNING id",
    )
    .bind(Uuid::new_v4())
    .bind(owner_id)
...
async fn create_asn_rejects_inactive_supplier_and_cross_owner_product(pool: PgPool) {
    ...
    sqlx::query(
        "INSERT INTO products (id, owner_id, product_code, product_name, specification, storage_condition, status) VALUES ($1, $2, $3, 'Foreign Product', '1 unit', 'normal', 'active')",
    )
