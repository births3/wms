...
async fn seed_location(
    ...
    sqlx::query(
        "INSERT INTO warehouse_zones (id, owner_id, warehouse_id, zone_code, zone_name, temperature_zone, quality_color) VALUES ($1, $2, $3, $4, 'query test zone', 'normal', 'qualified_green')",
    )
    .bind(zone_id)
    .bind(owner_id)
    .bind(warehouse_id)
...
async fn seed_query_batch(
    ...
    let now = Utc::now();
    sqlx::query(
        "INSERT INTO inventory_batches (id, owner_id, product_code, batch_no, production_date, expiry_date, qty_on_hand, quality_status, location_id, location_code, created_at, updated_at) VALUES ($1, $2, 'P-QUERY-LOCATION', $3, '2026-01-01', '2027-01-01', 10, $4, $5, $6, $7, $7)",
    )
    .bind(id)
    .bind(owner_id)
    .bind(batch_no)
...
async fn expired_batches_are_isolated_idempotently_and_owner_scoped(pool: PgPool) {
    ...
    assert_eq!(first.value[0].quality_status, STATUS_UNQUALIFIED);
    ...
    assert_eq!(
        replay.value[0].quality_status,
        first.value[0].quality_status
    );
    ...
    let rows = repo
        .list_inventory_batches_with_query(
            ...
            wms_domain::InventoryBatchQuery {
                ...
                quality_status: Some(STATUS_QUALIFIED.to_string()),
