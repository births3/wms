...
use wms_domain::{
    ...
    CreateBillingContractRequest, CreateBillingRuleRequest, CreateReceivingOrderRequest,
    InventoryBatch, InventoryMovement, PutawayRecord, PutawayRequest, ReceiveReceivingOrderRequest,
    ReceivingOrder, ReceivingOrderLine, ReceivingOrderReceipt,
};

use crate::{auth::AuthContext, inventory::STATUS_QUALIFIED};
...
async fn replay_idempotency<T: DeserializeOwned>(
    ...
    .await
    .map_err(map_db_error)?;
    let Some((stored_hash, response_body)) = row else {
        return Ok(None);
    };
