...
pub(super) async fn acquire_item_locks(
    ...
    let batches: Vec<(Uuid, String, String)> = sqlx::query_as(
        "SELECT batch.id,
                CASE
                    WHEN batch.status = 'qualified' THEN batch.status
        ...
        if current_status == "qualified" {
            sqlx::query(
                "UPDATE inventory_batches
                    SET status = 'quarantined', updated_at = $3, version = version + 1
                  WHERE owner_id = $1 AND id = $2 AND status = 'qualified'",
            )
            .bind(owner_id)
            .bind(batch_id)
            .bind(now)
        ...
        if !has_other_active_lock {
            let result = sqlx::query(
                "UPDATE inventory_batches
                  ...
                  WHERE owner_id = $1 AND id = $2 AND status = 'quarantined'",
            )
            .bind(owner_id)
            .bind(batch_id)
            .bind(previous_status)
