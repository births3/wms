    ...
    for row_no in req.row_start..=req.row_end {
        for column_no in req.column_start..=req.column_end {
            for layer_no in req.layer_start..=req.layer_end {
                let current_owner_id = req.current_owner_id;
                locations.push(Location {
                    ...
                    current_owner_id,
                    allows_container: req.allows_container.unwrap_or(true),
                    mix_product_policy: req.mix_product_policy.clone().unwrap_or_else(|| "single_product_only".into()),
                    mix_batch_policy: req.mix_batch_policy.clone().unwrap_or_else(|| "single_batch".into()),
                    lock_status: req.lock_status.clone().unwrap_or_else(|| "normal".into()),
                    pick_zone_level: req.pick_zone_level.clone(),
                    pick_sequence_no: req.pick_sequence_no,
                    putaway_sequence_no: req.putaway_sequence_no,
                    is_agv_managed: req.is_agv_managed.unwrap_or(false),
                    agv_pod_code: req.agv_pod_code.clone(),
                    agv_unreachable_at: None,
                    replenish_strategy_id: req.replenish_strategy_id,
...
fn location_wizard_drafts(
    owner_id: Uuid,
    req: &BatchGenerateLocationsRequest,
    now: DateTime<Utc>,
) -> Result<Vec<Location>, MasterDataError> {
    let is_agv = req.rule_type.eq_ignore_ascii_case("agv")
        || req.rule_type.eq_ignore_ascii_case("agv_pod")
        || req.is_agv_managed == Some(true)
        || req.pod_start.is_some()
        || req.pod_end.is_some();

    if is_agv {
        let pod_prefix = req.pod_prefix.as_deref().unwrap_or("POD").trim();
        let pod_start = req.pod_start.or(req.row_start).unwrap_or(1);
        let pod_end = req.pod_end.or(req.row_end).unwrap_or(1);
        let layer_start = req.layer_start.unwrap_or(1);
        let layer_end = req.layer_end.unwrap_or(1);
        let grid_start = req.grid_start.or(req.column_start).unwrap_or(1);
        let grid_end = req.grid_end.or(req.column_end).unwrap_or(1);
        ...
        let total_count = (pod_end - pod_start + 1)
        ...
        let location_type = req.location_type.as_deref().unwrap_or("storage");
        let allows_container = req.allows_container.unwrap_or(location_type == "storage" || location_type == "staging");
        let mix_product_policy = req.mix_product_policy.clone().unwrap_or_else(|| "single_product_only".into());
        let mix_batch_policy = req.mix_batch_policy.clone().unwrap_or_else(|| "single_batch".into());
        let lock_status = req.lock_status.clone().unwrap_or_else(|| "normal".into());
        let max_volume_cm3 = if req.max_volume_cm3 > 0 { req.max_volume_cm3 } else { 100000 };
        let max_sku_count = if req.max_sku_count > 0 { req.max_sku_count } else { 3 };
        let current_owner_id = req.current_owner_id;

        let init_pick = req.initial_pick_sequence.unwrap_or(1);
        let step_pick = req.pick_sequence_step.unwrap_or(1);
        let init_putaway = req.initial_putaway_sequence.unwrap_or(1);
        let step_putaway = req.putaway_sequence_step.unwrap_or(1);

        let mut locations = Vec::with_capacity(total_count as usize);
        let mut seq_idx = 0i32;

        // 与向导预览一致：排(货架) → 列(格) → 层，保证 pick/putaway_sequence_no 与预览相同。
        for pod_no in pod_start..=pod_end {
            let agv_pod_code = format!("{pod_prefix}{pod_no:02}");
            for grid_no in grid_start..=grid_end {
                for layer_no in layer_start..=layer_end {
                    let location_code = format!("{agv_pod_code}-F{layer_no}-{grid_no:02}");
                    let pick_seq = init_pick + seq_idx * step_pick;
                    let putaway_seq = init_putaway + seq_idx * step_putaway;
                    seq_idx += 1;
                    ...
                    locations.push(Location {
                        ...
                        current_owner_id,
                        allows_container,
                        mix_product_policy: mix_product_policy.clone(),
                        mix_batch_policy: mix_batch_policy.clone(),
                        lock_status: lock_status.clone(),
                        pick_zone_level: req.pick_zone_level.clone().or_else(|| Some("normal".to_string())),
                        pick_sequence_no: Some(pick_seq),
                        putaway_sequence_no: Some(putaway_seq),
                        is_agv_managed: true,
                        agv_pod_code: Some(agv_pod_code.clone()),
                        agv_unreachable_at: None,
                        replenish_strategy_id: req.replenish_strategy_id,
                        status: "available".to_string(),
                        created_at: now,
        ...
        let prefix = req.prefix.as_deref().unwrap_or("LOC").trim();
        let aisle_start = req.aisle_start.unwrap_or(1);
        let aisle_end = req.aisle_end.unwrap_or(1);
        let row_start = req.row_start.unwrap_or(1);
        let row_end = req.row_end.unwrap_or(1);
        let column_start = req.column_start.unwrap_or(1);
        let column_end = req.column_end.unwrap_or(1);
        let layer_start = req.layer_start.unwrap_or(1);
        let layer_end = req.layer_end.unwrap_or(1);
        let has_grid = req.grid_start.is_some() || req.grid_end.is_some();
        let grid_start = req.grid_start.unwrap_or(1);
        let grid_end = req.grid_end.unwrap_or(1);
        let has_aisle = req.aisle_start.is_some() || req.aisle_end.is_some();

        if aisle_start < 1
            || aisle_start > aisle_end
            || aisle_end > 99
            || row_start < 1
            || row_start > row_end
            || row_end > 99
            ...
            || layer_start > layer_end
            || layer_end > 99
            || (has_grid && (grid_start < 1 || grid_start > grid_end || grid_end > 99))
            || req.max_volume_cm3 < 0
            || req.max_sku_count <= 0
            || prefix.is_empty()
        {
        ...
        let total_count = (aisle_end - aisle_start + 1)
            * (row_end - row_start + 1)
            * (column_end - column_start + 1)
            * (layer_end - layer_start + 1)
            * (if has_grid { grid_end - grid_start + 1 } else { 1 });
        ...
        for aisle_no in aisle_start..=aisle_end {
            for row_no in row_start..=row_end {
                for column_no in column_start..=column_end {
                    for layer_no in layer_start..=layer_end {
                        if has_grid {
                            for grid_no in grid_start..=grid_end {
                                let location_code = if has_aisle {
                                    format!("{prefix}-{aisle_no:02}-{row_no:02}-{layer_no:02}-{grid_no:02}")
                                } else {
                                    format!("{prefix}-{row_no:02}-{column_no:02}-{layer_no:02}-{grid_no:02}")
                                };
                                let pick_seq = init_pick + seq_idx * step_pick;
                                let putaway_seq = init_putaway + seq_idx * step_putaway;
                                seq_idx += 1;
                            ...
                            let location_code = if has_aisle {
                                format!("{prefix}-{aisle_no:02}-{row_no:02}-{column_no:02}-{layer_no:02}")
                            } else {
                                format!("{prefix}-{row_no:02}-{column_no:02}-{layer_no:02}")
                            };
                            let pick_seq = init_pick + seq_idx * step_pick;
                            let putaway_seq = init_putaway + seq_idx * step_putaway;
                            seq_idx += 1;
...
async fn load_master_data_before(
    ...
    let sql = match resource_type {
        ...
        "location" => "SELECT to_jsonb(t) FROM (SELECT id, owner_id, warehouse_id, zone_id, location_code, row_no, column_no, layer_no, max_volume_cm3, used_volume_cm3, max_sku_count, location_type, current_owner_id, allows_container, mix_product_policy, mix_batch_policy, lock_status, pick_zone_level, pick_sequence_no, putaway_sequence_no, is_agv_managed, agv_pod_code, agv_unreachable_at, replenish_strategy_id, status, created_at, updated_at FROM warehouse_locations WHERE owner_id=$1 AND id=$2) t",
        "warehouse_zone" => "SELECT to_jsonb(t) FROM (SELECT id, owner_id, warehouse_id, zone_code, zone_name, temperature_zone, quality_color, allowed_categories, is_external_use_zone, is_fragrant_zone, is_special_drug_zone, status, created_at, updated_at FROM warehouse_zones WHERE owner_id=$1 AND id=$2) t",
...
impl From<ProductRow> for Product {
    fn from(row: ProductRow) -> Self {
        ...
        Self {
            ...
            is_external_use: row.is_external_use.unwrap_or(false),
            is_fragrant: row.is_fragrant.unwrap_or(false),
            status: row.status,
            attrs,
...
impl From<LocationRow> for Location {
    fn from(row: LocationRow) -> Self {
        Self {
            ...
            allows_container: row.allows_container.unwrap_or(true),
            mix_product_policy: row.mix_product_policy.unwrap_or_else(|| "single_product_only".into()),
            mix_batch_policy: row.mix_batch_policy.unwrap_or_else(|| "single_batch".into()),
            lock_status: row.lock_status.unwrap_or_else(|| "normal".into()),
            pick_zone_level: row.pick_zone_level,
            pick_sequence_no: row.pick_sequence_no,
            putaway_sequence_no: row.putaway_sequence_no,
            is_agv_managed: row.is_agv_managed.unwrap_or(false),
            agv_pod_code: row.agv_pod_code,
            agv_unreachable_at: row.agv_unreachable_at,
            replenish_strategy_id: row.replenish_strategy_id,
            status: row.status,
            created_at: row.created_at,
            updated_at: row.updated_at,
        }
