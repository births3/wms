impl PgWave3Repository {
    ...
    pub async fn list_receiving_orders(
        ...
        attach_latest_receipts(&self.pool, ctx.owner_id, &mut orders).await?;
        Ok((orders, total))
    }
...
async fn attach_latest_receipts(
    pool: &sqlx::PgPool,
    owner_id: Uuid,
    orders: &mut [ReceivingOrder],
) -> Result<(), Wave3RepositoryError> {
    if orders.is_empty() {
        return Ok(());
    }
    let ids: Vec<Uuid> = orders.iter().map(|order| order.id).collect();
    let rows = sqlx::query_as::<_, ReceivingOrderReceiptRow>(
        r#"
        SELECT DISTINCT ON (receiving_order_id)
               id, receiving_order_id, owner_id, actual_qty, shortage_qty,
               rejected_qty, arrival_temperature_celsius, exception_note, receiving_details, occurred_at
          FROM receiving_order_receipts
         WHERE owner_id = $1 AND receiving_order_id = ANY($2)
         ORDER BY receiving_order_id, occurred_at DESC, id DESC
        "#,
    )
    .bind(owner_id)
    .bind(&ids)
    ...
    for row in rows {
        receipts.insert(
            row.receiving_order_id,
            wms_domain::mask_receiving_receipt_contacts(map_receiving_order_receipt(row)),
        );
    }
    for order in orders {
        order.receipt = receipts.remove(&order.id);
    }
    Ok(())
}
