...
impl PgPrintOrchestrationRepository {
    ...
    pub(super) async fn manual_cutoff(
        ...
        insert_group(&mut tx, ctx, &group, now).await?;
    ...
    pub(super) async fn scheduled_cutoff(
        ...
        now: DateTime<Utc>,
    ) -> Result<Option<DeliveryNoteGroup>, PrintOrchestrationError> {
        ...
        if let Some(group) =
            load_scheduled_group(&mut tx, ctx.owner_id, plan.id, boundary, scheduled_at).await?
        {
            return Ok(Some(group));
        }
        let order_ids =
        ...
        .await?;
        append_cutoff_audit(&mut tx, ctx, "scheduled_cutoff_delivery_note", &group, now).await?;
        tx.commit().await.map_err(map_db_error)?;
        Ok(Some(group))
    }
}
...
async fn load_scheduled_group(
    ...
    let row = sqlx::query_as::<_, DeliveryNoteGroupRow>(
        r#"
        SELECT id, owner_id, warehouse_id, customer_id, delivery_address_id,
         ...
         WHERE owner_id = $1
           ...
           AND route_code = $5
           AND cutoff_plan_id = $6
           AND scheduled_cutoff_at = $7
        "#,
    )
    .bind(owner_id)
    .bind(boundary.warehouse_id)
    ...
    let order_ids = sqlx::query_scalar::<_, Uuid>(
        ...
        SELECT outbound_order_id
         ...
         ORDER BY created_at, outbound_order_id
        "#,
    )
    .bind(owner_id)
    .bind(row.id)
