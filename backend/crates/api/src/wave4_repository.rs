...
use wms_domain::{
    ...
    CreateOutboundOrderRequest, CreateOutboundWaveRequest, CreatePurchaseReturnRequest, DriverTask,
    DriverTaskListResponse, InventoryBatch, OutboundColdChainPackage, OutboundOrder,
    OutboundOrderLine, OutboundShipment, OutboundWave, PageMeta, PurchaseReturnOrder,
    RejectPurchaseReturnRequest, ReviewOutboundOrderRequest, ReviewValidationError,
    ShipOutboundOrderRequest, ShipOutboundValidationError, StoreDashboardResponse,
...
fn map_inventory_batch(row: InventoryBatchRow) -> InventoryBatch {
    InventoryBatch {
        ...
        qty_frozen: row.qty_frozen,
        qty_allocated: Some(row.qty_allocated),
        qty_replenish_in_transit: Some(row.qty_replenish_in_transit),
        qty_replenish_out_transit: Some(row.qty_replenish_out_transit),
        status: row.status,
        warehouse_id: None,
        zone_id: None,
