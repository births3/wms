...
async fn seed_active_supplier_and_product(
    ...
    sqlx::query(
        "INSERT INTO products (id, owner_id, product_code, product_name, specification, storage_condition, volume_cm3, status) VALUES ($1, $2, $3, 'Active Product', '1 unit', 'normal_10_30', 1, 'active')",
    )
    .bind(Uuid::new_v4())
    .bind(owner_id)
    .bind(product_code)
...
async fn postgres_putaway_handler_commits_inventory_and_audit(pool: PgPool) {
    ...
    sqlx::query("INSERT INTO warehouse_zones (id, owner_id, warehouse_id, zone_code, zone_name, temperature_zone, quality_color, status) VALUES ($1,$2,$3,'M2-HANDLER-ZONE','Z','normal_10_30','qualified_green','active')")
            .bind(zone_id).bind(owner_id).bind(warehouse_id).execute(&pool).await.expect("seed zone");
    sqlx::query("INSERT INTO warehouse_locations (id, owner_id, warehouse_id, zone_id, location_code, row_no, column_no, layer_no, max_volume_cm3, used_volume_cm3, max_sku_count, location_type, allows_container, status) VALUES ($1,$2,$3,$4,$5,1,1,1,100000,0,3,'storage',TRUE,'available')")
            .bind(location_id).bind(owner_id).bind(warehouse_id).bind(zone_id).bind(&location_code)
            .execute(&pool).await.expect("seed location");
    sqlx::query(
        "INSERT INTO lpn_containers (id, owner_id, lpn_code, container_type, status, current_lock_category, created_at, updated_at) VALUES ($1,$2,'LPN-HANDLER-PUTAWAY','pallet','in_use','qualified',$3,$3)",
    )
    .bind(Uuid::new_v4())
    .bind(owner_id)
    .bind(now)
    ...
    let Json(putaway) = putaway_receiving_order_handler(
        ...
        Json(PutawayRequest {
            ...
            quality_status: crate::inventory::STATUS_QUALIFIED.to_string(),
                    lpn_code: Some("LPN-HANDLER-PUTAWAY".to_string()),
                    witness_id: None,
        }),
    )
    .await
