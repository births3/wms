...
impl PgMasterDataReadRepository {
    ...
    pub async fn create_location(
        ...
        ensure_warehouse_zone_in_owner(&mut tx, ctx.owner_id, req.warehouse_id, req.zone_id)
            .await?;
        let target_owner = req.current_owner_id;
        ensure_bound_owner_exists(&mut tx, target_owner).await?;
        ensure_enabled_dictionary_item(
            &mut tx,
            ctx.owner_id,
            LOCATION_TYPE_DICT,
        ...
        let row = sqlx::query_as::<_, LocationRow>(
            r#"INSERT INTO warehouse_locations (
                ...
                current_owner_id, allows_container, mix_product_policy, mix_batch_policy,
                lock_status, pick_zone_level, pick_sequence_no, putaway_sequence_no,
                is_agv_managed, agv_pod_code, replenish_strategy_id, status, created_at, updated_at
            ) VALUES (
                $1,$2,$3,$4,$5,$6,$7,$8,$9,0,$10,$11,$12,
                COALESCE($13, true),
                COALESCE($14, 'single_product_only'),
                COALESCE($15, 'single_batch'),
                COALESCE($16, 'normal'),
                $17, $18, $19,
                COALESCE($20, false),
                $21, $22,
                'available', $23, $23
                      ...
                      current_owner_id, allows_container, mix_product_policy, mix_batch_policy,
                      lock_status, pick_zone_level, pick_sequence_no, putaway_sequence_no,
                      is_agv_managed, agv_pod_code, agv_unreachable_at, replenish_strategy_id,
                      status, created_at, updated_at"#,
        ).bind(Uuid::new_v4()).bind(ctx.owner_id).bind(req.warehouse_id).bind(req.zone_id).bind(&req.location_code)
        .bind(req.row_no).bind(req.column_no).bind(req.layer_no).bind(req.max_volume_cm3).bind(req.max_sku_count)
        .bind(&req.location_type).bind(target_owner).bind(req.allows_container).bind(req.mix_product_policy)
        .bind(req.mix_batch_policy).bind(req.lock_status).bind(req.pick_zone_level).bind(req.pick_sequence_no)
        .bind(req.putaway_sequence_no).bind(req.is_agv_managed).bind(req.agv_pod_code).bind(req.replenish_strategy_id).bind(now)
    ...
    pub async fn update_location(
        ...
        let row = sqlx::query_as::<_, LocationRow>(
            r#"UPDATE warehouse_locations SET
                ...
                layer_no=COALESCE($7,layer_no),
                max_volume_cm3=COALESCE($8,max_volume_cm3),
                used_volume_cm3=COALESCE($9,used_volume_cm3),
                max_sku_count=COALESCE($10,max_sku_count),
                location_type=COALESCE($11,location_type),
                current_owner_id=COALESCE($12,current_owner_id),
                allows_container=COALESCE($13,allows_container),
                mix_product_policy=COALESCE($14,mix_product_policy),
                mix_batch_policy=COALESCE($15,mix_batch_policy),
                lock_status=COALESCE($16,lock_status),
                pick_zone_level=COALESCE($17,pick_zone_level),
                pick_sequence_no=COALESCE($18,pick_sequence_no),
                putaway_sequence_no=COALESCE($19,putaway_sequence_no),
                is_agv_managed=COALESCE($20,is_agv_managed),
                agv_pod_code=COALESCE($21,agv_pod_code),
                replenish_strategy_id=COALESCE($22,replenish_strategy_id),
            ...
            RETURNING id, owner_id, warehouse_id, zone_id, location_code, row_no, column_no,
                      ...
                      is_agv_managed, agv_pod_code, agv_unreachable_at, replenish_strategy_id,
                      status, created_at, updated_at"#,
        ).bind(ctx.owner_id).bind(id).bind(req.zone_id).bind(req.location_code).bind(req.row_no).bind(req.column_no).bind(req.layer_no).bind(req.max_volume_cm3).bind(req.used_volume_cm3).bind(req.max_sku_count).bind(req.location_type).bind(target_owner).bind(req.allows_container).bind(req.mix_product_policy).bind(req.mix_batch_policy).bind(req.lock_status).bind(req.pick_zone_level).bind(req.pick_sequence_no).bind(req.putaway_sequence_no).bind(req.is_agv_managed).bind(req.agv_pod_code).bind(req.replenish_strategy_id).bind(req.status).bind(now)
        .fetch_optional(&mut *tx).await.map_err(map_db_error)?.ok_or(MasterDataError::NotFound)?;
        let value = Location::from(row);
        append_master_data_update_audit(
            &mut tx,
    ...
    pub async fn list_warehouse_zones(
        ...
        let rows = sqlx::query_as::<_, WarehouseZoneRow>(
            "SELECT id, owner_id, warehouse_id, zone_code, zone_name, temperature_zone, quality_color, allowed_categories, is_external_use_zone, is_fragrant_zone, is_special_drug_zone, status, created_at, updated_at FROM warehouse_zones WHERE owner_id = $1 ORDER BY updated_at DESC, zone_code, id LIMIT $2 OFFSET $3",
    ...
    pub async fn create_warehouse_zone(
        ...
        let row = sqlx::query_as::<_, WarehouseZoneRow>(
            r#"INSERT INTO warehouse_zones (
                ...
                allowed_categories, is_external_use_zone, is_fragrant_zone, is_special_drug_zone,
                status, created_at, updated_at
            ) VALUES (
                $1,$2,$3,$4,$5,$6,$7,
                COALESCE($8, '[]'::jsonb),
                ...
                COALESCE($11, false),
                'active',$12,$12
            ) RETURNING id, owner_id, warehouse_id, zone_code, zone_name, temperature_zone, quality_color,
                      allowed_categories, is_external_use_zone, is_fragrant_zone, is_special_drug_zone,
                      status, created_at, updated_at"#,
        ).bind(Uuid::new_v4()).bind(ctx.owner_id).bind(req.warehouse_id).bind(&req.zone_code).bind(&req.zone_name).bind(&req.temperature_zone).bind(&req.quality_color).bind(req.allowed_categories).bind(req.is_external_use_zone).bind(req.is_fragrant_zone).bind(req.is_special_drug_zone).bind(now)
    ...
    pub async fn update_warehouse_zone(
        ...
        let row = sqlx::query_as::<_, WarehouseZoneRow>(
            r#"UPDATE warehouse_zones SET
                zone_name=COALESCE($3,zone_name),
                temperature_zone=COALESCE($4,temperature_zone),
                quality_color=COALESCE($5,quality_color),
                allowed_categories=COALESCE($6,allowed_categories),
                is_external_use_zone=COALESCE($7,is_external_use_zone),
                is_fragrant_zone=COALESCE($8,is_fragrant_zone),
                is_special_drug_zone=COALESCE($9,is_special_drug_zone),
            ...
            RETURNING id, owner_id, warehouse_id, zone_code, zone_name, temperature_zone, quality_color,
                      allowed_categories, is_external_use_zone, is_fragrant_zone, is_special_drug_zone,
                      status, created_at, updated_at"#,
        ).bind(ctx.owner_id).bind(id).bind(req.zone_name).bind(req.temperature_zone).bind(req.quality_color).bind(req.allowed_categories).bind(req.is_external_use_zone).bind(req.is_fragrant_zone).bind(req.is_special_drug_zone).bind(req.status).bind(now)
        .fetch_optional(&mut *tx).await.map_err(map_db_error)?.ok_or(MasterDataError::NotFound)?;
        let value = WarehouseZone::from(row);
        append_master_data_update_audit(
            &mut tx,
...
impl From<WarehouseZoneRow> for WarehouseZone {
    fn from(row: WarehouseZoneRow) -> Self {
        Self {
            ...
            allowed_categories: row.allowed_categories.unwrap_or_else(|| serde_json::json!([])),
            is_external_use_zone: row.is_external_use_zone.unwrap_or(false),
            is_fragrant_zone: row.is_fragrant_zone.unwrap_or(false),
            is_special_drug_zone: row.is_special_drug_zone.unwrap_or(false),
            status: row.status,
