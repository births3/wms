...
pub fn master_data_router(state: MasterDataAppState) -> Router {
    Router::new()
        ...
        .route(
            ...
            post(batch_generate_locations_handler),
        )
        .route(
        ...
        state
            .update_warehouse_zone(
                ...
                UpdateWarehouseZoneRequest {
                    ...
                    is_external_use_zone: None,
                    is_fragrant_zone: None,
                    is_special_drug_zone: None,
                    status: Some("disabled".into()),
                },
                chrono::Utc::now(),
                &idempotency_key,
...
async fn batch_generate_locations_handler(
    ctx: AuthContext,
    State(state): State<MasterDataAppState>,
    headers: HeaderMap,
    Json(req): Json<BatchGenerateLocationsRequest>,
) -> Result<Json<BatchGenerateLocationsResponse>, MasterDataHandlerError> {
    ctx.require_permission("m1.location.batch-generate")?;
    let idempotency_key = idempotency_key_from_headers(&headers)?;
    Ok(Json(
        state
            .batch_generate_locations(&ctx, req, chrono::Utc::now(), &idempotency_key)
            .await?,
    ))
}
        ...
        state
            .update_location(
                ...
                UpdateLocationRequest {
                    ...
                    allows_container: None,
                    mix_product_policy: None,
                    mix_batch_policy: None,
                    lock_status: None,
                    pick_zone_level: None,
                    pick_sequence_no: None,
                    putaway_sequence_no: None,
                    is_agv_managed: None,
                    agv_pod_code: None,
                    replenish_strategy_id: None,
                    status: Some("disabled".into()),
                },
                chrono::Utc::now(),
                &idempotency_key,
