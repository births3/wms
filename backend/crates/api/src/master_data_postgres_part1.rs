...
struct ProductRow {
    ...
    attrs: Value,
    #[sqlx(default)]
    is_external_use: Option<bool>,
    #[sqlx(default)]
    is_fragrant: Option<bool>,
    status: String,
...
struct WarehouseZoneRow {
    ...
    allowed_categories: Option<Value>,
    is_external_use_zone: Option<bool>,
    is_fragrant_zone: Option<bool>,
    is_special_drug_zone: Option<bool>,
    status: String,
    created_at: DateTime<Utc>,
    updated_at: DateTime<Utc>,
}
...
struct LocationRow {
    ...
    allows_container: Option<bool>,
    mix_product_policy: Option<String>,
    mix_batch_policy: Option<String>,
    lock_status: Option<String>,
    pick_zone_level: Option<String>,
    pick_sequence_no: Option<i32>,
    putaway_sequence_no: Option<i32>,
    is_agv_managed: Option<bool>,
    agv_pod_code: Option<String>,
    agv_unreachable_at: Option<DateTime<Utc>>,
    replenish_strategy_id: Option<Uuid>,
    status: String,
    created_at: DateTime<Utc>,
    updated_at: DateTime<Utc>,
}
...
impl PgMasterDataReadRepository {
    ...
    pub async fn list_locations(
        ...
        let rows = sqlx::query_as::<_, LocationRow>(
            ...
            SELECT id, owner_id, warehouse_id, zone_id, location_code, row_no, column_no,
                   ...
                   location_type, current_owner_id, allows_container, mix_product_policy,
                   mix_batch_policy, lock_status, pick_zone_level, pick_sequence_no,
                   putaway_sequence_no, is_agv_managed, agv_pod_code, agv_unreachable_at,
                   replenish_strategy_id, status, created_at, updated_at
    ...
    pub async fn batch_generate_locations(
        &self,
        ctx: &AuthContext,
        req: BatchGenerateLocationsRequest,
        now: DateTime<Utc>,
        idempotency_key: &str,
    ) -> Result<BatchGenerateLocationsResponse, MasterDataError> {
        let request_hash = request_hash(&json!({
            "path": "/api/v1/master-data/locations/batch-generate",
            "request": &req,
        }))?;
        let mut tx = self.pool.begin().await.map_err(map_db_error)?;
        lock_idempotency_key(&mut tx, ctx.owner_id, idempotency_key).await?;
        if let Some(value) =
            replay_idempotency(
                ...
                &request_hash,
                "POST",
                "/api/v1/master-data/locations/batch-generate",
                now,
            )
            .await?
        {
        ...
        ensure_warehouse_zone_in_owner(&mut tx, ctx.owner_id, req.warehouse_id, req.zone_id)
            .await?;
        ensure_bound_owner_exists(&mut tx, target_owner).await?;
        let loc_type = req.location_type.as_deref().unwrap_or("storage");
        ensure_enabled_dictionary_item(
            &mut tx,
            ctx.owner_id,
            LOCATION_TYPE_DICT,
        ...
        .await?;
        let drafts = location_wizard_drafts(ctx.owner_id, &req, now)?;
        let location_codes = drafts
            .iter()
            .map(|location| location.location_code.clone())
            .collect::<Vec<_>>();
        ...
        for draft in drafts {
            ...
            .bind(&draft.location_type)
            .bind(draft.current_owner_id)
            .bind(draft.allows_container)
            .bind(&draft.mix_product_policy)
            .bind(&draft.mix_batch_policy)
            .bind(&draft.lock_status)
            .bind(&draft.pick_zone_level)
            .bind(draft.pick_sequence_no)
            .bind(draft.putaway_sequence_no)
            .bind(draft.is_agv_managed)
            .bind(&draft.agv_pod_code)
            .bind(draft.replenish_strategy_id)
            .bind(now)
            .fetch_one(&mut *tx)
            .await
            .map_err(map_location_write_error)?;
        ...
        let response = BatchGenerateLocationsResponse {
            total_generated: locations.len() as u32,
            locations,
        };
        store_idempotency_success(
            &mut tx,
