...
use sqlx::FromRow;
use uuid::Uuid;
use wms_domain::{InboundDocumentEntry, InboundDocumentEntryListResponse, PageMeta};
...
impl PgDrugInspectionDocumentRepository {
    pub async fn list_inbound_documents(
    ...
    ) -> Result<InboundDocumentEntryListResponse, DrugInspectionDocumentRepositoryError> {
        let rows = sqlx::query_as::<_, InboundDocumentRow>(
            r#"
            WITH source AS (
        ...
        let count = u32::try_from(rows.len()).map_err(|_| {
            DrugInspectionDocumentRepositoryError::Serialize("入库资料列表过大".to_string())
        })?;
        Ok(InboundDocumentEntryListResponse {
