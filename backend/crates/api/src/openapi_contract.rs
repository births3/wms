...
const AUDIT_EXEMPTION_GROUPS: &[AuditExemptionGroup] = &[
    AuditExemptionGroup {
    ...
    AuditExemptionGroup {
        operations: &[(
            "/api/v1/inbound/putaway/validate-location",
            PathItemType::Post,
        )],
        reason: "上架 6 维探测端点不修改业务数据，拦截记录自动写入专有拦截审计表。",
    },
];
...
const IDEMPOTENCY_EXEMPTION_GROUPS: &[IdempotencyExemptionGroup] = &[
    ...
    IdempotencyExemptionGroup {
        operations: &[("/api/v1/inbound/putaway/validate-location", PathItemType::Post)],
        reason: "上架 6 维前置探测只执行合规校验与拦截留痕，不修改库存；使用 POST 承载探测参数。",
    },
    IdempotencyExemptionGroup {
