...
impl PgWave4Repository {
    pub async fn batch_complete_pick_tasks(
        ...
        idempotency_key: &str,
        audit: Option<AuditWriteRequest>,
    ) -> Result<IdempotentMutation<wms_domain::BatchCompletePickTaskResponse>, Wave4RepositoryError> {
        ...
        if !matches!(
            order.status.as_str(),
            OUTBOUND_STATUS_IN_WAVE | "picking" | "picked" | "picked_short" | OUTBOUND_STATUS_REVIEWED_SHORT
        ) {
            return Err(Wave4RepositoryError::InvalidStatus {
                expected: "in_wave|picking|picked|picked_short|reviewed_short".to_string(),
                actual: order.status,
            });
        }
        ...
        for item in &req.items {
            ...
            .bind(ctx.owner_id)
            .bind(order_id)
            .bind(i32::try_from(item.line_no).map_err(|_| Wave4RepositoryError::InvalidQuantity)?)
            .fetch_optional(&mut *tx)
            .await
            .map_err(map_db_error)?
            .ok_or(Wave4RepositoryError::NotFound)?;

            let short_qty = short_pick_qty(planned_qty, item.picked_qty)
                .map_err(|_| Wave4RepositoryError::InvalidQuantity)?;

            let mut cleaned_trace_codes: Vec<String> = item
                .trace_codes
                .iter()
                .map(|code| code.trim().to_string())
                .filter(|code| !code.is_empty())
                .collect();
            cleaned_trace_codes.sort();
            cleaned_trace_codes.dedup();
        ...
        if let Some(lpn) = req.outbound_lpn.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
            let container: Option<(Uuid, String)> = sqlx::query_as(
                r#"
                SELECT id, status
                  FROM lpn_containers
                 WHERE owner_id = $1
            ...
            if let Some((_container_id, status)) = container {
                if status == "idle" {
                    sqlx::query(
                        r#"
        ...
        let response = wms_domain::BatchCompletePickTaskResponse {
            ...
            outbound_lpn: req.outbound_lpn.clone(),
        ...
        .await?;
        append_outbound_audit(&mut tx, ctx, audit, "batch_complete_pick_task", order_id, now).await?;
        tx.commit().await.map_err(map_db_error)?;
        Ok(IdempotentMutation {
            value: response,
            replayed: false,
