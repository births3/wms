...
use wms_domain::{
    ...
    DOCUMENT_TYPE_SALES_OUTBOUND, DOCUMENT_TYPE_SALES_RETURN,
    SYSTEM_DICTIONARY_CONTAINER_QUARANTINE_REASON, SYSTEM_DICTIONARY_CONTAINER_REJECTED_REASON,
...
impl PgSystemDictionaryRepository {
    ...
    pub async fn is_effective_item_enabled(
        &self,
        owner_id: Uuid,
        dict_code: &str,
        item_code: &str,
        effective_at: DateTime<Utc>,
    ) -> Result<bool, SystemDictionaryError> {
        let mut tx = self.begin().await?;
        let enabled =
            effective_item_enabled_in_tx(&mut tx, owner_id, dict_code, item_code, effective_at)
                .await
                .map_err(map_db_error)?;
        tx.commit().await.map_err(map_db_error)?;
        Ok(enabled)
    ...
    pub async fn validate_container_quality_lock_reason(
        &self,
        owner_id: Uuid,
        lock_category: &str,
        reason_code: &str,
        ...
        let dict_code = match lock_category {
            "quarantine" => SYSTEM_DICTIONARY_CONTAINER_QUARANTINE_REASON,
            "rejected" => SYSTEM_DICTIONARY_CONTAINER_REJECTED_REASON,
            _ => return Ok(false),
        };
        self.is_effective_item_enabled(owner_id, dict_code, reason_code, effective_at)
            .await
    }
}
...
pub async fn effective_item_enabled_in_tx(
    tx: &mut Transaction<'_, Postgres>,
    owner_id: Uuid,
    dict_code: &str,
    item_code: &str,
...
pub async fn validate_container_quality_lock_reason_in_tx(
    tx: &mut Transaction<'_, Postgres>,
    owner_id: Uuid,
    lock_category: &str,
    reason_code: &str,
    ...
    effective_item_enabled_in_tx(tx, owner_id, dict_code, reason_code, effective_at).await
}
