...
#[utoipa::path(
    ...
    tag = "master-data",
    params(("Idempotency-Key" = String, Header, description = "客户端生成的幂等键")),
    request_body = BatchGenerateLocationsRequest,
    responses(
        (status = 200, description = "批量生成库位", body = BatchGenerateLocationsResponse),
        (status = 400, description = "缺少或非法幂等键", body = ErrorResponse),
        (status = 401, description = "未登录", body = ErrorResponse),
        (status = 403, description = "权限不足", body = ErrorResponse),
        (status = 409, description = "库位编码或幂等冲突", body = ErrorResponse),
        (status = 422, description = "库位批量生成范围或规则非法", body = ErrorResponse),
    )
)]
#[allow(dead_code)]
pub(crate) fn batch_generate_locations() {}
