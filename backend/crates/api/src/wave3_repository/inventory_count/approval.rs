        ...
        for line in &lines {
            let batch = sqlx::query_as::<_, InventoryCountBatchRow>(
                ...
                SELECT batch.id, location.warehouse_id, batch.product_code,
                       batch.batch_no, batch.expiry_date, batch.status,
                       batch.qty_on_hand, batch.qty_frozen
            ...
            snapshots
                ...
                .and_modify(|(amount, allocated)| {
                    *amount += next_qty;
                    *allocated += batch.qty_frozen;
                })
                .or_insert((next_qty, batch.qty_frozen));
        }
