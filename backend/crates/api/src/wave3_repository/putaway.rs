...
struct PutawayLocationRow {
    ...
    // 6 维候选过滤所需（与 LocationZoneValidationRow 对齐的列）
    location_type: String,
    allows_container: bool,
    zone_code: String,
    allowed_categories: Value,
    is_external_use_zone: bool,
    is_fragrant_zone: bool,
    is_special_drug_zone: bool,
}

impl From<PutawayLocationRow> for LocationZoneValidationRow {
    fn from(row: PutawayLocationRow) -> Self {
        Self {
            location_id: row.location_id,
            location_code: row.location_code,
            location_type: row.location_type,
            allows_container: row.allows_container,
            zone_code: row.zone_code,
            temperature_zone: row.temperature_zone,
            quality_color: row.quality_color,
            allowed_categories: row.allowed_categories,
            is_external_use_zone: row.is_external_use_zone,
            is_fragrant_zone: row.is_fragrant_zone,
            is_special_drug_zone: row.is_special_drug_zone,
            warehouse_id: None,
        }
    }
}
...
impl PgWave3Repository {
    ...
    pub async fn recommend_putaway_locations(
        ...
        let product = sqlx::query_as::<_, PutawayProductPolicyRow>(
            "SELECT storage_condition, volume_cm3, attrs, is_external_use, is_fragrant FROM products WHERE owner_id = $1 AND product_code = $2 AND status = 'active'",
        )
        .bind(ctx.owner_id)
        .bind(&query.product_code)
        .fetch_optional(&mut *tx)
        ...
        let lpn_code = query
            ...
            .map(str::trim)
            .filter(|value| !value.is_empty());
        let lock_category = if let Some(code) = lpn_code {
            sqlx::query_scalar::<_, Option<String>>(
                "SELECT current_lock_category FROM lpn_containers WHERE owner_id = $1 AND lpn_code = $2",
            )
            .bind(ctx.owner_id)
            .bind(code)
            .fetch_optional(&mut *tx)
        ...
        let quality_status =
            wms_domain::recommend_quality_status(lock_category.as_deref(), &query.quality_status);
        let quality_color =
            resolve_quality_color(&mut tx, ctx.owner_id, quality_status, Utc::now()).await?;
        ...
        let locations = sqlx::query_as::<_, PutawayLocationRow>(
            ...
            SELECT
                ...
                location.allows_container,
                zone.zone_code,
                zone.allowed_categories,
                zone.is_external_use_zone,
                zone.is_fragrant_zone,
                zone.is_special_drug_zone,
             ...
             GROUP BY location.id, location.location_code, location.max_volume_cm3,
                      ...
                      location.location_type, location.allows_container,
                      zone.zone_code, zone.allowed_categories,
                      zone.is_external_use_zone, zone.is_fragrant_zone, zone.is_special_drug_zone,
             ...
             ORDER BY same_product DESC, same_product_distance NULLS LAST,
                      available_volume_cm3, MIN(location.putaway_sequence_no) NULLS LAST,
                      location.location_code
             LIMIT $7
            "#,
        )
        ...
        let product_category = product
            .attrs
            .get("category")
            .or_else(|| product.attrs.get("product_category"))
            .and_then(|value| value.as_str())
            .unwrap_or("");
        let locations: Vec<_> = locations
            .into_iter()
            .filter(|row| {
                let passes = |is_container: bool| {
                    wms_domain::recommend_candidate_passes_6d(
                        ...
                        row.is_external_use_zone,
                        product.is_fragrant,
                        row.is_fragrant_zone,
                        &row.location_type,
                        row.allows_container,
                        is_container,
                        lock_category.as_deref(),
                    )
                };
                match lpn_code {
                    Some(_) => passes(true),
                    None => passes(true) || passes(false),
                }
            })
            .collect();
