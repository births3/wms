...
const baseURL = process.env.WMS_WEB_ADMIN_E2E_BASE_URL ?? "http://127.0.0.1:19188";
...
const jwtSigningKey = process.env.WMS_JWT_SECRET ?? `web-admin-m9-real-e2e-${crypto.randomUUID()}`;
const executablePath = process.env.PLAYWRIGHT_CHROMIUM_EXECUTABLE ||
  (fs.existsSync("/usr/bin/google-chrome") ? "/usr/bin/google-chrome" : undefined);

if (!databaseURL) throw new Error("DATABASE_URL or WMS_DB_URL is required for M9 real-data E2E");
...
export default defineConfig({
  testDir: "./e2e",
  testMatch: /web-admin-m9-real\.spec\.ts/,
  timeout: 90_000,
  expect: { timeout: 10_000 },
  workers: 1,
  reporter: [["list"], ["json", { outputFile: "../apps/web-admin/.e2e-artifacts/m9-real/playwright-report.json" }]],
  use: {
  ...
  webServer: [
    ...
    {
      command: `pnpm --dir ${path.join("..", "apps", "web-admin")} dev --host 127.0.0.1 --port ${new URL(baseURL).port || "19188"}`,
      url: baseURL,
      reuseExistingServer: false,
      timeout: 120_000,
      env: {
